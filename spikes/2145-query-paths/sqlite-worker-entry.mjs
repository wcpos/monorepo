...
const ready = sqlite3InitModule().then(s => s.installOpfsSAHPoolVfs({ name: `spike-2145-${MODIFIER}`, initialCapacity: 64 }));
const sqliteBasics = getSQLiteBasicsOo1({ journalMode: 'WAL', openDb: async n => new (await ready).OpfsSAHPoolDb('/' + n) });
let active = false, stats = {};
const originalAll = sqliteBasics.all;
sqliteBasics.all = async (db, q) => {
  const start = performance.now(), rows = await originalAll(db, q);
  if (active) { stats.allMs += performance.now() - start; stats.calls++; stats.rows += rows.length; }
  return rows;
};
async function measure(fn) {
  if (active) return fn(); // count() invokes query(): retain the outer count's timer/counters.
  stats = { allMs: 0, calls: 0, rows: 0 }; active = true;
  const start = performance.now();
  try { return await fn(); } finally { stats.workerMs = performance.now() - start; active = false; }
}
// Deliberately only the brief's fixed ASCII predicates; not a production Mango translator.
function predicate(selector, params) {
  return '(' + Object.entries(selector).map(([field, value]) => {
    if (field === '$and' || field === '$or') return value.map(s => predicate(s, params)).join(field === '$and' ? ' AND ' : ' OR ');
    ...
    if (value.$in) { params.push(...value.$in); return `${column} IN (${value.$in.map(() => '?').join(',')})`; }
    params.push(value.$eq ?? value); return `${column} = ?`;
  }).join(' AND ') + ')';
}
const storage = getRxStorageSQLite({ sqliteBasics, storeAttachmentsAsBase64String: true,
  ...(MODIFIER ? { queryModifier(q, prepared) {
...
const instances = new Map(), create = storage.createStorageInstance.bind(storage);
storage.createStorageInstance = async p => {
  const instance = await create(p); instances.set(p.collectionName, instance);
...
self.addEventListener('message', async ({ data: m }) => {
  ...
  try {
    ...
    if (m.action !== 'stats') {
      ...
      const params = [], where = predicate(m.query.selector, params);
      const sort = m.query.sort.map(s => Object.entries(s).map(([k, v]) => `${k === instance.primaryPath ? 'id' : `JSON_EXTRACT(data, '$.${k}')`} ${v}`).join(',')).join(',');
      const query = `SELECT ${m.count ? 'COUNT(1) AS count' : 'data'} FROM "${instance.tableName}" WHERE ${where}` +
        (m.count ? '' : ` ORDER BY ${sort}${m.query.limit === undefined ? '' : ` LIMIT ${m.query.limit}`}`);
      const q = { query, params, context: { method: 'spike-direct', data: {} } };
      if (m.action === 'explain') result = { query, params, plan: await sqliteBasics.all(db, { ...q, query: 'EXPLAIN QUERY PLAN ' + query }) };
      else result = await measure(async () => {
        const rows = await sqliteBasics.all(db, q);
        return m.count ? { count: rows[0].count } : { documents: rows.map(r => JSON.parse(r.data)) };
      });
    }
    self.postMessage({ type: 'spike-2145-result', id: m.id, result });
  } catch (e) { self.postMessage({ type: 'spike-2145-result', id: m.id, error: String(e.stack || e) }); }
});
