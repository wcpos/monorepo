 ...
 */
const TEST_MODIFIER =
  "(?:only|skip|todo|failing|fails|concurrent|sequential|each|runIf|skipIf)";
/**
...
export function formatFailure(
  ...
  if (deleted.length > 0) {
    ...
    for (const entry of deleted) {
      lines.push(
        `  ${entry.path}  (${entry.count} test${entry.count === 1 ? "" : "s"})`,
      );
    }
  }
  if (shrunk.length > 0) {
    lines.push(
      `${shrunk.length} test file(s) lost more than ${threshold} tests:`,
    );
    for (const entry of shrunk) {
      const moved =
        entry.oldPath === entry.path ? "" : `  (was ${entry.oldPath})`;
      lines.push(
        `  ${entry.path}  ${entry.before} -> ${entry.after} (${entry.delta})${moved}`,
      );
    }
  }
  const total =
...
export function checkTestRemoval({
  env = process.env,
  cwd = repoRoot,
  baseOverride,
} = {}) {
  ...
  if (!context) {
    console.log(
      "✓ check-test-removal: not a pull-request context, nothing to check",
    );
    return;
  }
  ...
  const changes = parseNameStatusZ(
    git(
      ["diff", "--find-renames", "--name-status", "-z", `${base}...HEAD`],
      cwd,
    ),
  );
  const head = git(["rev-parse", "HEAD"], cwd).trim();
  const readSource = (side, filePath) =>
    readBlob(side === "base" ? base : head, filePath, cwd);
  ...
  if (removals.deleted.length === 0 && removals.shrunk.length === 0) {
    ...
    console.log(
      `✓ check-test-removal: ${testFiles} test file(s) touched, none removed`,
    );
    return;
  }
