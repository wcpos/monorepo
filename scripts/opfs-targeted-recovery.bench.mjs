...
const identityWrap = (storage) => ({
	...
	async createStorageInstance(params) {
		...
		instance.findDocumentsById = async (ids, withDeleted) => findDocumentsById(ids, withDeleted);
		instance.bulkWrite = async (documentWrites, context) => bulkWrite(documentWrites, context);
		return instance;
	},
});
...
async function runConfig({ goldenPath, docs, wrapped, kind, warm, batchSize }) {
	...
	try {
		...
		const storage = wrapped ? withTargetedOpfsRecovery(rawStorage) : identityWrap(rawStorage);
		...
		for (const [kind, warm] of [
			...
			for (const batchSize of BATCH_SIZES) {
				...
				const throughputDelta =
					((wrapped.docsPerSec - baseline.docsPerSec) / baseline.docsPerSec) * 100;
				console.log(
