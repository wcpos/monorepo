...
function main() {
	...
	if (Object.keys(importers).length === 0) {
		console.error(
			'check-dep-duplicates: no importers found in pnpm-lock.yaml — lockfile format change?'
		);
		process.exit(1);
	}
	...
	if (duplicates.length > 0) {
		console.error(
			`Found ${duplicates.length} dependency duplicate(s) across workspace packages:\n`
		);
