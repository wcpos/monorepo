...
const SCRIPT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), 'e2e-change-scope.mjs');
...
test('a real code change next to a comment still runs everything', () => {
	const scope = scopeOf(({ append, join }) => {
		append(join('packages/core/src/index.ts'), '// comment\nexport const y = 2;\n');
	});
...
test('a string that merely looks like a comment is NOT treated as one', () => {
	...
	const scope = scopeOf(({ append, join }) => {
		append(join('packages/core/src/index.ts'), 'export const url = "https://x.test";\n');
	});
	assert.equal(scope.behavioural, 'true');
});
...
test('a closed block comment followed by code is NOT comment-only', () => {
	...
	const scope = scopeOf(({ append, join }) => {
		append(join('packages/core/src/index.ts'), '/* note */ export const danger = 1;\n');
	});
	assert.equal(scope.behavioural, 'true');
});
