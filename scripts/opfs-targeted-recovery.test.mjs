...
async function corruptRecord(basePath, id, makeCorruptBytes) {
	...
	await writeFile(documentsPath, Buffer.concat([documents, corruptRecordBytes]));
...
test('falls back to singleton reads when only a combined response is malformed', async () => {
	...
	const instance = {
		...
		findDocumentsById: async (ids) =>
			ids.length > 1 ? '[{malformed' : JSON.stringify(records.filter(({ id }) => ids.includes(id))),
		bulkWrite: async () => ({ error: [] }),
...
test('refuses malformed-document read and write repair when multi-instance', async () => {
	...
	await assert.rejects(recovering.findDocumentsById(['cache:orders'], false), expectedRefusal);
...
test('skips the write preflight for ids already verified clean', async () => {
	...
	await recovering.bulkWrite([{ document: document('cache:customers', 2) }], 'after-read');
	assert.equal(probeCalls, 2);
});
...
test('does not treat tombstones filtered from a read as verified clean', async () => {
	...
	await recovering.bulkWrite([{ document: document('cache:deleted', 1) }], 'filtered-tombstone');
	assert.equal(probeCalls, 1, 'a filtered tombstone id must still preflight');

	await recovering.findDocumentsById(['cache:gone'], true);
	await recovering.bulkWrite([{ document: document('cache:gone', 2) }], 'with-deleted');
...
for (const method of ['findDocumentsById', 'query', 'getChangedDocumentsSince']) {
	test(`${method} validates the result returned after recovery`, async () => {
		...
		try {
			...
			const initial = await rawStorage.createStorageInstance(storageParams(`${method}-initial`));
			...
			const { withTargetedOpfsRecovery } = await import('./opfs-targeted-recovery.mjs');
			const recovering = await withTargetedOpfsRecovery(malformedStorage).createStorageInstance(
				storageParams(`${method}-recovering`)
			);
			const args =
				method === 'findDocumentsById' ? [[record.id], false] : method === 'query' ? [{}] : [10];
			try {
...
test('repairs one malformed record without removing its collection siblings', async () => {
	...
	try {
		const rawStorage = getRxStorageFilesystemNode({ basePath });
		const initial = await rawStorage.createStorageInstance(storageParams('initial'));
		...
		await corruptRecordInPlace(basePath, 'product:6660', () =>
			Buffer.from(`          ,{,${JSON.stringify({ ...records[1], value: 'x' })}`)
		);

		const { withTargetedOpfsRecovery } = await import('./opfs-targeted-recovery.mjs');
		const recoveryStorage = withTargetedOpfsRecovery(getRxStorageFilesystemNode({ basePath }));
		const recovering = await recoveryStorage.createStorageInstance(storageParams('recovering'));
		...
		const syncing = await recoveryStorage.createStorageInstance(storageParams('syncing'));
		const changed = await syncing.getChangedDocumentsSince(10);
		assert.deepEqual(changed.documents.map((item) => item.id).sort(), [...ids].sort());
		await syncing.close();
...
test('drops whitespace-only index rows after cleanup fails', async () => {
	...
	try {
		...
		assert.deepEqual(
			(await recovering.findDocumentsById([dead.id, survivor.id], false)).map((item) => item.id),
			[survivor.id]
		);
		const query = prepareQuery(
			schema,
...
test('repairs a malformed record before retrying its pending write', async () => {
	const basePath = await mkdtemp(join(tmpdir(), 'wcpos-targeted-write-recovery-'));
	...
	try {
		...
		const current = await recovering.findDocumentsById([openOrder.id, siblingOrder.id], false);
		assert.equal(current.find((item) => item.id === openOrder.id)?.value, 'updated-open-order');
...
test('refuses to recover a matching nested object as the whole document', async () => {
	...
	try {
		...
		await assert.rejects(recovering.findDocumentsById([id], false), {
			name: 'SyntaxError',
			message: new RegExp(`targeted recovery failed for ${id}: index-mismatch$`),
		});
		await recovering.close();
	} finally {
		await rm(basePath, { recursive: true, force: true });
...
test('refuses a matching id whose recovered index values differ', async () => {
	const basePath = await mkdtemp(join(tmpdir(), 'wcpos-targeted-index-refusal-'));
	...
	try {
		...
		await initial.cleanup(0);
		await initial.close();
		await corruptRecord(basePath, id, () => Buffer.from(`{"id":"${id}"}garbage`));
...
const laneSchema = {
	...
	required: ['id', 'alpha', 'beta', 'value', '_deleted', '_rev', '_meta', '_attachments'],
	indexes: [
		['_deleted', 'alpha', 'id'],
		['_deleted', 'beta', 'id'],
	],
...
async function shiftSecondaryIndexOffsets(basePath, id, shift, position = 1) {
	const directory = join(basePath, (await readdir(basePath))[0]);
	const indexNames = (await readdir(directory)).filter((name) => name.startsWith('index-')).sort();
...
test('refuses an index rebuild when the primary index is itself unsound', async () => {
	...
	try {
		...
		const primaryPath = join(
			directory,
			(await readdir(directory)).filter((n) => n.startsWith('index-')).sort()[0]
		);
...
test('recovers every concurrent query against the same stale index', async () => {
	...
	try {
		...
		const results = await Promise.all(Array.from({ length: 4 }, () => recovering.query(betaQuery)));
...
test('refuses a rebuild when the primary points at a stale duplicate revision', async () => {
	...
	try {
		...
		await assert.rejects(recovering.query(betaQuery), {
			name: 'SyntaxError',
			message: /index reconciliation refused: uncorroborated-primary-range:lane:ccc$/,
		});
		await recovering.close();
	} finally {
		await rm(basePath, { recursive: true, force: true });
...
test('reports an incomplete index rollback and recovers on retry', async () => {
	...
	try {
		...
		const indexPaths = (await readdir(directory))
			...
			.sort()
			.map((name) => join(directory, name));
		const before = await Promise.all(indexPaths.map((path) => readFile(path, 'utf8')));
		...
		const persistRollbackIndex = rollbackIndex.persistInMemoryRows.bind(rollbackIndex);
...
test('refuses a rebuild when index ID sets differ despite equal counts', async () => {
	...
	try {
		...
		const metaPath = join(
			directory,
			(await readdir(directory)).filter((n) => n.startsWith('index-')).sort()[2]
		);
...
test('rebuilds a secondary index that duplicates one document and drops another', async () => {
	...
	try {
		...
		const betaPath = join(
			directory,
			(await readdir(directory)).filter((n) => n.startsWith('index-')).sort()[1]
		);
