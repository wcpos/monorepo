...
test("TypeScript tests consumed by package builds remain build inputs", () => {
  ...
  assert.ok(
    inputsFor("@wcpos/sync-core#build").has(
      "src/applyReplicationActions.test.ts",
    ),
  );
  assert.ok(
    inputsFor("@wcpos/sync-engine#build").has(
      "src/automatic-tick-gate.test.ts",
    ),
  );
});
...
test("JavaScript plugin tests do not invalidate the main app build", () => {
  assert.ok(
    !inputsFor("@wcpos/main#build").has("plugins/with-printer-support.test.js"),
  );
  assert.ok(
    !inputsFor("@wcpos/main#build").has(
      "plugins/with-wedge-key-events.test.js",
    ),
  );
});
