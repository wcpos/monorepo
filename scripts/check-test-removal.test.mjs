...
test('counts it/test declarations and their modifiers', () => {
	...
	assert.equal(countTestDeclarations("it.only('a', () => {})\nit.skip('b', () => {})"), 2);
...
test('parses NUL-delimited name-status records, renames included', () => {
	const stdout = 'M\0a.test.ts\0D\0b.test.ts\0R096\0old.test.ts\0new.test.ts\0A\0c.test.ts\0';
...
const sourceWith = (count) =>
	Array.from({ length: count }, (_, index) => `it('case ${index}', () => {});`).join('\n');

/** A reader over `{ 'base:path': source, 'head:path': source }`. */
const readerFor = (blobs) => (side, filePath) => blobs[`${side}:${filePath}`] ?? '';
...
test('a deleted test file trips regardless of how few tests it held', () => {
	...
	const { deleted, shrunk } = findRemovals(changes, readerFor({ 'base:a.test.ts': sourceWith(1) }));
	assert.equal(shrunk.length, 0);
	assert.deepEqual(deleted, [{ path: 'a.test.ts', count: 1 }]);
});

test('a deleted NON-test file is ignored', () => {
	const changes = [{ status: 'D', oldPath: 'src/index.ts', path: 'src/index.ts' }];
...
test('a pure rename does NOT trip — the content is identical', () => {
	...
	const changes = [{ status: 'R', oldPath: 'old/a.spec.ts', path: 'new/deep/a.spec.ts' }];
...
test('a rename that also loses a few tests stays under the threshold', () => {
	...
	const changes = [{ status: 'R', oldPath: 'old/a.spec.ts', path: 'new/a.spec.ts' }];
	...
	assert.equal(shrunk.length, 0, `a drop of exactly ${COUNT_DROP_THRESHOLD} is tolerated`);
});
...
test('a brand-new test file never trips', () => {
	...
	const { deleted, shrunk } = findRemovals(changes, readerFor({ 'head:a.test.ts': sourceWith(9) }));
	assert.equal(deleted.length + shrunk.length, 0);
});
...
test('no PR context outside a pull_request event', () => {
	...
	assert.equal(prContext({ GITHUB_EVENT_NAME: 'push', GITHUB_BASE_REF: 'next' }), null);
	assert.equal(prContext({ GITHUB_EVENT_NAME: 'pull_request' }), null, 'no base ref');
	assert.deepEqual(prContext({ GITHUB_EVENT_NAME: 'pull_request', GITHUB_BASE_REF: 'next' }), {
		baseRef: 'next',
		eventPath: undefined,
	});
});
...
test('end to end: the event base SHA wins over an advanced base ref', (t) => {
	...
	repo.write('event.json', JSON.stringify({ pull_request: { base: { sha: base }, labels: [] } }));
...
test('end to end: a label in a partially malformed event acknowledges removal', (t) => {
	...
	assert.equal(run(repo.dir, { GITHUB_EVENT_PATH: path.join(repo.dir, 'event.json') }), null);
});
...
test('end to end: moving a spec across directories passes', (t) => {
	...
	execFileSync('git', ['mv', 'e2e/checkout.spec.ts', 'src/e2e/pos-checkout.spec.ts'], {
		cwd: repo.dir,
	});
