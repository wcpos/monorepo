...
const appTokenChecks = [
	...
	},
	{
		...
		pattern: /^\s*private-key:\s*\$\{\{\s*secrets\.WCPOS_BOT_PRIVATE_KEY\s*\}\}\s*$/m,
	},
];

const actionStepPattern = /^\s*(?:-\s*)?uses:\s*actions\/create-github-app-token@/m;
const checkoutStepPattern = /^\s*(?:-\s*)?uses:\s*actions\/checkout@/m;
const checkoutTokenPattern = /^\s*token:\s*\$\{\{\s*steps\.app-token\.outputs\.token\s*\}\}\s*$/m;
