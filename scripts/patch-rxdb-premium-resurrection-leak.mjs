...
const esm = join(packageRoot, 'dist/esm/plugins/storage-abstract-filesystem/bulk-write.js');
const cjs = join(packageRoot, 'dist/cjs/plugins/storage-abstract-filesystem/bulk-write.js');

if (!existsSync(esm) || !existsSync(cjs)) {
	throw new Error('rxdb-premium dist not found — run after the package postinstall');
}
