...
for (const [name, requiredText, expectedFailure] of requiredWiring) {
	test(`rejects a workflow missing ${name}`, () => {
		...
		assert.throws(() => checkBumpSubmodulesWorkflow(incompleteWorkflow), expectedFailure);
	});
}
...
test('rejects the app token reference in an unrelated step', () => {
	const misplacedToken = validWorkflow.replace(
		'      token: ${{ steps.app-token.outputs.token }}\n',
		''
	).concat(`
...
test('rejects a required value that appears only in a comment', () => {
	const commentedOwner = validWorkflow.replace('      owner: wcpos\n', '      # owner: wcpos\n');

	assert.throws(() => checkBumpSubmodulesWorkflow(commentedOwner), /targets the wcpos owner/);
});
