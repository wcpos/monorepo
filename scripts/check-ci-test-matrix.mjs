...
const SKIP_DIRS = new Set(['node_modules', 'dist', 'build', 'coverage', 'ios', 'android']);
...
export const OTHER_LANES = [
	{
		...
		reason: 'Playwright specs — run against a deployed preview by the e2e job in deploy.yml',
	},
...
export const ALLOWLIST = [
	{
		...
		reason: 'gallery specs need the sibling woocommerce-pos checkout — TODO(#1017)',
	},
];
...
export function readSubmodulePaths(root = repoRoot) {
	...
	return new Set([...text.matchAll(/^\s*path\s*=\s*(.+?)\s*$/gm)].map((match) => match[1]));
}
...
const LOOP_TEST_INVOCATION = /\b(?:jest|vitest|playwright|maestro)\b|\bpnpm\b[^\n;&|]*\btests?\b/;
...
export function surveyPackages(root = repoRoot) {
	...
	return resolveWorkspacePackages(globs, root).map((dir) => {
		...
		for (const file of all) {
			const lane = OTHER_LANES.find((entry) => file.startsWith(`${entry.dir}/`));
...
export function detectLanes(sources, packages) {
	...
	for (const [source, rawText] of sources) {
		...
		for (const match of text.matchAll(/--filter[= ]+'?"?(@wcpos\/[\w-]+)'?"?(?=([^\n]*))/g)) {
...
export function readLaneSources(root = repoRoot) {
	...
	return [...sources, ...invoked.map(([name, body]) => [`package.json#${name}`, body])];
}
...
export function checkCiTestMatrix(root = repoRoot) {
	...
	const missingOtherLanes = OTHER_LANES.filter(
		(entry) => !entry.invocation.test(sourceText.get(`.github/workflows/${entry.workflow}`) ?? '')
	);
	if (missingOtherLanes.length > 0) {
		throw new Error(
			missingOtherLanes
				.map((entry) => `${entry.reason} — NO CI lane invocation found in ${entry.workflow}`)
				.join('\n')
		);
	}
	...
	for (const row of rows) {
		...
		const other = row.entry.otherLane.length > 0 ? ` +${row.entry.otherLane.length} e2e` : '';
		const note = row.status === 'allowlisted' ? ` — ${allowed.get(row.entry.dir).reason}` : '';
		console.log(
	...
	console.log(`✓ every workspace package with tests runs in a CI lane (${rows.length} packages)`);
}
