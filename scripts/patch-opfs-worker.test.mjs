...
const patcher = fileURLToPath(new URL('./patch-opfs-worker.mjs', import.meta.url));
const shippedWorker = new URL('../apps/main/public/opfs.worker.js', import.meta.url);
...
test('patched worker preserves native option and empty-write semantics', () => {
	...
	FakeAccessHandle.prototype.write = function (buffer, options) {
		calls.push({ at: options?.at, length: buffer.byteLength });
		return buffer.byteLength === 0 ? 0 : Math.max(1, Math.floor(buffer.byteLength / 2));
	};
