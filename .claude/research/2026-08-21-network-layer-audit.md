...
**Refund pagination (loose end #1).** `form.tsx:174-189` reads `x-wp-totalpages` with a silent `|| 1` fallback and fans out pages 2..N. Under a header-stripping proxy it fetches page 1 only. Mitigating fact the handoff didn't have: `per_page: 100`, so it takes an order with >100 refunds to lose data — rare, but the lost rows feed `maxRefundable` (`form.tsx:203-208`), i.e. under-counting **enables over-refunding real money**. The audit's original recommendation was loop-until-short-page (no header, no envelope dependency). **Superseded same-day by PR #1418** (parallel session): a hook-level envelope fallback in `useRestHttpClient` — `_wcpos_envelope=1` on GETs plus a shape-guarded `unwrapResponseEnvelope` that backfills stripped pagination headers, mirroring the engine lane's B9 seam. Reviewed and endorsed: better scope (covers every pagination-header read on the axios lane, one idiom across both lanes). Residual accepted edge: pre-envelope servers behind stripping proxies — out-of-contract for the hostile-headers program. Reconciliation on #1413.
...
| 3 | Refund list / axios-lane pagination under stripped headers — **superseded by PR #1418** (hook-level envelope fallback in `useRestHttpClient`; endorsed) | all | Closes the B9 Tier-2 gap for the whole axios lane; prevents header-stripped under-count feeding `maxRefundable` (over-refund risk) | negligible | landed in #1418 |
...
| 3 | [#1413](https://github.com/wcpos/monorepo/issues/1413) — Refund form pagination under stripped headers — closes via [PR #1418](https://github.com/wcpos/monorepo/pull/1418) (hook-level envelope fallback, endorsed over this audit's original loop-until-short-page recommendation) |
