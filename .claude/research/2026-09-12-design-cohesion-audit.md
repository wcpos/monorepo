...
- **Do not label spinner strokes as card borders:** `border-[6px]` is one terminal progress ring (`packages/core/src/screens/main/pos/checkout/tender/terminal-leg-view.tsx:351`), and the sole `border-4` is the camera scan-feedback flash outline (`packages/core/src/screens/main/pos/products/camera-scanner-panel.tsx:197-202`), not an ordinary card border.
...
1. **One semantic theme pipeline:** preserve the colour-role names and light/dark/theme contract (`apps/main/global.css:40-92,152-153,172-173`), rather than replacing them with a “unified” hardcoded palette; the selected graph already contains **950 semantic declarations**, with **76 S utility forms** using semantic colours.
