...
- Instruments already prepared (all in the research folder above; copy them into a scratch directory under `/tmp/overnight` before use):
  - `pull-rows.sh` — pulls the app's storage directory over USB and prints the "JS thread timing report" rows (one per minute: `maxMs`, stall buckets at 16/50/100/250/500/1000 ms, and the busiest storage collection×method). `bash pull-rows.sh 96D11509-CE68-5507-9AA5-713657C0B14A /tmp/overnight/pulls/pull-$(date +%H%M%S)`. Trust `maxMs` and the buckets; `blockedMs` is inflated on this ProMotion iPad while idle.
...
## Paul's named sluggish spots — exercise these first, and again every cycle

These six are what Paul feels on the iPad today. Each gets its own flow, run at least ten times over the night, timed tap-to-visible with `testID` selectors (read the component files to find the ids; add none). For each, the report needs median and worst tap-to-response, the probe row for the minute, any error it fired, and one Hermes profile (60 s, one per scenario at most, and only three profiles for the whole night — pick the three worst). The hypotheses are mine from today's data; confirm or reject them with evidence, do not restate them.

1. **Store select → "Open POS".** After login, selecting the store and tapping Open POS takes 3–4 s before the POS screen appears. Time from the tap to the products grid's first row being visible. Hypothesis: the scope switch opens every collection on the store database serially (today's login minute is 90 `_rxdb_internal` reads and 74 writes at 100–900 ms each) plus the first products query; split the time between database open and first render.
2. **Reports page.** The slowest screen: it hangs for a while after tapping Reports. Time tap → first report content visible, and note how long the JS thread is blocked (probe row). Hypothesis: it queries whole collections (orders, line items) on the JS thread and builds aggregates in one synchronous pass; capture one profile here.
3. **Rapid taps on a simple product.** On the POS grid, tap one simple product about 20 times as fast as possible. It gets laggy and jumpy and "overwhelmed". Use Maestro `repeat` with no delay. Measure the time from the last tap until the cart's line quantity settles at the final value and the UI is responsive again, and count how many cart writes the probe row shows (`orders` / `recordMutations` `bulkWrite` calls). Hypothesis: every tap writes the order document and its mutation record and re-renders the whole cart; the write queue coalesces but the renders don't. Watch the error listener for anything thrown mid-burst.
4. **Variation popovers.** Tap a variable product so its variation popover opens, then without closing it tap the next variable product, and the next, four or five in a row, then add a variation to the cart from whichever popover is open. Paul sees several popovers open at once and the UI "freaks out". Record what actually happens (how many popovers are on screen, whether the add goes through, any error), time each popover's tap → visible, and note the recovery time. Hypothesis: the popover is per-row state that a tap on another row does not dismiss, so N popovers mount N variation lists.
5. **List view ↔ tile view switch.** Toggle between the products list view and the tile (grid) view repeatedly, five times each way with a 2 s pause. Time each switch tap → first row visible in the new layout. Hypothesis: the switch unmounts and remounts the whole product list (today's heap had 42 k mounted React fibers), so each toggle re-creates every row.
6. **List view: expanding a variable product's variations.** In list view, tap the expand control on a variable product. The expand is very janky; the collapse is better. Time tap → last variation row visible, and repeat on products with few and many variations. Hypothesis: the expand renders every variation row at once and each row subscribes to its own observables (today's heap had ~40 k live RxJS subscriptions); compare expand time against variation count.

If the web build is reachable from the Mac (Metro for web on another port, or a dev store URL), time scenarios 1, 2, 5 and 6 there once, with the same actions, so the report can show the gap Paul means by "the way it is on the web". If it is not reachable, say so; do not set it up.

## Driving the device

1. Install the same pinned Maestro release verified by `.github/workflows/e2e-native.yml`:
   ```sh
