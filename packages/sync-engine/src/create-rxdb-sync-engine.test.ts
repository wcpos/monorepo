...
import { createRxdbSyncEngine, type EngineEvent, type RxdbSyncEnginePorts, type StoreScopeIdentity } from './create-rxdb-sync-engine';
...
import { scopeDatabaseName, scopeKeyFor } from '@woo-rxdb-lab/sync-core';
...
async function insertOrder(db: { collections: Record<string, unknown> }, id: string): Promise<void> {
...
async function enqueueMutation(db: { collections: Record<string, unknown> }, mutationId: string): Promise<void> {
...
describe('createRxdbSyncEngine — slice 2 scope lifecycle', () => {
  ...
  it('A→B→A switch is pause/resume: every scope keeps its data and mutation queue', async () => {
    ...
    const pending = scopeA2.database.collections.recordMutations as { count(): { exec(): Promise<number> } };
  ...
  it('cross-site switch is an exception — multi-site is a new engine', async () => {
    ...
    const respelled = await engine.scope.switch({ site: 'HTTP://lab.example.test/', storeId: 1, cashierId: a.cashierId });
  ...
  it('resetCollection empties exactly the named collection and leaves the rest', async () => {
    ...
    const pending = scope.database.collections.recordMutations as { count(): { exec(): Promise<number> } };
    expect(await pending.count().exec()).toBe(1); // queue untouched by a data reset
    expect(events).toContainEqual({ type: 'collection-reset', scopeId: scope.scopeId, collection: 'orders' });
    await engine.dispose();
  });
  ...
  it("resetCollection('orders') rewinds the syncCheckpoints custom-pull checkpoint — the persisted drain's cursor store (#430 phase 2)", async () => {
    ...
    const saved = normalizeCheckpoint({ sequence: 42, orderId: 7, updatedAtGmt: '2026-07-01T00:00:00.000Z', revision: 'r42' });
  ...
  it("resetCollection('mutations') with pending mutations is a VALUE: needs-confirmation, queue intact", async () => {
    ...
    const refused = await engine.scope.resetCollection('mutations', { beforeDrop: refusedBeforeDrop });
    ...
    const confirmedBeforeDrop = vi.fn(async (active: typeof scope) => {
      const live = active.database.collections.recordMutations as { count(): { exec(): Promise<number> } };
    ...
    const recreated = scope.database.collections.recordMutations as { count(): { exec(): Promise<number> } };
  ...
  it('unknown collection is an exception (caller misuse), not a value', async () => {
    ...
    await engine.ready;
    await expect(
      engine.scope.resetCollection('engineKv' as never),
  ...
  it('fails closed when the connectivity adapter throws', async () => {
    ...
    const engine = engineWith({ mode: 'manual', connectivity: () => { throw new Error('adapter boom'); }, diagnostics });
    await engine.ready;
    expect(engine.status()).toMatchObject({ connectivity: 'offline', gatedBy: 'offline' });
    await expect(engine.sync('write-drain')).resolves.toMatchObject({ status: 'skipped', reason: 'offline' });
    expect(diagnostics).toHaveBeenCalledWith(expect.objectContaining({ type: 'engine.connectivity-error' }));
    await engine.dispose();
  });
  ...
  it('gate2 #516 item 6: the seed lanes tick BEFORE the scheduler drain in a full sync() — no self-seeded work left pending', async () => {
    ...
    expect(lanes['order-window-seed'].lastTick!.atMs).toBeLessThan(lanes['scheduler-drain'].lastTick!.atMs);
    expect(lanes['reference-seed'].lastTick!.atMs).toBeLessThan(lanes['scheduler-drain'].lastTick!.atMs);
    expect(lanes['write-drain'].lastTick!.atMs).toBeLessThan(lanes['order-window-seed'].lastTick!.atMs);
    await engine.dispose();
  });
  ...
  it('projects host-facing scope events and guard stats without exposing engine event names', async () => {
    ...
    const engine = engineWith(undefined, a);
    await engine.ready;
    const events: Array<{ type: string; scopeId: string; collection?: string }> = [];
  ...
  it('a reset racing a pending switch resets the FIFO-settled scope, never the outgoing one', async () => {
    ...
    expect(outcome).toBe('reset');
    expect(events).toContainEqual({ type: 'collection-reset', scopeId: scopeB.scopeId, collection: 'orders' });
