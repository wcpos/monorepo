...
export type EngineSourceFetcher = (url: string, init?: { signal?: AbortSignal }) => Promise<Response>;
...
async function getEnvelope<Row>(
  ...
  if (!response.ok) {
    throw new ChangeSignalPoisonError(`${label} failed: HTTP ${response.status}`, path, response.status);
  }
  ...
  if (contentType !== '' && !contentType.toLowerCase().includes('application/json')) {
    throw new ChangeSignalPoisonError(`${label}: non-JSON response (content-type: ${contentType})`, path, response.status);
  }
  ...
  try {
    ...
    throw new ChangeSignalPoisonError(`${label}: response body is not valid JSON`, path, response.status);
  }
  ...
  if (typeof parsed !== 'object' || parsed === null || !Array.isArray((parsed as { changes?: unknown }).changes)) {
    throw new ChangeSignalPoisonError(`${label}: not a changes envelope (missing 'changes' array)`, path, response.status);
  }
...
function toHybridCollection(value: string | undefined): HybridCollection | undefined {
  return value !== undefined && SUPPORTED_HYBRID_COLLECTIONS.has(value) ? (value as HybridCollection) : undefined;
}
...
export function createLiveChangeSignalSource(
  ...
  return {
    ...
    async revisionHashForIds({ ids, collection }) {
      ...
      const rows: Array<{ id: number; revision: string }> = [];
      const limit = 200;
