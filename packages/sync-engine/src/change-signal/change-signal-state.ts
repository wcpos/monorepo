...
import type { ReplicationActions } from '@wcpos/sync-core';

type EngineState = ReplicationActions['nextState'];

type BaselineEntries = [
	string,
	EngineState['baselineDigests'] extends Map<string, infer V> ? V : never,
][];
...
function isValidBucketDigest(value: unknown): boolean {
	...
	if (v.detector === 'range-checksum') {
		return (
			...
			(v.ids === undefined ||
				(Array.isArray(v.ids) && v.ids.every((id) => Number.isSafeInteger(id))))
		);
	}
	return false;
}
	...
	try {
		...
		return {
			...
			baselineDigests: new Map(
				(parsed.baselineDigests as unknown as [string, Record<string, unknown>][]).map(
