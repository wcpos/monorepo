...
export function createChangeSignalLane(deps: ChangeSignalLaneDeps): ChangeSignalLane {
  ...
  async function fetchHeadSequence(): Promise<number> {
    const response = await sourceFetcher(`${deps.syncBaseUrl}/changes/sequence-log?collection=all&since=0&limit=1`);
  ...
  async function engineFor(scopeId: string): Promise<HybridChangeSignalEngine> {
    ...
    const initial = restored ?? { initialCursor: { sequence: await fetchHeadSequence() }, baselineDigests: undefined };
    const engine = createHybridChangeSignalEngine({
      source: createLiveChangeSignalSource({ syncBaseUrl: deps.syncBaseUrl, fetcher: sourceFetcher }),
      ...
      configSource: createConfigFingerprintLiveSource({ syncBaseUrl: deps.syncBaseUrl, fetcher: sourceFetcher }),
      initialCursor: initial.initialCursor,
      ...(initial.baselineDigests !== undefined ? { baselineDigests: initial.baselineDigests } : {}),
      ...(restored?.configBaseline !== undefined ? { configBaseline: restored.configBaseline } : {}),
  ...
  async function runTick(signal?: AbortSignal): Promise<ChangeSignalReport> {
    ...
    try {
      return await deps.manager.runGuarded(async (bound) => {
        ...
        if (!database) {
          return { lane: 'change-signal' as const, status: 'skipped' as const, reason: 'scope database not open' };
        }
        // Bind BEFORE engineFor — a cold start's head-priming fetch rides this
        // tick's scope ticket too.
        const tickFetcher: Fetcher = signal === undefined
          ? deps.fetcher
          : async (url, init) => {
        ...
        const wrote = await bound.guardWrite(async () => {
          ...
          await applyReplicationActions(actions, buildReplicationHandlers({
            database,
            fetch: activeFetch as Fetcher,
            syncBaseUrl: deps.syncBaseUrl,
            persistState: async (state) => {
              await deps.writeBlob(scopeId, CHANGE_SIGNAL_STATE_KEY, serializeChangeSignalState(state));
            },
            log: (line) => deps.diagnostics({ type: 'signal.log', level: 'debug', message: line }),
            observe: deps.diagnostics,
          }));
        });
        if (wrote === 'dropped') {
          report = { lane: 'change-signal', status: 'skipped', reason: 'scope moved mid-tick (writes dropped)' };
        }
        lastError = null;
        return report;
      });
