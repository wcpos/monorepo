...
async function fetchBody(ctx: HandlerContext, path: string, params: Record<string, string>): Promise<unknown> {
...
async function fetchPayloadPage(ctx: HandlerContext, path: string, params: Record<string, string>): Promise<Array<Record<string, unknown>>> {
...
async function pullByIds(
  ...
  for (let at = 0; at < ids.length; at += INCLUDE_CHUNK) {
    ...
    const payloads = d.parse(await fetchBody(ctx, d.pullPath, {
      include: chunk.join(','),
      per_page: String(chunk.length),
    }));
    if (payloads.length < chunk.length) {
      throw new Error(`${d.pullPath} include pull returned ${payloads.length}/${chunk.length} records — failing the tick so the cursor cannot advance past an unapplied record`);
    }
    const documents = payloads.map((payload) => d.project(payload));
    if (persist) await persist(documents);
    else {
      assertBulkSuccess(await collection.bulkUpsert(documents as never[]), 'change-signal-handlers upsert');
      const rows = documents.flatMap((document) => manifestRowOf(document) ? [manifestRowOf(document)!] : []);
      if (rows.length > 0) {
        const manifestName = d.collection === 'customers' ? 'existenceManifestCustomers' : 'existenceManifest';
...
async function removeByWooIds(ctx: HandlerContext, name: string, wooIdField: string, ids: number[]): Promise<number> {
  if (ids.length === 0) return 0;
  const collection = collectionOf(ctx, name);
  const docs = await collection.find({ selector: { [wooIdField]: { $in: ids } } as never }).exec();
  if (docs.length > 0) {
    assertBulkSuccess(await collection.bulkRemove(docs.map((doc) => (doc as { primary: string }).primary)), 'change-signal-handlers remove');
  }
...
async function fetchAll(ctx: HandlerContext, path: string): Promise<Array<Record<string, unknown>>> {
  const all: Array<Record<string, unknown>> = [];
  for (let page = 1; ; page += 1) {
    const payloads = await fetchPayloadPage(ctx, path, { per_page: String(REFRESH_PAGE_SIZE), page: String(page) });
    all.push(...payloads);
...
type TargetedEffects = { pull: (ids: number[]) => Promise<number>; remove: (ids: number[]) => Promise<number> };
  ...
  for (const descriptor of COLLECTION_DESCRIPTORS) {
    switch (descriptor.shape) {
      ...
      case 'upsert-refresh': {
        ...
        deleteTaxRates = (ids) =>
          removeByWooIds(ctx, descriptor.collection, 'wooTaxRateId', ids);
        break;
      }
...
async function loadSyncedProducts(ctx: HandlerContext, collection: HybridCollection): Promise<SyncedDocument[]> {
...
export function buildReplicationHandlers(ctx: HandlerContext): ReplicationActionHandlers {
  ...
  const handlers: ReplicationActionHandlers = {
    ...
    applyBarcodeIndex: (collection, index) => {
      ctx.log(`barcode index for ${collection}: ${index.index.size} entries (no scan-index store in the engine — logged only)`);
      return true;
    },
