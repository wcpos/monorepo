...
function createCollection(documents: QueryTotalRequestState[] = []) {
  ...
  const find = vi.fn((_query?: unknown) => ({
    exec: vi.fn(async () => [...stored.values()].map((document) => ({ toJSON: () => ({ ...document }) }))),
  }));
    ...
    exec: vi.fn(async () => {
      ...
      return {
        ...
        incrementalModify: vi.fn(
          async (mutator: (current: QueryTotalRequestState & { _deleted?: boolean }) => QueryTotalRequestState & { _deleted?: boolean }) => {
...
function repositoryFor(documents: QueryTotalRequestState[] = []) {
  ...
  const repository = new RxQueryTotalRequestStateRepository({ queryTotalRequestStates: fixture.collection } as never);
...
describe('RxQueryTotalRequestStateRepository', () => {
  ...
  it('does not expose the persisted schema marker when reading request states', async () => {
    ...
    await expect(repository.readForQueryKeys(['orders:a'])).resolves.toEqual([state({ queryKey: 'orders:a' })]);
  });

  it('returns runnable expired owner and retry-ready states', async () => {
    const { repository } = repositoryFor([
      state({ queryKey: 'active-owner', status: 'in-flight', claimedUntilMs: 2_000, retryAfterMs: null }),
      state({ queryKey: 'expired-owner', status: 'in-flight', claimedUntilMs: 1_000, retryAfterMs: null }),
      state({ queryKey: 'backoff-active', status: 'failed', ownerId: null, claimedUntilMs: null, retryAfterMs: 2_000 }),
      state({ queryKey: 'retry-ready', status: 'failed', ownerId: null, claimedUntilMs: null, retryAfterMs: 1_000 }),
    ]);
  ...
  it('preserves Woo request metadata when reading runnable states', async () => {
    ...
    const { repository } = repositoryFor([
      state({ queryKey: 'expired-owner', status: 'in-flight', claimedUntilMs: 1_000, retryAfterMs: null, request }),
    ]);
  ...
  it('does not remove a newer state for the same query key', async () => {
    const older = state({ queryKey: 'orders:a', ownerId: 'tab-a', attempt: 1, claimedUntilMs: 1_000, updatedAtMs: 1_000 });
    const newer = state({ queryKey: 'orders:a', ownerId: 'tab-b', attempt: 2, claimedUntilMs: 2_000, updatedAtMs: 1_500 });
  ...
  it('claims a runnable request state only when the stored state still matches the expected state', async () => {
    const expired = state({ queryKey: 'orders:a', ownerId: 'tab-a', attempt: 1, claimedUntilMs: 1_000, updatedAtMs: 1_000 });
    const claimed = state({ ...expired, ownerId: 'tab-b', attempt: 2, claimedUntilMs: 2_000, updatedAtMs: 1_500 });
  ...
  it('does not claim over a newer request owner attempt', async () => {
    const older = state({ queryKey: 'orders:a', ownerId: 'tab-a', attempt: 1, claimedUntilMs: 1_000, updatedAtMs: 1_000 });
    const newer = state({ queryKey: 'orders:a', ownerId: 'tab-c', attempt: 3, claimedUntilMs: 3_000, updatedAtMs: 2_000 });
    const claimedFromOlder = state({ ...older, ownerId: 'tab-b', attempt: 2, claimedUntilMs: 2_000, updatedAtMs: 1_500 });
    const { repository, stored } = repositoryFor([newer]);
  ...
  it('marks a request failed only when the stored state still matches the owner attempt', async () => {
    const inFlight = state({ queryKey: 'orders:a', ownerId: 'tab-a', attempt: 1, claimedUntilMs: 1_000, updatedAtMs: 1_000 });
    const failed = state({ ...inFlight, status: 'failed', ownerId: null, claimedUntilMs: null, retryAfterMs: 2_000, updatedAtMs: 1_500 });
  ...
  it('does not mark a newer request owner failed from an older owner attempt', async () => {
    const older = state({ queryKey: 'orders:a', ownerId: 'tab-a', attempt: 1, claimedUntilMs: 1_000, updatedAtMs: 1_000 });
    const newer = state({ queryKey: 'orders:a', ownerId: 'tab-b', attempt: 2, claimedUntilMs: 2_000, updatedAtMs: 1_500 });
    const failedOlder = state({ ...older, status: 'failed', ownerId: null, claimedUntilMs: null, retryAfterMs: 2_500, updatedAtMs: 2_000 });
    const { repository, stored } = repositoryFor([newer]);
  ...
  it('does not remove or mark failed state when request metadata differs', async () => {
    ...
    const expectedRequest = { ...storedRequest, params: { status: 'completed', page: 1, per_page: 1 } };
    ...
    const failedState = state({ ...expectedState, status: 'failed', ownerId: null, claimedUntilMs: null, retryAfterMs: 2_000 });
