...
import { createRxdbSyncEngine, type RxdbSyncEngine, type StoreScopeIdentity } from './create-rxdb-sync-engine';
    ...
    return new Response(JSON.stringify({
      documents: [{
        ...
        payload: {
          ...
          meta_data: [{ key: '_woocommerce_pos_uuid', value: `77777777-7777-4777-8777-${String(id).padStart(12, '0')}` }],
        },
        sync: { revision: checkpoint.revision, partial: false, source: 'custom-pull', checkpoint },
...
function engineWith(fetch: (url: string, init?: { signal?: AbortSignal }) => Promise<Response>): RxdbSyncEngine {
...
describe('require() for orders (slice 5f — the durable path)', () => {
  ...
  it('pulls a missing order via a persisted task + drain, then serves local without a second fetch', async () => {
    ...
    const first = await engine.require({ id: 'pos-open-order', collection: 'orders', kind: 'targeted-records', wooIds: [7] }).ready;
    ...
    const second = await engine.require({ id: 'pos-open-order', collection: 'orders', kind: 'targeted-records', wooIds: [7] }).ready;
  ...
  it('force-refreshes a resident order when the caller requires a re-fetch', async () => {
    ...
    await engine.require({ id: 'resident', collection: 'orders', kind: 'targeted-records', wooIds: [7] }).ready;
    const pullsBefore = server.state.pulls;
  ...
  function orchestrationHarness(now?: () => number) {
    ...
    const database = {
      collections: {
        orders: {
          find: () => ({
            exec: async () => residentOrderIds.map((wooOrderId) => ({
              toJSON: () => ({ wooOrderId }),
            })),
          }),
        },
    ...
    const plane = createRequirePlane({
      ...
      manager: { runGuarded: async (operation: (captured: typeof bound) => Promise<unknown>) => operation(bound) } as never,
    ...
    return {
      ...
      boundFetch,
      diagnostics,
      setResidentOrderIds: (ids: number[]) => { residentOrderIds = ids; },
    };
  }

  it('disables completed-task dedupe when a required order is absent', async () => {
    const harness = orchestrationHarness();
    const seed = vi.spyOn(orderTaskSeeder, 'seedTargetedOrderSchedulerTask').mockResolvedValue({ ...EMPTY_SEED_RESULT, requeued: 1 });
    ...
    await harness.plane.require({ id: 'dedupe', collection: 'orders', kind: 'targeted-records', wooIds: [8] }).ready;
  ...
  it('runs the durable order drain with the scope-bound fetcher', async () => {
    const harness = orchestrationHarness();
    vi.spyOn(orderTaskSeeder, 'seedTargetedOrderSchedulerTask').mockResolvedValue({ ...EMPTY_SEED_RESULT, inserted: 1 });
    const drain = vi.spyOn(schedulerDrain, 'runEngineSchedulerDrain').mockImplementation(async () => {
      harness.setResidentOrderIds([8]);
      return { ...EMPTY_DRAIN_RESULT, succeeded: 1 };
    });

    await harness.plane.require({ id: 'bound-fetch', collection: 'orders', kind: 'targeted-records', wooIds: [8] }).ready;
  ...
  it('routes an order query requirement through the persisted filter seeder and drain', async () => {
    const harness = orchestrationHarness();
    const seed = vi.spyOn(orderTaskSeeder, 'seedOrderFilterSchedulerTask').mockResolvedValue({ ...EMPTY_SEED_RESULT, inserted: 1 });
    vi.spyOn(schedulerDrain, 'runEngineSchedulerDrain').mockResolvedValue({ ...EMPTY_DRAIN_RESULT, succeeded: 1 });
    ...
    expect(seed).toHaveBeenCalledWith(expect.objectContaining({ status: 'processing', search: '', limit: 50 }));
  });

  it('routes a full order refresh through the persisted greedy seeder and drain', async () => {
    const harness = orchestrationHarness();
    const seed = vi.spyOn(orderTaskSeeder, 'seedOrderSchedulerTasks').mockResolvedValue({ ...EMPTY_SEED_RESULT, inserted: 1 });
    vi.spyOn(schedulerDrain, 'runEngineSchedulerDrain').mockResolvedValue({ ...EMPTY_DRAIN_RESULT, succeeded: 1, totalDocuments: 25, totalRequests: 2 });
    ...
    expect(seed).toHaveBeenCalledWith(expect.objectContaining({ perPage: 250, completedDedupeForMs: 0 }));
  ...
  it('emits per-batch foreground drain progress and the cumulative require outcome', async () => {
    const harness = orchestrationHarness();
    vi.spyOn(orderTaskSeeder, 'seedOrderSchedulerTasks').mockResolvedValue({ ...EMPTY_SEED_RESULT, inserted: 1 });
    ...
    expect(events.filter((event) => event.type === 'queue.drain.progress').map((event) => event.fields)).toEqual([
    ...
    expect(events).toContainEqual(expect.objectContaining({
      ...
      fields: expect.objectContaining({ action: 'fetched', documents: 35, requests: 2, durationMs: expect.any(Number) }),
    }));
  });
  ...
  it.each([
  ...
  ])('reports a released outcome when an order $label task is actively owned elsewhere', async ({ requirement, seed }) => {
    const harness = orchestrationHarness();
    seed().mockResolvedValue({ ...EMPTY_SEED_RESULT, skippedActive: 1 });
    const drain = vi.spyOn(schedulerDrain, 'runEngineSchedulerDrain').mockResolvedValue(EMPTY_DRAIN_RESULT);
    ...
    expect(harness.diagnostics.mock.calls.some(([event]) => event.type === 'coverage.gate.hit' || event.type === 'coverage.gate.miss')).toBe(false);
  });
  ...
  it.each([
    ...
    const harness = orchestrationHarness();
    if (kind === 'query') {
      vi.spyOn(orderTaskSeeder, 'seedOrderFilterSchedulerTask').mockResolvedValue({ ...EMPTY_SEED_RESULT, inserted: 1 });
    } else {
      vi.spyOn(orderTaskSeeder, 'seedOrderSchedulerTasks').mockResolvedValue({ ...EMPTY_SEED_RESULT, inserted: 1 });
    }
    ...
    const requirement = kind === 'query'
      ? { id: `lost-${lossCounter}`, collection: 'orders' as const, kind, queryKey: 'orders:browser:status=processing:search=:limit=50' }
  ...
  it.each(['query', 'refresh'] as const)('releases an order %s drain loss after partial progress with its aggregates', async (kind) => {
    const harness = orchestrationHarness();
    if (kind === 'query') {
      vi.spyOn(orderTaskSeeder, 'seedOrderFilterSchedulerTask').mockResolvedValue({ ...EMPTY_SEED_RESULT, inserted: 1 });
    } else {
    ...
    const requirement = kind === 'query'
      ? { id: 'partial-lost-query', collection: 'orders' as const, kind, queryKey: 'orders:browser:status=processing:search=:limit=50' }
  ...
  it.each(['query', 'refresh'] as const)('rejects a failed order %s drain', async (kind) => {
    ...
    vi.spyOn(schedulerDrain, 'runEngineSchedulerDrain').mockResolvedValue({ ...EMPTY_DRAIN_RESULT, failed: 1 });

    const requirement = kind === 'query'
      ? { id: 'failed-query', collection: 'orders' as const, kind, queryKey: 'orders:browser:status=processing:search=:limit=50' }
      : { id: 'failed-refresh', collection: 'orders' as const, kind };
    await expect(harness.plane.require(requirement).ready).rejects.toThrow(/scheduler drain failed 1 task/i);
    expect(harness.diagnostics.mock.calls.map(([event]) => event)).toContainEqual(expect.objectContaining({
  ...
  it('release aborts an in-flight foreground order drain and settles as released', async () => {
    ...
    const handle = harness.plane.require({ id: 'slow-refresh', collection: 'orders', kind: 'refresh' });
    await vi.waitFor(() => expect(observedSignal).toBeDefined());
    handle.release();

    await expect(handle.ready).resolves.toMatchObject({ action: 'released', reason: 'released during drain' });
  ...
  it('release aborts an in-flight targeted order drain and frees the pump for the next requirement', async () => {
    ...
    const slow = harness.plane.require({ id: 'slow-targeted', collection: 'orders', kind: 'targeted-records', wooIds: [8] });
    const next = harness.plane.require({ id: 'next-targeted', collection: 'orders', kind: 'targeted-records', wooIds: [9] });
    await vi.waitFor(() => expect(observedSignal).toBeDefined());
    slow.release();

    await expect(slow.ready).resolves.toMatchObject({ action: 'released', reason: 'released during drain' });
  ...
  it('rejects ready when the durable order drain fails and the order remains absent', async () => {
    vi.spyOn(orderTaskSeeder, 'seedTargetedOrderSchedulerTask').mockResolvedValue({ ...EMPTY_SEED_RESULT, inserted: 1 });
    vi.spyOn(schedulerDrain, 'runEngineSchedulerDrain').mockResolvedValue({ ...EMPTY_DRAIN_RESULT, scanned: 1, failed: 1 });
    const harness = orchestrationHarness();

    await expect(harness.plane.require({ id: 'failed', collection: 'orders', kind: 'targeted-records', wooIds: [8] }).ready)
      .rejects.toThrow(/scheduler drain failed/i);
  });

  it('waits for an identical active task to make the required order resident', async () => {
    vi.spyOn(orderTaskSeeder, 'seedTargetedOrderSchedulerTask').mockResolvedValue({ ...EMPTY_SEED_RESULT, skippedActive: 1 });
    vi.spyOn(schedulerDrain, 'runEngineSchedulerDrain').mockResolvedValue(EMPTY_DRAIN_RESULT);
    const harness = orchestrationHarness();
    const ready = harness.plane.require({ id: 'active', collection: 'orders', kind: 'targeted-records', wooIds: [8] }).ready;
  ...
  it('uses the injected clock for targeted order seeding and draining', async () => {
    const nowMs = 12_345;
    const seed = vi.spyOn(orderTaskSeeder, 'seedTargetedOrderSchedulerTask')
    ...
    await harness.plane.require({ id: 'injected-clock', collection: 'orders', kind: 'targeted-records', wooIds: [8] }).ready;
  ...
  it('times out when an identical active task never makes the required order resident', async () => {
    ...
    const drain = vi.spyOn(schedulerDrain, 'runEngineSchedulerDrain').mockResolvedValue(EMPTY_DRAIN_RESULT);
    const harness = orchestrationHarness();
    const ready = harness.plane.require({ id: 'active-timeout', collection: 'orders', kind: 'targeted-records', wooIds: [8] }).ready;
  ...
  it('backs off while waiting for an active targeted order task', async () => {
    vi.useFakeTimers();
    vi.setSystemTime(0);
    vi.spyOn(orderTaskSeeder, 'seedTargetedOrderSchedulerTask')
      .mockResolvedValue({ ...EMPTY_SEED_RESULT, skippedActive: 1 });
    const drain = vi.spyOn(schedulerDrain, 'runEngineSchedulerDrain').mockResolvedValue(EMPTY_DRAIN_RESULT);
    const harness = orchestrationHarness(() => Date.now());
    const ready = harness.plane.require({ id: 'active-backoff', collection: 'orders', kind: 'targeted-records', wooIds: [8] }).ready;
    const rejected = expect(ready).rejects.toThrow(/timed out waiting for an active order task/i);
  ...
  it('release interrupts a long active-task backoff and starts the next requirement without leaking its timer', async () => {
    ...
    const waiting = harness.plane.require({ id: 'active-long-backoff', collection: 'orders', kind: 'targeted-records', wooIds: [8] });
    const next = harness.plane.require({ id: 'after-release', collection: 'orders', kind: 'targeted-records', wooIds: [9] });
  ...
  it('release aborts a non-order pull promptly and frees the requirement pump', async () => {
    ...
    const engine = engineWith(async (url, init) => {
      ...
      return new Response(JSON.stringify([{
        ...
        meta_data: [{ key: '_woocommerce_pos_uuid', value: '22222222-2222-4222-8222-222222222222' }],
        date_modified_gmt: '2026-07-10T00:00:00',
        price: '1.00', stock_status: 'instock', type: 'simple', categories: [], brands: [],
        on_sale: false, featured: false, stock_quantity: null,
      }]), { status: 200, headers: { 'content-type': 'application/json' } });
    });
    await engine.ready;
    const slow = engine.require({ id: 'slow-product', collection: 'products', kind: 'targeted-records', wooIds: [1] });
    const next = engine.require({ id: 'next-product', collection: 'products', kind: 'targeted-records', wooIds: [2] });
