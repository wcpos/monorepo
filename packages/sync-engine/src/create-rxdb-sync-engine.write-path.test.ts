...
import { createFakeWriteServer } from '@woo-rxdb-lab/sync-core/testing';
import type { StoreScopeIdentity } from '@woo-rxdb-lab/sync-core';
import { createRxdbSyncEngine, type EngineEvent, type RxdbSyncEngine } from './create-rxdb-sync-engine';
...
import type { SyncEvent, SyncObserver } from '@woo-rxdb-lab/shared';
...
function engineWith(input: {
  ...
  return createRxdbSyncEngine({
    ...
    ...(input.writeDrainPollMs !== undefined ? { intervals: { writeDrainPollMs: input.writeDrainPollMs } } : {}),
...
async function orderJson(engine: RxdbSyncEngine, id: string): Promise<Record<string, unknown> | null> {
  const scope = engine.active();
  if (!scope) throw new Error('no active scope');
  const doc = await (scope.database.collections.orders as { findOne(id: string): { exec(): Promise<{ toJSON(): Record<string, unknown> } | null> } }).findOne(id).exec();
  return doc ? doc.toJSON() : null;
}

/** A SERVER-BORN resident order (uuid PK + known Woo id + anchored revision). */
async function insertServerBornOrder(engine: RxdbSyncEngine, id: string, over: { wooOrderId: number; revision: string; status?: string }): Promise<void> {
  const scope = engine.active();
  if (!scope) throw new Error('no active scope');
  await (scope.database.collections.orders as { insert(doc: unknown): Promise<unknown> }).insert({
    id,
    ...
    total: '10.00',
    customerId: 0,
    payload: { id: over.wooOrderId, status: over.status ?? 'processing', meta_data: [{ key: '_woocommerce_pos_uuid', value: id }] },
...
async function queueRows(engine: RxdbSyncEngine): Promise<Array<Record<string, unknown>>> {
  const scope = engine.active();
  if (!scope) throw new Error('no active scope');
  const docs = await (scope.database.collections.recordMutations as { find(): { exec(): Promise<Array<{ toJSON(): Record<string, unknown> }>> } }).find().exec();
  return docs.map((doc) => doc.toJSON());
}

/** Routes push envelopes to the fake write server and targeted /orders pulls to a scripted proxy. */
function routedFetch(server: ReturnType<typeof createFakeWriteServer>, orderTruth: () => Record<string, unknown>) {
...
describe('write() + sync("write-drain") through the public handle', () => {
  it('continues writing and draining after resetCollection mutations replaces the RxCollection', async () => {
    ...
    try {
      ...
      await engine.write({ collection: 'orders', operation: 'create', recordId: UUID_A, payload: { status: 'pos-open' } });
      expect((await engine.sync('write-drain')).pushed).toBe(1);
      expect(await engine.scope.resetCollection('mutations', { confirmDestroyQueue: true })).toBe('reset');
      await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { status: 'pos-paid' } });
      expect((await engine.sync('write-drain')).pushed).toBe(1);
  ...
  it('re-materializes a missing order from a queued create acknowledgement after orders reset', async () => {
    ...
    try {
      ...
      expect(events).toContainEqual(expect.objectContaining({ type: 'write-ack-rematerialized', recordId: UUID_A }));
    } finally { await engine.dispose(); }
  });
  ...
  it('includes write report counters in automatic lane telemetry', async () => {
    ...
    try {
      ...
      await vi.waitFor(() => expect(events.some((event) =>
        event.type === 'engine.lane.tick' && event.fields?.['lane'] === 'write-drain' && event.fields?.['status'] === 'ran',
      )).toBe(true));
      expect(events.find((event) =>
        event.type === 'engine.lane.tick' && event.fields?.['lane'] === 'write-drain' && event.fields?.['status'] === 'ran',
      )?.fields).toMatchObject({ pushed: 1, conflicts: 0, deferred: 0, failed: 0, rejected: 0 });
    } finally {
  ...
  it('enqueue-offline → reconnect → drain → acknowledged, revision re-anchored', async () => {
    ...
    const engine = engineWith({ fetch: (url, init) => server.fetch(url, init as never), connectivity: connectivity.signal });
    await engine.ready;
    const events: EngineEvent[] = [];
    engine.events((event) => events.push(event));
    await insertBornLocalOrder(engine, UUID_A);
    ...
    const receipt = await engine.write({ collection: 'orders', operation: 'create', recordId: UUID_A, payload: { status: 'pos-open', meta_data: [{ key: '_woocommerce_pos_uuid', value: UUID_A }] } });
    ...
    expect(ack).toMatchObject({ collection: 'orders', recordId: UUID_A, mutationId: receipt.mutationId });
  ...
  it('a conflict surfaces as an event and the mutation STAYS queued', async () => {
    const server = createFakeWriteServer();
    server.script(() => ({ kind: 'conflict' as const, current: null, currentRevision: 'sha256:server' }));
    const engine = engineWith({ fetch: (url, init) => server.fetch(url, init as never) });
    await engine.ready;
    const events: EngineEvent[] = [];
    engine.events((event) => events.push(event));
    await insertBornLocalOrder(engine, UUID_A);
    await engine.write({ collection: 'orders', operation: 'create', recordId: UUID_A, payload: { status: 'pos-open', meta_data: [{ key: '_woocommerce_pos_uuid', value: UUID_A }] } });
  ...
  it('the queue survives A→B→A: an offline write drains after the return', async () => {
    ...
    const engine = engineWith({ fetch: (url, init) => server.fetch(url, init as never), identity: a, connectivity: connectivity.signal });
    await engine.ready;
    await insertBornLocalOrder(engine, UUID_A);
    await engine.write({ collection: 'orders', operation: 'create', recordId: UUID_A, payload: { status: 'pos-open', meta_data: [{ key: '_woocommerce_pos_uuid', value: UUID_A }] } });
  ...
  it('transient failure backs off durably, then succeeds once the window elapses', async () => {
    ...
    expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', deferred: 1, pushed: 0 });
...
describe('#507 offline write flows through the public handle', () => {
  it('regression 1: two queued updates coalesce into ONE push that lands the SECOND snapshot through the real revision check', async () => {
    ...
    try {
      ...
      const first = await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { status: 'pos-open' } });
      const second = await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { status: 'completed' } });
      ...
      connectivity.set('online');
      expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', pushed: 1, conflicts: 0, rejected: 0 });
      // ONE envelope, the second snapshot, the ORIGINAL base — accepted by the server's real revision check.
      expect(server.received).toHaveLength(1);
      expect(server.received[0]).toMatchObject({ operation: 'update', mutationId: second.mutationId, baseRevision: 'sha256:base-r1' });
      ...
      const order = await orderJson(engine, UUID_A);
      expect((order?.sync as { revision?: string }).revision).toBe(server.applied.get(UUID_A)?.revision);
  ...
  it('regression 2: an edit queued behind an IN-FLIGHT ack drains with the re-anchored base — no 409', async () => {
    ...
    const gated = new Promise<void>((resolve) => { gateOpen = resolve; });
    let holds = 0;
    const fetch = async (url: string, init?: { signal?: AbortSignal }): Promise<Response> => {
    ...
    try {
      ...
      await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { status: 'completed' } });
      ...
      expect(server.received[1]).toMatchObject({ operation: 'update', baseRevision: revisionAfterCreate });
      expect(engine.status().queueDepth).toBe(0);
    } finally {
      await engine.dispose();
    }
  ...
  it("regression 3: a cross-client conflict parks durably — fetch count FROZEN, server truth in conflicts(), then retry-with-server-base wins", async () => {
    ...
    try {
      ...
      const receipt = await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { status: 'completed' } });

      expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', pushed: 0, conflicts: 1 });
      const fetchesAtConflict = server.received.length;
      // The conflicted mutation LEAVES the drain: further ticks push NOTHING.
      expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', pushed: 0, conflicts: 0, deferred: 0, failed: 0 });
      expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', pushed: 0, conflicts: 0, deferred: 0, failed: 0 });
      ...
      expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', pushed: 1, conflicts: 0 });
  ...
  it('regression 4: a rejected mutation frees the record — a pull applies server truth; the dead letter resolves by discard', async () => {
    ...
    try {
      ...
      expect((await orderJson(engine, UUID_A))?.local).toMatchObject({ dirty: false, pendingMutationIds: [] });
      ...
      await engine.require({ id: 'recover', collection: 'orders', kind: 'targeted-records', wooIds: [42], forceRefresh: true }).ready;
      ...
      await expect(engine.resolveConflict(receipt.mutationId, 'retry-with-server-base')).rejects.toThrow(/only be discarded/i);
      await engine.resolveConflict(receipt.mutationId, 'discard');
      expect(await engine.conflicts()).toEqual([]);
    } finally {
      await engine.dispose();
  ...
  it('regression 5: same-millisecond create+update under a fixed clock drains AS the create, carrying the latest snapshot', async () => {
    ...
    const engine = engineWith({ fetch: (url, init) => server.fetch(url, init as never), now: () => 1_752_105_600_000 });
    try {
      await engine.ready;
      await insertBornLocalOrder(engine, UUID_A);
      await engine.write({ collection: 'orders', operation: 'create', recordId: UUID_A, payload: { status: 'pos-open' } });
      const second = await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { status: 'pos-paid' } });
      ...
      expect(server.received).toHaveLength(1);
      expect(server.received[0]).toMatchObject({ operation: 'create', mutationId: second.mutationId });
  ...
  it('regression 6 (re-ruled by #516 item 3): pending-create + delete ANNIHILATES — terminal write-annihilated event, local row REMOVED, nothing pushed', async () => {
    ...
    const server = createFakeWriteServer();
    const engine = engineWith({ fetch: (url, init) => server.fetch(url, init as never), diagnostics: (event) => events.push(event) });
    try {
      await engine.ready;
      ...
      const receipt = await engine.write({ collection: 'orders', operation: 'delete', recordId: UUID_A });
      ...
      expect(engineEvents).toContainEqual({ type: 'write-annihilated', collection: 'orders', recordId: UUID_A, mutationId: receipt.mutationId });
  ...
  it('regression 6b: a pending update ∘ delete coalesces to ONE delete at the original position (server record — not annihilation)', async () => {
    ...
    try {
      await engine.ready;
      await insertServerBornOrder(engine, UUID_A, { wooOrderId: 42, revision: 'sha256:base-r1' });
      const update = await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { status: 'completed' } });
      const del = await engine.write({ collection: 'orders', operation: 'delete', recordId: UUID_A });
      expect(del.mutationId).not.toBe(update.mutationId);

      const rows = await queueRows(engine);
      expect(rows).toHaveLength(1);
      expect(rows[0]).toMatchObject({ operation: 'delete', mutationId: del.mutationId, baseRevision: 'sha256:base-r1' });
  ...
  it('regression 7: a failed dirty-mark rolls back the enqueue — write() rejects and the queue holds no orphan', async () => {
    ...
    const failRows = (rows: Array<{ document: Record<string, unknown> }>) => ({
      error: rows.map((row) => ({
        status: 422,
      ...
      get(target, prop, receiver) {
        if (prop === 'createStorageInstance') {
          return async (params: { collectionName: string }) => {
            const instance = await (inner as unknown as { createStorageInstance(p: unknown): Promise<object> }).createStorageInstance(params);
            ...
            return new Proxy(instance, {
              get(instanceTarget, instanceProp) {
                ...
                if (instanceProp === 'bulkWrite' && typeof value === 'function') {
                  return (...args: unknown[]) => (armed
                    ? Promise.resolve(failRows(args[0] as Array<{ document: Record<string, unknown> }>))
                    : (value as (...a: unknown[]) => unknown).apply(instanceTarget, args));
                }
                return typeof value === 'function' ? (value as (...a: unknown[]) => unknown).bind(instanceTarget) : value;
              },
            });
          };
        }
    ...
    try {
      ...
      armed = true;
      await expect(
        engine.write({ collection: 'orders', operation: 'create', recordId: UUID_A, payload: { status: 'pos-open' } }),
  ...
  it('P1-1a: two partial updates touching DIFFERENT fields coalesce into a snapshot carrying BOTH edits', async () => {
    ...
    try {
      ...
      await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { status: 'completed' } });
      await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { customer_note: 'ring twice' } });

      expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', pushed: 1, conflicts: 0, rejected: 0 });
      expect(server.received).toHaveLength(1);
      const pushedPayload = server.received[0].payload as { status?: string; customer_note?: string };
  ...
  it('P1-1b: create + partial update coalesces into a create retaining the original fields plus the edit', async () => {
    ...
    try {
      await engine.ready;
      await insertBornLocalOrder(engine, UUID_A);
      await engine.write({ collection: 'orders', operation: 'create', recordId: UUID_A, payload: { status: 'pos-open', currency: 'AUD' } });
  ...
  it('P1-2a: a delete does NOT annihilate a CLAIMED (in-flight) create — it queues behind and lands after the ack', async () => {
    ...
    try {
      ...
      try {
        ...
        expect(rows.map((row) => [row.operation, row.status])).toEqual([['create', 'claimed'], ['delete', 'pending']]);
  ...
  it('P1-2b: concurrent write() calls coalesce onto one pending row exactly once — one well-ordered row, every edit kept', async () => {
    ...
    try {
      ...
      await Promise.all([
        engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { customer_note: 'ring twice' } }),
        engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { transaction_id: 'txn-7' } }),
      ]);

      const rows = await queueRows(engine);
      expect(rows).toHaveLength(1); // exactly one row — no duplicated ordering metadata
      expect(rows[0]).toMatchObject({ seq: 1, coalesced: 2, status: 'pending', baseRevision: 'sha256:base-r1' });

      expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', pushed: 1, conflicts: 0, rejected: 0 });
      expect(server.received).toHaveLength(1);
      const pushedPayload = server.received[0].payload as { status?: string; customer_note?: string; transaction_id?: string };
      expect(pushedPayload).toMatchObject({ status: 'completed', customer_note: 'ring twice', transaction_id: 'txn-7' }); // every edit kept
    } finally {
      await engine.dispose();
    }
  });
...
describe('gate2 #516 — coalescing survives replay, reordering, and its own contracts', () => {
  it('#526: a lost create ack replays as created, acknowledges once, and does not born-twice requeue', async () => {
    ...
    const engine = engineWith({
      ...
      fetch: async (url, init) => {
        ...
        if (drop) { drop = false; throw new TypeError('ack lost'); }
        return response;
      },
    });
    try {
      ...
      engine.events((event) => engineEvents.push(event));
      await insertBornLocalOrder(engine, UUID_A);
      const create = await engine.write({ collection: 'orders', operation: 'create', recordId: UUID_A, payload: { status: 'pos-open' } });
      ...
      expect(engineEvents.filter((event) => event.type === 'write-acknowledged' && event.mutationId === create.mutationId)).toHaveLength(1);
    } finally { await engine.dispose(); }
  });
  ...
  it('#526: a lost born-twice create ack replays as 200 and requeues the discarded payload', async () => {
    ...
    try {
      await engine.ready;
      await insertBornLocalOrder(engine, UUID_A);
      await engine.write({ collection: 'orders', operation: 'create', recordId: UUID_A, payload: { status: 'pos-paid' } });
      expect((await engine.sync('write-drain')).failed).toBe(1);
      nowMs = 120_000;
      expect((await engine.sync('write-drain')).pushed).toBe(1);
      expect(events.some((event) => event.type === 'queue.write.born-twice-requeue')).toBe(true);
      expect(await queueRows(engine)).toEqual([expect.objectContaining({ operation: 'update', payload: expect.objectContaining({ status: 'pos-paid' }) })]);
    } finally { await engine.dispose(); }
  });
  ...
  it('gate2 R2 (item 1): an applied-but-unacked create never re-coalesces — the edit queues BEHIND, the replay dedupes, and the edit lands', async () => {
    ...
    try {
      ...
      const edit = await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { status: 'pos-paid' } });
      const rows = (await queueRows(engine)).sort((a, b) => (a.seq as number) - (b.seq as number));
      ...
      nowMs += 120_000;
      expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', pushed: 2, conflicts: 0, rejected: 0 });
  ...
  it('gate2 R2 delete (item 1): a delete behind an applied-but-unacked create queues BEHIND instead of annihilating maybe-applied work', async () => {
    ...
    let nowMs = 1_752_105_600_000;
    const engine = engineWith({ fetch, now: () => nowMs, diagnostics: (event) => events.push(event) });
    try {
      await engine.ready;
      await insertBornLocalOrder(engine, UUID_A);
      const create = await engine.write({ collection: 'orders', operation: 'create', recordId: UUID_A, payload: { status: 'pos-open' } });
  ...
  it('gate2 R2 honest ack (item 1): a born-twice create ack (HTTP 200) re-queues the discarded snapshot as a follow-up update', async () => {
    ...
    try {
      await engine.ready;
      await insertBornLocalOrder(engine, UUID_A);
      const create = await engine.write({ collection: 'orders', operation: 'create', recordId: UUID_A, payload: { status: 'pos-paid', currency: 'AUD' } });
      ...
      expect((rows[0]?.payload as { status?: string; currency?: string })).toMatchObject({ status: 'pos-paid', currency: 'AUD' });
  ...
  it('gate2 review P2 (item 1): CAS exhaustion under a hot same-record writer FAILS the ack — the create stays queued and a quiet drain lands both', async () => {
    ...
    let nowMs = 1_752_105_600_000;
    const engine = engineWith({ fetch: (url, init) => server.fetch(url, init as never), now: () => nowMs });
    try {
      await engine.ready;
      engine.events((event) => engineEvents.push(event));
      await insertBornLocalOrder(engine, UUID_A);
      ...
      vi.spyOn(queue, 'pending').mockImplementation(async () => {
        const rows = await realPending();
        if (hot) {
          const tail = rows.filter((row) => row.recordId === UUID_A && row.operation !== 'create' && (row.status === undefined || row.status === 'pending')).at(-1);
          if (tail) {
            await queue.coalesceInto(tail.mutationId, { ...tail, mutationId: `gate2-p2-hot-${++hotSeq}`, coalesced: (tail.coalesced ?? 0) + 1, payload: { note: 'hot' }, status: 'pending' });
          } else {
            await queue.enqueue({ mutationId: `gate2-p2-hot-${++hotSeq}`, collectionName: 'orders', operation: 'update', recordId: UUID_A, origin: 'existing', payload: { note: 'hot' }, baseRevision: null, queuedAt: '2026-07-10T00:00:01.000Z' });
          }
        }
        return rows;
      });
      ...
      hot = true;
      expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', pushed: 0, failed: 1 });
      hot = false;
      ...
      expect(stillQueuedCreate).toMatchObject({ mutationId: create.mutationId, status: 'pending', attempts: 1 }); // queued to replay
      ...
      // everything lands.
      nowMs += 5 * 60_000;
      expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', pushed: 1, failed: 0 }); // the create acks
      expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', pushed: 1, failed: 0 }); // the merged follow-up lands
      expect(await queueRows(engine)).toEqual([]);
      expect(server.received.at(-1)?.operation).toBe('update');
      expect(server.received.at(-1)?.payload).toMatchObject({ status: 'pos-paid', currency: 'AUD', note: 'hot' }); // both the snapshot and the hot edit landed
  ...
  it('gate2 R3 (item 2): a third edit coalesces into the LAST pending row — the server converges on the newest values', async () => {
    ...
    try {
      await engine.ready;
      await insertServerBornOrder(engine, UUID_A, { wooOrderId: 900_200_777, revision: 'rev-1', status: 'pos-open' });

      // U1 enqueued, then a drain claims it and the push hangs.
      await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { note: 'U1' } });
      failNext = true;
      ...
      await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { note: 'U2', discount: '5.00' } });
      gate!();
      ...
      await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { note: 'U3', discount: '9.99' } });
      const rows = (await queueRows(engine)).sort((a, b) => (a.seq as number) - (b.seq as number));
      expect(rows).toHaveLength(2);
      expect((rows[0]?.payload as { note?: string }).note).toBe('U1'); // the ever-pushed head is untouched
      expect((rows[1]?.payload as { note?: string; discount?: string })).toMatchObject({ note: 'U3', discount: '9.99' });

      // Drain past the backoff: pushes arrive U1-then-U3 — the user's LAST
      // choice (9.99) is the server's final state.
      nowMs += 120_000;
      expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', pushed: 2, conflicts: 0, rejected: 0 });
      const pushOrder = server.received.filter((env) => env.operation === 'update').map((env) => (env.payload as { note?: string })?.note);
  ...
  it('gate2 item 2: a delete annihilates a never-pushed create+update CHAIN (crash-restored rows) — no guaranteed-404 orphan', async () => {
    ...
    try {
      ...
      const queueCollection = scope.database.collections.recordMutations as { insert(doc: unknown): Promise<unknown> };
      await queueCollection.insert({ mutationId: 'gate2-mc', recordId: UUID_A, collectionName: 'orders', operation: 'create', origin: 'minted', payload: { status: 'pos-open' }, baseRevision: null, queuedAt: '2026-07-10T00:00:00.000Z', seq: 1, status: 'pending' });
      await queueCollection.insert({ mutationId: 'gate2-mu', recordId: UUID_A, collectionName: 'orders', operation: 'update', origin: 'existing', payload: { status: 'pos-paid' }, baseRevision: null, queuedAt: '2026-07-10T00:00:01.000Z', seq: 2, status: 'pending' });
      ...
      expect(await queueRows(engine)).toEqual([]);
      expect(events).toContainEqual(expect.objectContaining({ type: 'queue.write.annihilate', fields: expect.objectContaining({ removed: 2 }) }));
      expect(engineEvents).toContainEqual({ type: 'write-annihilated', collection: 'orders', recordId: UUID_A, mutationId: receipt.mutationId });
      expect(await orderJson(engine, UUID_A)).toBeNull();
      expect(await engine.sync('write-drain')).toMatchObject({ status: 'ran', pushed: 0 });
  ...
  it('#526: failed resident removal restores ahead of a concurrent same-record newcomer', async () => {
    ...
    try {
      ...
      await queue.enqueue({ mutationId: 'chain-update', recordId: UUID_A, collectionName: 'orders', operation: 'update', origin: 'existing', payload: { status: 'chain' }, baseRevision: null, queuedAt: '2026-07-11T00:00:00.000Z' });
      ...
      vi.spyOn(resident!, 'remove').mockImplementationOnce(async () => {
        await engine.write({ collection: 'orders', operation: 'update', recordId: UUID_A, payload: { status: 'newcomer' } });
        throw new Error('scripted resident remove failure');
      });
      await expect(engine.write({ collection: 'orders', operation: 'delete', recordId: UUID_A })).rejects.toThrow('scripted resident remove failure');
  ...
  it("gate2 item 4: an unrecoverable 428 parks as 'needs-revision'; retry-with-server-base REFRESHES first (no same-base loop)", async () => {
    ...
    let fault = true;
    server.script((env) => (env.operation === 'update' && fault ? { kind: 'precondition_required' } : undefined));
    const engine = engineWith({ fetch });
    try {
      await engine.ready;
      engine.events((event) => engineEvents.push(event));
      ...
      await expect(engine.resolveConflict(receipt.mutationId, 'retry-with-server-base')).rejects.toThrow(/no longer returns|stays parked/i);
          ...
          return async (params: { collectionName: string }) => {
            ...
            return new Proxy(instance, {
              get(instanceTarget, instanceProp) {
                ...
                if (instanceProp === 'bulkWrite' && typeof value === 'function') {
                  return (...args: unknown[]) => {
                    if (!armed) return (value as (...a: unknown[]) => unknown).apply(instanceTarget, args);
                    return (async () => {
                      const database = engineRef!.active()!.database;
                      const queue = queueFor(database as never);
                      const row = (await queue.pending()).find((item) => item.recordId === UUID_A && item.status === 'pending');
                      if (row) await queue.claim({ ...row, status: 'claimed' });
                      return failRows(args[0] as Array<{ document: Record<string, unknown> }>);
                    })();
                  };
                }
                return typeof value === 'function' ? (value as (...a: unknown[]) => unknown).bind(instanceTarget) : value;
  ...
  it('gate2 item 5: the discard re-pull is queued DURABLY before local state clears — a failed pull self-heals on a later scheduler drain', async () => {
    ...
    try {
      ...
      const scope = engine.active()!;
      const tasks = (await (scope.database.collections.schedulerTaskStates as { find(): { exec(): Promise<Array<{ toJSON(): Record<string, unknown> }>> } }).find().exec()).map((doc) => doc.toJSON());
      const repullTask = tasks.find((task) => task.collectionName === 'orders' && String(task.queryKey).includes(':ids:42'));
...
describe('require() through the public handle', () => {
  ...
  function productServer() {
    ...
    const fetch = async (url: string): Promise<Response> => {
      ...
      pulls.push(include);
      return new Response(JSON.stringify(include.map((id) => ({
        id,
  ...
  it('serve-local when every record is resident; fetched when not; preemption; release', async () => {
    ...
    const first = await engine.require({ id: 'r1', collection: 'products', kind: 'targeted-records', wooIds: [1] }).ready;
    expect(first).toMatchObject({ action: 'fetched', missingRecordIds: [1] });
    expect(server.pulls).toEqual([[1]]);
    const scope = engine.active();
    if (!scope) throw new Error('no active scope');
    expect((await scope.database.collections.existenceManifest.findOne('1').exec())?.toJSON())
      .toMatchObject({ wooId: 1, objectType: 'product', digest: 'product-digest-1' });

    // Resident → serve-local, NO fetch.
    const again = await engine.require({ id: 'r2', collection: 'products', kind: 'targeted-records', wooIds: [1] }).ready;
    ...
    const low = engine.require({ id: 'low', collection: 'products', kind: 'targeted-records', wooIds: [10], priority: 100 });
    const mid = engine.require({ id: 'mid', collection: 'products', kind: 'targeted-records', wooIds: [20], priority: 500 });
    const high = engine.require({ id: 'high', collection: 'products', kind: 'targeted-records', wooIds: [30], priority: 900 });
    ...
    const busy = engine.require({ id: 'busy', collection: 'products', kind: 'targeted-records', wooIds: [40], priority: 100 });
    const released = engine.require({ id: 'released', collection: 'products', kind: 'targeted-records', wooIds: [50], priority: 50 });
    released.release();
  ...
  it("require('targeted-records') without ids, or on the wrong shape, is caller misuse", async () => {
    ...
    const engine = engineWith({ fetch: server.fetch });
    await engine.ready;
    await expect(engine.require({ id: 'x', collection: 'products', kind: 'targeted-records' }).ready).rejects.toThrow(/needs wooIds/i);
    await expect(engine.require({ id: 'y', collection: 'categories', kind: 'targeted-records', wooIds: [1] }).ready).rejects.toThrow(/targeted collection/i);
    await engine.dispose();
  });
});
...
describe('telemetry redaction corpus', () => {
  it('keeps seed, drain, require, conflict, and reset event fields metadata-only', async () => {
    ...
    await engine.write({ collection: 'orders', operation: 'create', recordId: UUID_A, payload: { status: 'pos-open' } });
    await engine.sync('write-drain');
    await engine.require({ id: 'resident', collection: 'orders', kind: 'targeted-records', wooIds: [] }).ready.catch(() => undefined);
    ...
    const inspect = (value: unknown, path: string): void => {
      ...
      if (typeof value === 'object') {
        for (const [key, nested] of Object.entries(value as Record<string, unknown>)) {
          ...
          if (nested !== null && typeof nested === 'object') offenders.push(`${path}.${key}:nested-object`);
