...
import { MUTATION_QUEUE_RXDB_COLLECTION, type SyncCollectionName } from '../collections/engine-collections';

export type WriteIntent =
  | { collection: SyncCollectionName; operation: 'create'; payload: Record<string, unknown>; recordId: string }
  | { collection: SyncCollectionName; operation: 'update'; recordId: string; payload: Record<string, unknown>; baseRevision?: string }
  | { collection: SyncCollectionName; operation: 'delete'; recordId: string; baseRevision?: string };
...
export function queueFor(db: RxDatabase): RecordMutationQueue {
  ...
  const queue = new RecordMutationQueue(
    new RxRecordMutationStorage(() => collectionOf(db, MUTATION_QUEUE_RXDB_COLLECTION) as unknown as RxRecordMutationCollection),
  );
  queues.set(db, queue);
  return queue;
}
...
export async function enqueueWriteIntent(input: {
  ...
  for (let attempt = 0; attempt < 10; attempt += 1) {
    ...
    const recordRows = rows.filter((item) => item.collectionName === intent.collection && item.recordId === intent.recordId);
    const pendingRows = recordRows.filter((item) => item.status === undefined || item.status === 'pending');
    ...
    const prior = lastPending !== undefined && (lastPending.attempts ?? 0) === 0 ? lastPending : undefined;
    ...
    const createAhead = recordRows.find(
      (item) => item.operation === 'create' && (item.status === undefined || item.status === 'pending' || item.status === 'claimed'),
    );
    ...
    if (
      ...
      input.observe?.({ type: 'queue.write.annihilate', level: 'info', collection: intent.collection, fields: { recordId: intent.recordId, removed: removed.length } });
    ...
    const stored = doc ? (doc.toJSON() as { sync?: { revision?: string }; payload?: Record<string, unknown> }) : undefined;
    ...
    if (intent.operation === 'create') {
      ...
      if (!doc) {
        throw new Error(`write(create): record "${intent.recordId}" is not resident in "${intent.collection}" — insert the born-local row first`);
      }
      mutation = buildCreateMutation(
        { collectionName: intent.collection, payload: intent.payload as never, currentId: intent.recordId },
        deps,
      );
    } else {
      if (!doc) {
        throw new Error(`write(${intent.operation}): record "${intent.recordId}" is not resident in "${intent.collection}"`);
      }
      ...
      if (intent.operation === 'update') {
        ...
        if (!baseRevision && !createAhead) {
          throw new Error('write(delete): a baseRevision is required — the server 428s an unconditional delete');
        }
        mutation = baseRevision
          ? buildDeleteMutation({ collectionName: intent.collection, recordId: intent.recordId, baseRevision }, deps)
          : {
    ...
    if (prior) {
      ...
      const payload =
        operation === 'delete'
          ? mutation.payload
          : { ...(stored?.payload ?? {}), ...(prior.operation === 'delete' ? {} : prior.payload), ...mutation.payload };
      const replacement = {
        ...mutation,
        operation,
        payload,
      ...
      input.observe?.({ type: 'queue.write.coalesce', level: 'info', collection: mutation.collectionName, fields: { recordId: mutation.recordId, removed: 1, added: 1 } });
    } else {
    ...
    if (doc) {
      try {
        await doc.incrementalModify((data) => {
          ...
          const pending = (Array.isArray(local.pendingMutationIds) ? local.pendingMutationIds : []).filter((id) => id !== prior?.mutationId && !supersededIds.has(id));
          return { ...data, local: { ...local, dirty: true, pendingMutationIds: [...pending, mutation.mutationId] } };
        });
      } catch (error) {
  ...
  throw new Error(`write(${intent.operation}): the mutation queue kept changing under "${intent.recordId}" — retry the intent`);
}
...
export async function requeueBornTwiceSnapshot(input: {
  ...
  const markBookkeeping = async (addId: string, dropId: string | null): Promise<void> => {
    const doc = (await collectionOf(input.db, mutation.collectionName).findOne(mutation.recordId).exec()) as MutationDoc | null;
    if (!doc) return;
    await doc.incrementalModify((data) => {
      const local = (data.local ?? {}) as { dirty?: boolean; pendingMutationIds?: string[] };
      const pending = (Array.isArray(local.pendingMutationIds) ? local.pendingMutationIds : []).filter((id) => id !== dropId);
      return { ...data, local: { ...local, dirty: true, pendingMutationIds: [...pending, addId] } };
    });
  };
  for (let attempt = 0; attempt < 10; attempt += 1) {
    const rows = (await queue.pending()).filter(
      (item) => item.collectionName === mutation.collectionName && item.recordId === mutation.recordId && item.mutationId !== mutation.mutationId,
    );
    ...
    if (last && (last.status === undefined || last.status === 'pending') && (last.attempts ?? 0) === 0) {
      ...
      input.observe?.({ type: 'queue.write.born-twice-requeue', level: 'warn', collection: mutation.collectionName, fields: { recordId: mutation.recordId, mutationId: mutation.mutationId, followUpMutationId: followUpId } });
      return { mutationId: followUpId };
    }
    const payload = rows.reduce<Record<string, unknown>>((acc, item) => ({ ...acc, ...item.payload }), { ...mutation.payload });
    ...
    const observedIds = rows.map((item) => item.mutationId).sort().join('|');
    const queued = await queue.enqueueWhen(
      ...
      (fresh) => fresh
        .filter((item) => item.collectionName === mutation.collectionName && item.recordId === mutation.recordId && item.mutationId !== mutation.mutationId && item.status !== 'rejected')
        .map((item) => item.mutationId).sort().join('|') === observedIds,
    );
  ...
  throw new Error(`born-twice requeue: the mutation queue kept changing under "${mutation.recordId}" — create ack refused; the replay retries`);
}
