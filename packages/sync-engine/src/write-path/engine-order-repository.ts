...
import { normalizeCheckpoint, withOrderColumns, type OrderDocument, type SyncCheckpoint } from '@woo-rxdb-lab/shared';
import { manifestRowOf, materializeExistingLocalOnly } from '../materialization/record-materialization';
import { removeManifestByWooIds, upsertManifestRows, type ManifestCollection } from '../local-coverage/rx-existence-manifest-repository';
...
export class EngineOrderRepository {
  ...
  async upsertMany(documents: OrderDocument[]): Promise<void> {
    ...
    const materialized = applicable.map((document) => manifestRowOf(document)
      ? { storedDocument: document, manifestRow: manifestRowOf(document) }
      : materializeExistingLocalOnly(document));
    const manifestRows = materialized.flatMap(({ manifestRow }) => manifestRow ? [manifestRow] : []);
    ...
    assertBulkSuccess(await this.db.orders.bulkUpsert(materialized.map(({ storedDocument }) => withOrderColumns(storedDocument))), 'engine-order-repository upsert');
  ...
  async removeDeletedOrders(
    ...
    if (storageIds.length > 0) assertBulkSuccess(await this.db.orders.bulkRemove(storageIds), 'engine-order-repository remove');
  ...
  async resetForResync(pendingMutationOrderIds?: ReadonlySet<string | number>): Promise<void> {
    ...
    if (removable.length > 0) assertBulkSuccess(await this.db.orders.bulkRemove(removable.map((doc) => doc.id)), 'engine-order-repository remove');
  }
   ...
   */
  private async unprotectedOrders(pendingMutationOrderIds?: ReadonlySet<string | number>): Promise<OrderDocument[]> {
    const docs = (await this.db.orders.find().exec()).map((doc) => doc.toJSON() as unknown as OrderDocument);
    return docs.filter((doc) => {
      const pendingIds = Array.isArray(doc.local?.pendingMutationIds) ? doc.local.pendingMutationIds : [];
  ...
  async firstPageForDisplay(limit = 100): Promise<OrderDocument[]> {
    const documents = await this.db.orders.find({
      selector: {},
      sort: [{ id: 'asc' }],
      limit,
    }).exec();
