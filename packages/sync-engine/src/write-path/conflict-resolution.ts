 ...
 */
/**
 * How long a durable resolution claim is honoured before another process may
 * steal it. Long enough to cover a real resolution (a discard can fetch a server
 * document and seed a scheduler task), short enough that a window which crashes
 * mid-resolution does not stall recovery for a shift.
 */
...
export function createConflictResolution(deps: ConflictResolutionDeps) {
	// This engine instance's identity for the durable resolution claim. One per
	// instance, minted LAZILY on first use: a read-only engine may open on a host
	// with no Web Crypto, where `mintUuid` throws, and an engine that never
	// resolves a conflict must not pay for an id it will never need.
	let instanceIdOnce: string | null = null;
	const instanceIdFor = (): string => (instanceIdOnce ??= deps.mintUuid());
	/** Injected clock, so the claim deadline is deterministic under test. */
	const nowMs = (): number => (deps.ports.now !== undefined ? deps.ports.now() : Date.now());
	...
	return {
		...
		resolveConflict: async (mutationId: string, resolution: ConflictResolutionChoice) => {
			...
			return serialize(async () => {
				...
				let claimed = false;
				let claimQueue: ReturnType<typeof queueFor> | null = null;
				...
				try {
					await manager.runGuarded(async (bound) => {
						...
						// DURABLE CROSS-PROCESS CLAIM (#832 follow-up). The in-memory chain
						// above orders this instance's resolutions; two Electron windows are
						// two processes over one storage and neither chain sees the other. The
						// row is the shared state, so the lock goes there — taken here, before
						// anything is read for the decision, and released in the `finally`
					...
					// that SUCCEEDED has removed the row, so this is a no-op (and must be:
					// re-appending would resurrect the dead letter it just retired). One
					// that failed hands the row straight back instead of parking it behind
					// a 30s deadline the cashier would have to wait out.
					if (claimed && claimQueue) {
						await (claimQueue as ReturnType<typeof queueFor>)
