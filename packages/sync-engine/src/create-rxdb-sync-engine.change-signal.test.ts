...
import { scopeKeyFor, type StoreScopeIdentity } from '@woo-rxdb-lab/sync-core';
...
type SequenceRow = { sequence: number; id: number; type: string; collection: string; modified_gmt: string };
...
function scriptedServer() {
  const state = {
    ...
    productPulls: 0,
    headFetches: 0,
    products: new Map<number, Record<string, unknown>>([[9, {
      id: 9,
  ...
  const fetch = async (url: string): Promise<Response> => {
    ...
    if (path.endsWith('/changes/sequence-log')) {
      ...
      return json({ changes: rows, checkpoint: { since: Math.max(maxSeen, since), head: state.head }, complete: true });
    }
    if (path.endsWith('/integrity/scan')) {
      return json({ changes: [], checkpoint: { after_id: Number(u.searchParams.get('after_id') ?? '0') }, complete: true });
    }
...
async function productCount(engine: RxdbSyncEngine): Promise<number> {
  ...
  return (scope.database.collections.products as { count(): { exec(): Promise<number> } }).count().exec();
}
...
describe('sync("change-signal") through the public handle', () => {
  it('primes to head, applies a fresh signal, persists the cursor across engines', async () => {
    ...
    server.state.head = 6;
    server.state.rows.push({ sequence: 6, id: 9, type: 'update', collection: 'products', modified_gmt: '2026-07-10T00:00:01' });
  ...
  it('adopts the server _rxdb_revision stamp as sync.revision and strips it from the stored payload (#423 step 1b)', async () => {
    ...
    const engine = engineWith({ storage: memoryEngineStorage(), fetch: server.fetch, identity: freshIdentity() });
    await engine.ready;
    ...
    const scope = engine.active();
    if (!scope) throw new Error('no active scope');
    const docs = await (scope.database.collections.products as { find(): { exec(): Promise<Array<{ toJSON(): Record<string, unknown> }>> } }).find().exec();
    expect(docs).toHaveLength(1);
    const json = docs[0]!.toJSON() as { sync: { revision: string }; payload: Record<string, unknown> };
  ...
  it('does not advance the cursor when bulkUpsert resolves with a per-row failure', async () => {
    ...
    products.bulkUpsert = (async () => ({ success: [], error: [{ documentId: UUID_9, status: 409 }] })) as unknown as typeof products.bulkUpsert;
  ...
  it('offline connectivity skips the tick before any request', async () => {
    ...
    const engine = engineWith({ storage: memoryEngineStorage(), fetch: server.fetch, identity: freshIdentity(), connectivity: connectivity.signal });
    await engine.ready;
