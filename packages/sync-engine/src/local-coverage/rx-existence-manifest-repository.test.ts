...
import { existenceManifestSchema, existenceManifestDocument } from '@woo-rxdb-lab/sync-engine-rxdb/testing';
import { upsertManifestRows, readManifestRange, removeManifestByWooIds } from './rx-existence-manifest-repository';
...
async function manifestCollection() {
  dbCounter += 1;
  const db = await createRxDatabase({ name: `manifesttest${dbCounter}`, storage: getRxStorageMemory() });
...
describe('existenceManifestDocument', () => {
  it('keys by String(wooId) and carries the numeric wooId + string digest', () => {
    expect(existenceManifestDocument({ wooId: 42, objectType: 'product', digest: '9223372036854775810' })).toEqual({
      id: '42',
      wooId: 42,
      objectType: 'product',
...
describe('existence manifest repository', () => {
  it('range-reads by wooId (the bucketing query) and upserts idempotently', async () => {
    ...
    await upsertManifestRows(c, [existenceManifestDocument({ wooId: 5, objectType: 'product', digest: 'a2' })]);
