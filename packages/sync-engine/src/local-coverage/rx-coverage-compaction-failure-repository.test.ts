...
function createCollection(initial?: FailureDocument) {
  ...
  return { collection: { bulkUpsert, findOne }, get stored() { return stored; }, bulkUpsert, findOne };
}

function repositoryFor(initial?: FailureDocument) {
  const fixture = createCollection(initial);
  const repository = new RxCoverageCompactionFailureRepository({ coverageCompactionFailures: fixture.collection } as never);
...
describe('RxCoverageCompactionFailureRepository', () => {
  it('reads no failure when the singleton state is absent or cleared', async () => {
    ...
    const cleared = repositoryFor({ stateKey: 'coverage-compaction', failedAtMs: null, retryAfterMs: null });
  ...
  it('records the singleton failure backoff state', async () => {
    ...
    expect(fixture.bulkUpsert).toHaveBeenCalledWith([{ stateKey: 'coverage-compaction', ...failure, schemaVersion: 1 }]);
  ...
  it('clears the singleton failure state without deleting the document', async () => {
    const { repository, fixture } = repositoryFor({ stateKey: 'coverage-compaction', failedAtMs: 1_500, retryAfterMs: 301_500 });

    await repository.clearFailure();

    expect(fixture.stored).toEqual({ stateKey: 'coverage-compaction', failedAtMs: null, retryAfterMs: null, schemaVersion: 1 });
    await expect(repository.readFailure()).resolves.toBeNull();
  });
});
