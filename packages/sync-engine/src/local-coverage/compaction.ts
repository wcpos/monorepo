import { planCoverageCompactionCadence, type CoverageCompactionCadenceDecision, type CoverageCompactionFailure, type CoverageCompactionLease } from '../scheduler/coverage-compaction-cadence';
import type { PersistedCoverageCompactionResult, PersistedCoverageDocumentSet } from '../scheduler/persisted-coverage-schema';

export type CoverageCompactionMaintenanceRepository = {
  readCoverageDocuments(): Promise<PersistedCoverageDocumentSet>;
  compactRetention(input: { nowMs: number; retainStaleForMs: number }): Promise<PersistedCoverageCompactionResult>;
};

export type CoverageCompactionLeaseStore = {
  readLease(): Promise<CoverageCompactionLease | null>;
  claimLease(lease: CoverageCompactionLease, expectedLease: CoverageCompactionLease | null): Promise<CoverageCompactionLease | null>;
...
export async function runCoverageCompactionMaintenance(input: CoverageCompactionMaintenanceInput): Promise<CoverageCompactionMaintenanceResult> {
  ...
  if (decision.action === 'skip' || decision.action === 'wait-for-owner' || decision.action === 'wait-for-failure-backoff') {
  ...
  try {
    ...
    try {
      ...
      await input.failureStore.recordFailure({ failedAtMs, retryAfterMs: failedAtMs + input.failureBackoffMs });
      throw error;
    }
