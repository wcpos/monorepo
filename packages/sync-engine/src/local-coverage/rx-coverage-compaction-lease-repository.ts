...
import { COVERAGE_COMPACTION_LEASE_KEY, type CoverageCompactionLeaseDocument } from './coverage-compaction-lease-schema';
...
type RxCoverageCompactionLeaseCollection<T> = {
  ...
  findOne(documentId: string): { exec(includeDeleted?: boolean): Promise<RxCoverageCompactionLeaseDocument<T> | null> };
};
...
export type CoverageCompactionLeaseDatabase = { coverageCompactionLeases: RxCoverageCompactionLeaseCollection<CoverageCompactionLeaseDocument> };
...
export class RxCoverageCompactionLeaseRepository {
  ...
  async readLease(): Promise<CoverageCompactionLease | null> {
    const document = await this.coverageCompactionLeases.findOne(COVERAGE_COMPACTION_LEASE_KEY).exec();
    return document ? toLease(toJson<CoverageCompactionLeaseDocument>(document)) : null;
  }

  async claimLease(lease: CoverageCompactionLease, expectedLease: CoverageCompactionLease | null): Promise<CoverageCompactionLease | null> {
    ...
    if (expectedLease === null) {
      try {
        ...
        const conflictDocument = await this.coverageCompactionLeases.findOne(COVERAGE_COMPACTION_LEASE_KEY).exec(true);
