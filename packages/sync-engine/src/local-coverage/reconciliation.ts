...
import { runExistenceReconcile, type ReconcileSummary } from './reconcile-existence-pass';
...
export async function resolveDirtyWooIds(
  mutations: ReadonlyArray<{ recordId: string; collectionName: string }>,
...
export async function reconcileExistence(deps: {
  ...
  const emptySummary: ReconcileSummary = { buckets: 0, pruned: 0, pulled: 0, repulled: 0, skippedDirty: 0 };
