...
import { runCoverageCompactionMaintenance, type CoverageCompactionFailureStore, type CoverageCompactionLeaseStore, type CoverageCompactionMaintenanceRepository } from './compaction';
import type { CoverageCompactionFailure, CoverageCompactionLease } from '../scheduler/coverage-compaction-cadence';
import type { PersistedCoverageDocumentSet } from '../scheduler/persisted-coverage-schema';

const freshLane = { collection: 'orders', queryKey: 'orders:open', complete: true, expectedRecordIds: ['order-1'], freshUntilMs: 2_000, updatedAtMs: 1_000 };
const expiredRecord = { collection: 'orders', id: 'order-expired', coveredQueryKeys: ['orders:open'], freshUntilMs: 700, updatedAtMs: 600 };
const refreshableRecord = { collection: 'orders', id: 'order-refresh', coveredQueryKeys: ['orders:open'], freshUntilMs: 950, updatedAtMs: 900 };
...
function createRepository(documents: PersistedCoverageDocumentSet): CoverageCompactionMaintenanceRepository {
  return {
    readCoverageDocuments: vi.fn(async () => documents),
    compactRetention: vi.fn(async () => ({
      decisions: [],
      documents: { records: [], lanes: [freshLane] },
      removed: [{ documentType: 'record' as const, collection: 'orders', key: 'order-expired', action: 'remove' as const, reason: 'coverage is stale beyond retention window' }],
    })),
  };
}

function createLeaseStore(lease: CoverageCompactionLease | null = null): CoverageCompactionLeaseStore {
  return {
...
function createFailureStore(failure: CoverageCompactionFailure | null = null): CoverageCompactionFailureStore {
  return {
...
describe('runCoverageCompactionMaintenance', () => {
  it('skips without claiming or compacting when stale coverage remains refreshable', async () => {
    ...
    const result = await runCoverageCompactionMaintenance({ ...baseInput, repository, leaseStore, failureStore });
  ...
  it('waits without claiming or compacting when another tab owns an active lease', async () => {
    ...
    const leaseStore = createLeaseStore({ ownerId: 'tab-b', acquiredAtMs: 900, expiresAtMs: 1_200 });
    const failureStore = createFailureStore();
  ...
  it('claims, compacts, releases, and reports removals when compaction can run', async () => {
    ...
    expect(leaseStore.claimLease).toHaveBeenCalledWith({ ownerId: 'tab-a', acquiredAtMs: 1_000, expiresAtMs: 1_250 }, null);
    expect(repository.compactRetention).toHaveBeenCalledWith({ nowMs: 1_000, retainStaleForMs: 100 });
  ...
  it('does not record compaction failure backoff when clearing stale failure state throws after compaction succeeds', async () => {
    ...
    await expect(runCoverageCompactionMaintenance({ ...baseInput, repository, leaseStore, failureStore })).rejects.toThrow('clear failed');
  ...
  it('releases the owned lease when compaction throws', async () => {
    ...
    await expect(runCoverageCompactionMaintenance({ ...baseInput, repository, leaseStore, failureStore })).rejects.toThrow('boom');

    expect(leaseStore.claimLease).toHaveBeenCalledWith({ ownerId: 'tab-a', acquiredAtMs: 1_000, expiresAtMs: 1_250 }, null);
    expect(failureStore.recordFailure).toHaveBeenCalledWith({ failedAtMs: 1_000, retryAfterMs: 1_300 });
    expect(leaseStore.releaseLease).toHaveBeenCalledWith('tab-a');
  });
  ...
  it('takes over an expired lease before compacting', async () => {
    ...
    expect(leaseStore.claimLease).toHaveBeenCalledWith({ ownerId: 'tab-a', acquiredAtMs: 1_000, expiresAtMs: 1_250 }, { ownerId: 'tab-b', acquiredAtMs: 400, expiresAtMs: 900 });
    expect(repository.compactRetention).toHaveBeenCalledOnce();
  });
});
