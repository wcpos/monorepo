...
type LeaseDocument = CoverageCompactionLease & { leaseKey: string; schemaVersion?: 1; _deleted?: boolean };
...
function createCollection(initial?: LeaseDocument) {
  ...
  const findOne = vi.fn((_documentId: string) => ({
    exec: vi.fn(async (includeDeleted = false) => {
      ...
      return {
        ...
        incrementalModify: vi.fn(async (mutator: (current: LeaseDocument & { _deleted?: boolean }) => LeaseDocument & { _deleted?: boolean }) => {
  ...
  return { collection: { insert, findOne }, get stored() { return stored; }, insert, findOne };
}

function repositoryFor(initial?: LeaseDocument) {
  const fixture = createCollection(initial);
  const repository = new RxCoverageCompactionLeaseRepository({ coverageCompactionLeases: fixture.collection } as never);
...
describe('RxCoverageCompactionLeaseRepository', () => {
  ...
  it('can claim a deleted singleton lease tombstone left by the previous release behavior', async () => {
    ...
    const { repository, fixture } = repositoryFor({ leaseKey: 'coverage-compaction', ...previous, _deleted: true });

    await expect(repository.readLease()).resolves.toBeNull();
    await expect(repository.claimLease(next, null)).resolves.toEqual(next);

    expect(fixture.stored).toEqual({ leaseKey: 'coverage-compaction', ...next, schemaVersion: 1, _deleted: false });
