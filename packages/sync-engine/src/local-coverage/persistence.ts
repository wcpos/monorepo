...
import { buildCoverageDocumentsFromQueryResult, type BuildCoverageDocumentsFromQueryResultInput, type BuildCumulativeCoverageDocumentsFromQueryResultInput, type QueryCoverageResultRecord } from '../scheduler/query-coverage-writes';
import {
...
export type RxCoverageDocument<T> = {
  ...
  incrementalModify(mutationFunction: (document: T & { _deleted?: boolean }) => T | (T & { _deleted?: boolean }) | Promise<T | (T & { _deleted?: boolean })>): Promise<unknown>;
};
...
type RxCoverageCollection<T> = {
  ...
  find(query?: unknown): { exec(): Promise<Array<RxCoverageDocument<T> | T>> };
...
function toJson<T>(document: RxCoverageDocument<T> | T): T {
  return document && typeof document === 'object' && 'toJSON' in document ? document.toJSON() as T : document;
}
...
function fromRecordDocument(document: CoverageRecordDocument | (PersistedCoverageRecord & { coverageKey: string; schemaVersion?: number })): PersistedCoverageRecord {
...
function fromLaneDocument(document: CoverageLaneDocument | (PersistedCoverageLane & { laneKey: string; schemaVersion?: number })): PersistedCoverageLane {
...
function sameCoverageRecord(left: PersistedCoverageRecord, right: PersistedCoverageRecord): boolean {
...
function retentionDecisionKey(documentType: PersistedCoverageRetentionDecision['documentType'], collection: string, key: string): string {
  return `${documentType}::${collection}::${key}`;
}

function isRxConflict(error: unknown): boolean {
  return Boolean(error && typeof error === 'object' && 'code' in error && error.code === 'CONFLICT');
}
...
function localLaneCoverage(lane: PersistedCoverageLane, nowMs: number): LocalLaneCoverageWithExpectedRecords {
  return {
...
function mergeRecordWithCurrentRevision(current: CoverageRecordDocument, next: PersistedCoverageRecord): CoverageRecordDocument {
...
function mergeLaneWithCurrentRevision(current: CoverageLaneDocument, next: PersistedCoverageLane): CoverageLaneDocument {
  const merged = mergePersistedCoverageWrites([
...
export class RxCoverageRepository {
  ...
  async upsertCoverageDocuments(documents: PersistedCoverageDocumentSet): Promise<void> {
    if (documents.records.length > 0) {
      assertBulkSuccess(await this.coverageRecords.bulkUpsert(documents.records.map(toRecordDocument)), 'persistence upsert');
    }
    if (documents.lanes.length > 0) {
      assertBulkSuccess(await this.coverageLanes.bulkUpsert(documents.lanes.map(toLaneDocument)), 'persistence upsert');
    }
  }
  ...
  async recordCumulativeQueryResult(input: BuildCumulativeCoverageDocumentsFromQueryResultInput): Promise<void> {
    const freshUntilMs = input.nowMs + input.freshForMs;
    const existingLane = input.resetCumulativeExpectedIds ? null : await this.readCoverageLane(input.collection, input.queryKey);
    const expectedRecordIds = [
      ...(existingLane?.expectedRecordIds ?? []),
      ...input.records.map((record) => record.id),
    ].filter((id, index, ids) => ids.indexOf(id) === index);

    const recordIdsToRefresh = input.complete ? expectedRecordIds : input.records.map((record) => record.id);

    const lanes = !input.complete && existingLane?.complete
      ? [{
        ...existingLane,
        expectedRecordIds,
        updatedAtMs: input.nowMs,
  ...
  async readCoverageDocuments(): Promise<PersistedCoverageDocumentSet> {
    const [records, lanes] = await Promise.all([
      this.coverageRecords.find({ selector: {}, sort: [{ collectionName: 'asc' }, { id: 'asc' }] }).exec(),
      this.coverageLanes.find({ selector: {}, sort: [{ collectionName: 'asc' }, { queryKey: 'asc' }] }).exec(),
    ]);
  ...
  async readLocalRecordCoverage(collection: string, id: string, nowMs: number): Promise<LocalRecordCoverage | null> {
  ...
  async readLocalRecordCoverages(collection: string, ids: string[], nowMs: number): Promise<LocalRecordCoverage[]> {
    const records = await Promise.all(ids.map((id) => this.readLocalRecordCoverage(collection, id, nowMs)));
    return records.filter((record): record is LocalRecordCoverage => record !== null);
  }

  async readLocalLaneCoverage(collection: string, queryKey: string, nowMs: number): Promise<LocalLaneCoverageWithExpectedRecords | null> {
    const lane = await this.readCoverageLane(collection, queryKey);
    return lane ? localLaneCoverage(lane, nowMs) : null;
  }

  private async readCoverageLane(collection: string, queryKey: string): Promise<PersistedCoverageLane | null> {
  ...
  async expectedRecordIdsForLane(collection: string, queryKey: string, nowMs: number): Promise<string[]> {
    return expectedRecordIdsForLane({ documents: await this.readCoverageDocuments(), collection, queryKey, nowMs });
  }

  async compactRetention(input: Omit<PlanPersistedCoverageRetentionInput, 'documents'>): Promise<PersistedCoverageCompactionResult> {
    const existingDocuments = await this.readCoverageDocuments();
    const plannedResult = compactPersistedCoverageDocuments({ ...input, documents: existingDocuments });
    const removalsByKey = new Map(plannedResult.removed.map((decision) => [
    ...
    for (const record of existingDocuments.records) {
      const removalDecision = removalsByKey.get(retentionDecisionKey('record', record.collection, record.id));
    ...
    for (const lane of existingDocuments.lanes) {
      const removalDecision = removalsByKey.get(retentionDecisionKey('lane', lane.collection, lane.queryKey));
  ...
  private async writeCoverageDocumentsWithMerge(documents: PersistedCoverageDocumentSet): Promise<void> {
  ...
  private async mergeExistingRecord(document: RxCoverageDocument<CoverageRecordDocument>, record: PersistedCoverageRecord): Promise<void> {
    await document.incrementalModify((currentDocument) => mergeRecordWithCurrentRevision(currentDocument, record));
  }
  ...
  private async mergeExistingLane(document: RxCoverageDocument<CoverageLaneDocument>, lane: PersistedCoverageLane): Promise<void> {
    await document.incrementalModify((currentDocument) => mergeLaneWithCurrentRevision(currentDocument, lane));
  }

  private async removeRecordIfUnchanged(record: PersistedCoverageRecord): Promise<boolean> {
    const document = await this.coverageRecords.findOne(coverageRecordKey(record.collection, record.id)).exec();
  ...
  private async removeLaneIfUnchanged(lane: PersistedCoverageLane): Promise<boolean> {
    const document = await this.coverageLanes.findOne(coverageLaneKey(lane.collection, lane.queryKey)).exec();
    if (!document) return false;
