...
import { COVERAGE_COMPACTION_FAILURE_KEY, type CoverageCompactionFailureDocument } from './coverage-compaction-failure-schema';
...
function fromDocument(document: CoverageCompactionFailureDocument): CoverageCompactionFailure | null {
...
export type CoverageCompactionFailureDatabase = { coverageCompactionFailures: RxCoverageCompactionFailureCollection<CoverageCompactionFailureDocument> };
...
export class RxCoverageCompactionFailureRepository {
  ...
  async readFailure(): Promise<CoverageCompactionFailure | null> {
    const document = await this.coverageCompactionFailures.findOne(COVERAGE_COMPACTION_FAILURE_KEY).exec();
  ...
  async recordFailure(failure: CoverageCompactionFailure): Promise<void> {
    assertBulkSuccess(await this.coverageCompactionFailures.bulkUpsert([toDocument(failure)]), 'rx-coverage-compaction-failure-repository upsert');
  }

  async clearFailure(): Promise<void> {
    assertBulkSuccess(await this.coverageCompactionFailures.bulkUpsert([toDocument(null)]), 'rx-coverage-compaction-failure-repository upsert');
  }
}
