...
const S = (id: number, digest: string): ServerDigestEntry => ({ id, digest, objectType: 'product' });
...
describe('runExistenceReconcile', () => {
  it('walks each bucket by its id-range and dispatches prune + (re)pull, accumulating a summary', async () => {
    ...
    expect((enqueuePull.mock.calls as unknown as Array<[unknown]>).map((c) => c[0])).toEqual([
  ...
  it('reads local and server for the correct half-open id-range per bucket', async () => {
    ...
    expect(readLocalBucket.mock.calls).toEqual([[0, 1000], [1000, 2000], [2000, 3000]]);
    expect(fetchServerBucket.mock.calls).toEqual([[0, 1000], [1, 1000], [2, 1000]]);
  });
