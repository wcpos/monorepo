...
import type { ExistenceManifestDocument } from '@woo-rxdb-lab/sync-engine-rxdb/testing';
...
describe('resolveDirtyWooIds', () => {
  it('resolves products/variations mutations recordId→wooId, ignoring other collections and unresolved ids', async () => {
    const lookup = vi.fn(async (recordId: string) =>
      ({ 'uuid-1': 10, 'uuid-2': 20, 'uuid-3': null } as Record<string, number | null>)[recordId] ?? null,
    );
...
describe('reconcileExistence', () => {
  const manifest = (wooId: number, digest: string, objectType: 'product' | 'variation' = 'product'): ExistenceManifestDocument => ({
    id: String(wooId),
    wooId,
    objectType,
    digest,
  });
  const server = (id: number, digest: string, objectType: 'product' | 'variation' = 'product'): ServerDigestEntry => ({
    id,
    digest,
    objectType,
  });
