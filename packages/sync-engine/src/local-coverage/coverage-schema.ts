import type { PersistedCoverageLane, PersistedCoverageRecord } from '../scheduler/persisted-coverage-schema';
...
export type CoverageRecordDocument = Omit<PersistedCoverageRecord, 'collection'> & PersistedSchedulerSchemaVersionMarker<2> & {
  coverageKey: string;
  collectionName: string;
};
export type CoverageRecordV0Document = PersistedCoverageRecord & { coverageKey: string };
export type CoverageRecordV1Document = CoverageRecordV0Document & PersistedSchedulerSchemaVersionMarker<1>;

export function migrateCoverageRecordV1(document: CoverageRecordV0Document): CoverageRecordV1Document {
  return markPersistedSchedulerDocument(document, 1);
}

export function migrateCoverageRecordV2(document: CoverageRecordV1Document | CoverageRecordDocument): CoverageRecordDocument {
...
export type CoverageLaneDocument = Omit<PersistedCoverageLane, 'collection'> & PersistedSchedulerSchemaVersionMarker<2> & {
  laneKey: string;
  collectionName: string;
};
export type CoverageLaneV0Document = PersistedCoverageLane & { laneKey: string };
export type CoverageLaneV1Document = CoverageLaneV0Document & PersistedSchedulerSchemaVersionMarker<1>;

export function migrateCoverageLaneV1(document: CoverageLaneV0Document): CoverageLaneV1Document {
  return markPersistedSchedulerDocument(document, 1);
}

export function migrateCoverageLaneV2(document: CoverageLaneV1Document | CoverageLaneDocument): CoverageLaneDocument {
  if ('collectionName' in document) {
    const { schemaVersion: _schemaVersion, ...rest } = document;
    return markPersistedSchedulerDocument(rest, 2);
  }
...
export const coverageRecordSchema = {
  ...
  required: ['coverageKey', 'collectionName', 'id', 'coveredQueryKeys', 'freshUntilMs', 'updatedAtMs', 'schemaVersion'],
  indexes: [
    ['collectionName', 'id'],
...
export const coverageLaneSchema = {
  ...
  required: ['laneKey', 'collectionName', 'queryKey', 'complete', 'expectedRecordIds', 'freshUntilMs', 'updatedAtMs', 'schemaVersion'],
  indexes: [
