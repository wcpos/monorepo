...
export type RxKeyedDocument<TDocument extends object> = {
	...
	incrementalModify?(
		mutationFunction: (
			document: TDocument & { _deleted?: boolean }
		) =>
			| TDocument
			| (TDocument & { _deleted?: boolean })
			| Promise<TDocument | (TDocument & { _deleted?: boolean })>
	): Promise<unknown>;
};
...
export type RxKeyedCollection<TDocument extends object> = {
	...
	find(query?: MangoQuery<TDocument>): {
		exec(): Promise<(RxKeyedDocument<TDocument> | TDocument)[]>;
	};
	findOne(documentId: string): {
		exec(includeDeleted?: boolean): Promise<RxKeyedDocument<TDocument> | null>;
	};
};
...
export function rxDocumentJson<TDocument extends object>(
	document: RxKeyedDocument<TDocument> | TDocument
): TDocument {
...
export function createRxKeyedRepository<TValue, TDocument extends object>(
	options: Options<TValue, TDocument>
) {
	const fromStored = (document: RxKeyedDocument<TDocument> | TDocument): TValue =>
		options.fromDocument(rxDocumentJson(document));

	async function findDocument(
		key: string,
		includeDeleted = false
	): Promise<RxKeyedDocument<TDocument> | null> {
	...
	async function modifyIfCurrent(
		...
		nextDocument: (
			current: TDocument & { _deleted?: boolean }
		) => TDocument & { _deleted?: boolean }
	): Promise<boolean> {
	...
	return {
		async upsert(value: TValue): Promise<void> {
			assertBulkSuccess(
				await options.collection.bulkUpsert([options.toDocument(value)]),
				'rx-keyed-repository upsert'
			);
		},
