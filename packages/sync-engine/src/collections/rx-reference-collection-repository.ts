...
export type ReferenceRxCollection = {
  ...
  find(): { exec(): Promise<Array<{ toJSON(): LocalReferenceDocument }>> };
};
...
export function referenceCollectionRepository(collection: ReferenceRxCollection): ReferenceCollectionRepository {
  return {
    async upsertMany(documents: LocalReferenceDocument[]): Promise<void> {
      assertBulkSuccess(await collection.bulkUpsert(documents), 'rx-reference-collection-repository upsert');
    },
    async pruneServerSourcedAbsent(keptDocumentIds: readonly string[]): Promise<string[]> {
      ...
      const toRemove = docs
        ...
        .map((doc) => doc.id);
      if (toRemove.length > 0) assertBulkSuccess(await collection.bulkRemove(toRemove), 'rx-reference-collection-repository remove');
      return toRemove;
    },
  };
}
