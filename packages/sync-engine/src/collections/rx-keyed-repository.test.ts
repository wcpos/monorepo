...
describe('createRxKeyedRepository', () => {
  ...
  it('returns the verdict from the final incremental-modify retry', async () => {
    ...
    const keyed = repository({
      findOne: () => ({ exec: async () => ({ toJSON: () => ({ id: 'one', value: 1, schemaVersion: 1 }), incrementalModify }) }),
    });

    await expect(keyed.replaceIfCurrent(
      { id: 'one', value: 1 },
      { id: 'one', value: 3 },
  ...
  it('deletes only while the current value still matches', async () => {
    ...
    const keyed = repository({
      findOne: () => ({
        exec: async () => ({
          toJSON: () => stored,
          incrementalModify: async (modify: (document: StoredState) => StoredState) => { stored = modify(stored); },
        }),
      }),
    });

    await expect(keyed.removeIfCurrent(
      { id: 'one', value: 1 },
      (left, right) => left.id === right.id && left.value === right.value,
    )).resolves.toBe(true);
