...
import { addRxPlugin, createRxDatabase, type HashFunction, type RxDatabase, type RxStorage } from 'rxdb';
...
import { createWriteDrainLane, fetchOrderServerRevision, type WriteOutcomeEvent } from './write-path/write-drain-lane';
import { createRequirePlane, type CoverageOutcome, type EngineRequirement, type RequirementHandle } from './require-plane';
...
import { createMaintenanceLanes, COVERAGE_COMPACTION_RETAIN_STALE_FOR_MS, type MaintenanceLaneName, type QueryTotalPort, type QueryTotalCacheEvent } from './maintenance/maintenance-lanes';
import { ORDER_SCHEDULER_COVERAGE_FRESH_FOR_MS } from './scheduler/engine-scheduler-drain';
import { createLocalCoverage, type LocalCoverage, type ReconcileRequest } from './local-coverage/local-coverage';
...
import { readManifestRange, removeManifestByWooIds, upsertManifestRows } from './local-coverage/rx-existence-manifest-repository';
...
export type { MaintenanceLaneName, MaintenanceLaneReport, QueryTotalPort, QueryTotalCacheEvent } from './maintenance/maintenance-lanes';
...
export type { SyncCollectionName, ResettableCollectionName } from './collections/engine-collections';
export type { StoreScopeIdentity } from '@woo-rxdb-lab/sync-core';
...
export type RxdbSyncEnginePorts = {
  ...
  storage: RxStorage<unknown, unknown> | ((identity: StoreScopeIdentity) => RxStorage<unknown, unknown>);
...
export type EngineEvent =
  ...
  | { type: 'reset-needs-confirmation'; scopeId: string; collection: typeof MUTATION_QUEUE_COLLECTION; detail?: string }
  | { type: 'guard'; kind: 'write-dropped' | 'late-response-dropped'; scopeId: string; detail?: string }
...
export type EngineStatus = {
  ...
  lanes: Record<Exclude<EngineLane, 'all'>, { lastError: string | null; lastTick: { atMs: number; status: SyncReport['status'] } | null }>;
...
export type RxdbSyncEngine = {
  ...
  write(intent: WriteIntent): Promise<{ mutationId: string; recordId: string; annihilated?: boolean }>;
  /**
   ...
   */
  resolveConflict(mutationId: string, resolution: 'retry-with-server-base' | 'discard'): Promise<void>;
...
export function createRxdbSyncEngine(
  ...
  const openScopeDatabase = async (scopeId: string): Promise<ScopeDatabase> => {
    ...
    try {
      ...
      const engineCheckpoint = await db.collections[ENGINE_KV_COLLECTION].findOne(CHANGE_SIGNAL_STATE_KEY).exec();
      if (!engineCheckpoint) {
        const legacyCheckpoint = await db.collections.changeSignalStates.findOne(CHANGE_SIGNAL_STATE_ID).exec();
        if (legacyCheckpoint) {
          const legacy = legacyCheckpoint.toJSON() as { state: string };
          await db.collections[ENGINE_KV_COLLECTION].upsert({ id: CHANGE_SIGNAL_STATE_KEY, value: legacy.state });
        }
      }
    } catch (error) {
      await db.close();
    ...
    const targeted = Object.fromEntries(
      COLLECTION_DESCRIPTORS.filter((descriptor) => descriptor.shape === 'targeted').map((descriptor) => [descriptor.collection, descriptor]),
    ) as Record<'products' | 'variations' | 'customers', Extract<(typeof COLLECTION_DESCRIPTORS)[number], { shape: 'targeted' }>>;
    ...
    const reconcilePort = (
      ...
      const sourceCollections = collection === 'products' ? ['products', 'variations'] as const : [collection] as const;
      const dirtyWooIds = async (): Promise<Set<number>> => {
        ...
        for (const name of sourceCollections) {
          ...
          for (const doc of docs) {
            const row = doc.toJSON() as { wooProductId?: number; wooId?: number; wooCustomerId?: number; wooOrderId?: number; local?: { dirty?: boolean; pendingMutationIds?: unknown[] } };
      ...
      const removeTargeted = async (name: 'products' | 'variations' | 'customers', field: string, wooIds: number[]) => {
        const docs = await db.collections[name].find({ selector: { [field]: { $in: wooIds } } as never }).exec();
        if (docs.length > 0) assertBulkSuccess(await db.collections[name].bulkRemove(docs.map((doc) => doc.primary)), 'create-rxdb-sync-engine remove');
      ...
      ) => pullTargetedByIds({ ...handlerContext, fetch: request?.fetcher ?? fetcher }, descriptor, wooIds, async (documents) => {
        const manifestRows = documents.flatMap((document) => manifestRowOf(document) ? [manifestRowOf(document)!] : []);
        assertBulkSuccess(await db.collections[descriptor.collection].bulkUpsert(documents as never[]), 'create-rxdb-sync-engine upsert');
      ...
      return {
        ...
        maxWooId: async () => {
          ...
          return docs.reduce((max, doc) => Math.max(max, Number((doc.toJSON() as { wooId?: unknown }).wooId) || 0), 0);
        },
        ...
        fetchServerBucket: async (bucket: number, bucketSize: number, request?: ReconcileRequest) => {
          const response = await (request?.fetcher ?? fetcher)(`${ports.site.syncBaseUrl}/integrity/bucket?bucket=${bucket}&bucket_size=${bucketSize}${collectionParam}`, request?.signal ? { signal: request.signal } : undefined);
          if (!response.ok) throw new Error(`existence bucket fetch failed: ${response.status}`);
          const body = await response.json() as { ids?: Array<{ id: number; digest: string; object_type?: string }> };
          return (body.ids ?? []).map((row) => ({ id: row.id, digest: row.digest, objectType: (row.object_type ?? (collection === 'orders' ? 'order' : collection === 'customers' ? 'customer' : 'product')) as 'product' | 'variation' | 'customer' | 'order' }));
        },
        deleteProducts: async (wooIds: number[]) => {
          if (collection === 'orders') return new EngineOrderRepository(db.collections as never).removeDeletedOrders(wooIds);
          return removeTargeted(collection, collection === 'products' ? 'wooProductId' : 'wooCustomerId', wooIds);
        },
        ...
        pullProducts: async (wooIds: number[], request?: ReconcileRequest) => {
          if (collection === 'orders') {
            for (const batch of chunk(wooIds, WOO_REST_MAX_PER_PAGE)) {
              const response = await (request?.fetcher ?? fetcher)(`${ports.site.syncBaseUrl}/orders?include=${batch.join(',')}&per_page=${batch.length}&orderby=include`, request?.signal ? { signal: request.signal } : undefined);
              if (!response.ok) throw new Error(`order existence pull failed: ${response.status}`);
              const payloads = await response.json() as Array<Record<string, unknown>>;
              const payloadByWooId = new Map(payloads.map((payload) => [Number(payload.id), payload]));
              const existingPayloads = batch.flatMap((wooId) => {
                const payload = payloadByWooId.get(wooId);
                return payload ? [payload] : [];
              });
              await new EngineOrderRepository(db.collections as never).upsertMany(existingPayloads.map((payload) => orderDocumentFromWooPayload(payload)));
            }
            return;
          }
          await pullTargetedAndPopulateManifest(targeted[collection], wooIds, request);
        },
        pullVariations: async (wooIds: number[], request?: ReconcileRequest) => { await pullTargetedAndPopulateManifest(targeted.variations, wooIds, request); },
      };
    };
    localCoverageByScopeId.set(scopeId, createLocalCoverage({
  ...
  for (const collection of SYNC_COLLECTION_NAMES) {
    manager.registerCursorInvalidator(collection, (scopeId) => removeCheckpoint(scopeId, collection));
  }
  ...
  const registerManifestInvalidator = (
    ...
    if (docs.length > 0) assertBulkSuccess(await manifest.bulkRemove(docs.map((doc) => doc.primary)), 'create-rxdb-sync-engine remove');
  });
  ...
  const emitEngineEvent = (event: EngineEvent): void => {
    ...
    const scopeEvent: EngineScopeEvent | null = (() => {
      switch (event.type) {
        case 'scope-switched': return { type: 'switched', scopeId: event.scopeId };
        case 'collection-reset': return { type: 'reset', scopeId: event.scopeId, collection: event.collection };
        case 'reset-needs-confirmation': return { type: 'needs-confirmation', scopeId: event.scopeId, ...(event.detail !== undefined ? { detail: event.detail } : {}) };
        case 'guard': return { type: event.kind, scopeId: event.scopeId, ...(event.detail !== undefined ? { detail: event.detail } : {}) };
        case 'bootstrap-failed': return { type: 'bootstrap-failed', scopeId: event.scopeId, detail: event.detail };
        default: return null;
      }
    })();
    if (scopeEvent) {
      for (const subscriber of [...scopeEventSubscribers]) {
        try { subscriber(scopeEvent); } catch { /* observer seam never throws into the engine */ }
      }
    }
  };
  ...
  manager.onEvent((event: ScopeEvent) => {
    switch (event.type) {
      ...
      case 'reset': {
        diagnostics({ type: 'engine.collection-reset', level: 'info', collection: event.detail, fields: { scopeId: event.scopeId } });
        emitEngineEvent({
      ...
      case 'needs-confirmation': {
        diagnostics({ type: 'engine.reset-needs-confirmation', level: 'warn', collection: 'mutations', fields: { scopeId: event.scopeId } });
        emitEngineEvent({
      ...
      case 'late-response-dropped': {
        diagnostics({ type: 'engine.guard', level: 'warn', fields: { scopeId: event.scopeId, outcome: event.type } });
        emitEngineEvent({
          type: 'guard',
          kind: event.type,
          scopeId: event.scopeId,
  ...
  const switchScope = (identity: StoreScopeIdentity): Promise<ActiveScope> => {
    ...
    return enqueueLifecycle(async () => {
      ...
      diagnostics({ type: 'engine.scope-switched', level: 'info', message: `active scope: ${scopeId}` });
  ...
  manager.registerCursorInvalidator('orders', async (scopeId) => {
    const db = databaseByScopeId.get(scopeId);
    if (!db) return;
    await new EngineOrderRepository(db as never).writeCustomPullCheckpoint(normalizeCheckpoint(null));
  });
  ...
  const runAutomaticTick = async (tick: () => Promise<SyncReport>): Promise<void> => {
    ...
    if (report.lane !== 'all') laneLastTick.set(report.lane, { atMs: ports.now !== undefined ? ports.now() : Date.now(), status: report.status });
    diagnostics({
  ...
  return {
    ...
    scope: {
      ...
      resetCollection: async (name, opts) => {
        ...
        return enqueueLifecycle(async () => {
          const scopeId = manager.activeScope;
          if (scopeId === null) {
            throw new Error('Cannot reset a collection: no active scope (did the initial open fail?)');
          }
    ...
    write: async (intent) => {
      ...
      if (!writeFacetFor(intent.collection)) {
        throw new Error(`write: collection "${intent.collection}" is not client-writeable (no push/ack contract) — orders is the writeable collection today`);
      }
      ...
      return manager.runGuarded(async (bound) => {
        ...
        const enqueueResult = result as { mutationId: string; recordId: string; annihilated?: boolean };
        if (enqueueResult.annihilated) {
          ...
          emitEngineEvent({ type: 'write-annihilated', collection: intent.collection, recordId: enqueueResult.recordId, mutationId: enqueueResult.mutationId });
        ...
        diagnostics({
          ...
          fields: { mutationId: enqueueResult.mutationId, recordId: enqueueResult.recordId, queueDepth: writeDrainLane.lastKnownQueueDepth() },
        });
        return enqueueResult;
      });
    },
    conflicts: async () => {
      ...
      if (!database) return [];
      return (await queueFor(database).all()).filter((entry) => entry.status === 'conflicted' || entry.status === 'needs-revision' || entry.status === 'rejected');
    },
    resolveConflict: async (mutationId, resolution) => {
      ...
      await manager.runGuarded(async (bound) => {
        ...
        if (!entry || (entry.status !== 'conflicted' && entry.status !== 'needs-revision' && entry.status !== 'rejected')) {
        ...
        if (resolution === 'retry-with-server-base') {
          ...
          if (!serverBase) {
            if (entry.collectionName !== 'orders') {
              throw new Error(`resolveConflict: no server revision on "${mutationId}" and no refresh seam for "${entry.collectionName}" — discard instead`);
            }
            const row = (await database.collections.orders?.findOne(entry.recordId).exec())?.toJSON() as { wooOrderId?: number | null } | undefined;
            if (typeof row?.wooOrderId !== 'number') {
              throw new Error(`resolveConflict: cannot refresh the server revision for "${mutationId}" — the record has no server identity; discard instead`);
            }
            serverBase = await fetchOrderServerRevision({ fetch: bound.bindFetch(fetcher as never) as never, wpJsonRoot: ports.site.wpJsonRoot, wooOrderId: row.wooOrderId });
            if (!serverBase) {
              throw new Error(`resolveConflict: the server no longer returns record ${row.wooOrderId} — the row stays parked; retry later or discard`);
            }
          }
        }
        const applied = await bound.guardWrite(async () => {
          if (resolution === 'retry-with-server-base') {
            ...
            const { conflictDocument: _document, conflictRevision: _revision, attempts: _attempts, nextAttemptAt: _gate, ...intact } = entry;
            await queue.replace({ ...intact, baseRevision: serverBase ?? entry.baseRevision, status: 'pending' });
            ...
            const doc = await database.collections[entry.collectionName]?.findOne(entry.recordId).exec() as { incrementalModify(fn: (data: Record<string, unknown>) => Record<string, unknown>): Promise<unknown> } | null;
            if (doc && serverBase) {
            ...
            const doc = await database.collections[entry.collectionName]?.findOne(entry.recordId).exec() as { toJSON(): Record<string, unknown>; incrementalModify(fn: (data: Record<string, unknown>) => Record<string, unknown>): Promise<unknown> } | null;
            ...
            if (doc) {
              await doc.incrementalModify((data) => {
                ...
                const pendingMutationIds = (local.pendingMutationIds ?? []).filter((id) => id !== mutationId);
        ...
        if (applied === 'dropped') {
          throw new Error('resolveConflict: scope moved during resolution — retry against the settled scope');
        }
      ...
      if (needsRepull !== null) {
        try {
          await requirePlane.require({ id: `conflict-discard:${mutationId}`, collection: 'orders', kind: 'targeted-records', wooIds: [needsRepull], forceRefresh: true, priority: 1_000 }).ready;
        } catch (error) {
          diagnostics({
      ...
      if (settled) {
        diagnostics({ type: 'queue.write.resolve', level: 'info', collection: settled.collectionName, fields: { recordId: settled.recordId, mutationId, resolution } });
      }
    },
    require: (requirement) => {
      assertNotDisposed();
    ...
    sync: async (lane, options) => {
      ...
      const MAINTENANCE_LANES: Record<MaintenanceLaneName, (signal?: AbortSignal) => Promise<SyncReport>> = {
        ...
        'query-total-retry': async (signal) => maintenanceLanes.queryTotalRetry !== null
      ...
      if (lane !== undefined && lane !== 'change-signal' && lane !== 'write-drain' && !(lane in MAINTENANCE_LANES)) {
      ...
      if (pendingLifecycleOps > 0) {
        ...
        return finish({ lane: lane ?? 'all', status: 'skipped', reason: 'lifecycle operation pending' });
      }
      ...
      const ordered: EngineLane[] = ['change-signal', 'write-drain', 'order-window-seed', 'reference-seed', 'scheduler-drain', 'query-total-retry', 'coverage-compaction', 'existence-prime', 'existence-reconcile'];
      const reports: SyncReport[] = [];
      for (const name of ordered) {
        const report = name === 'change-signal'
        ...
        laneLastTick.set(name, { atMs: ports.now !== undefined ? ports.now() : Date.now(), status: report.status });
        reports.push(report);
      }
      ...
      return finish({
        lane: 'all',
        status: worst,
        ...(reports.find((report) => report.error)?.error ? { error: reports.find((report) => report.error)!.error } : {}),
        ...(reports.find((report) => report.reason)?.reason ? { reason: reports.find((report) => report.reason)!.reason } : {}),
        ...(drain.pushed !== undefined ? { pushed: drain.pushed, conflicts: drain.conflicts, deferred: drain.deferred, failed: drain.failed, rejected: drain.rejected } : {}),
      });
    },
    events: (cb) => {
      assertNotDisposed();
    ...
    status: () => {
      ...
      const stats = manager.stats();
      const laneStatus = (name: EngineLane, lastError: string | null) => ({ lastError, lastTick: laneLastTick.get(name) ?? null });
      return {
        disposed,
        mode,
        connectivity: connectivityNow,
        gatedBy: pendingLifecycleOps > 0 ? 'lifecycle' : connectivityNow === 'offline' ? 'offline' : (manager.activeScope !== null && bootstrapFailures.has(manager.activeScope)) ? 'bootstrap-failed' : null,
        ...
        lanes: {
          ...
          'order-window-seed': laneStatus('order-window-seed', maintenanceLanes.orderWindowSeed.lastError()),
          'reference-seed': laneStatus('reference-seed', maintenanceLanes.referenceSeed.lastError()),
          'scheduler-drain': laneStatus('scheduler-drain', maintenanceLanes.schedulerDrain.lastError()),
          'query-total-retry': laneStatus('query-total-retry', maintenanceLanes.queryTotalRetry?.lastError() ?? null),
          'coverage-compaction': laneStatus('coverage-compaction', maintenanceLanes.coverageCompaction.lastError()),
          'existence-prime': laneStatus('existence-prime', maintenanceLanes.existencePrime.lastError()),
          'existence-reconcile': laneStatus('existence-reconcile', maintenanceLanes.existenceReconcile.lastError()),
        },
    ...
    dispose: async () => {
      ...
      return enqueueLifecycle(async () => {
        ...
        diagnostics({ type: 'engine.disposed', level: 'info', message: 'engine disposed; every scope database closed' });
      });
    },
  };
}
