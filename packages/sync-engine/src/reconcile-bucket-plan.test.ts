...
const L = (wooId: number, digest: string, extra: { objectType?: 'product' | 'variation'; dirty?: boolean } = {}) => ({
  wooId,
  digest,
  objectType: extra.objectType ?? 'product',
  ...(extra.dirty ? { dirty: true } : {}),
});
const S = (id: number, digest: string, objectType: 'product' | 'variation' = 'product') => ({ id, digest, objectType });
const P = (wooId: number, objectType: 'product' | 'variation' = 'product') => ({ wooId, objectType });
...
describe('reconcileBucketPlan', () => {
  ...
  it('carries the lane per action so the executor routes products vs variations', () => {
    ...
    const plan = reconcileBucketPlan([L(5, 'a', { objectType: 'variation' })], [S(9, 'b', 'variation')]);
  ...
  describe('dirty guard', () => {
    it('never prunes a dirty record the server lacks — surfaces it in skippedDirty', () => {
      expect(reconcileBucketPlan([L(5, 'a', { dirty: true })], [])).toEqual({ ...empty, skippedDirty: [P(5)] });
    });
