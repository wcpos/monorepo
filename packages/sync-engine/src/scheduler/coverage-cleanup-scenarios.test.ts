...
import { coverageCleanupScenarios, summarizeCoverageCleanupScenarios } from './coverage-cleanup-scenarios';
...
describe('coverageCleanupScenarios', () => {
  ...
  it('encodes stale expected records as refresh work', () => {
    const scenario = coverageCleanupScenarios.find((item) => item.id === 'open-orders-stale-expected-record');
    expect(scenario).toBeDefined();

    const decisions = planCoverageCleanup(scenario!.input);
    expect(decisions).toEqual([
      { recordId: 'woo-order:open-1', action: 'refresh', reason: 'record is expected by lane but stale' },
      { recordId: 'woo-order:open-2', action: 'keep', reason: 'record is expected by lane and fresh' },
    ]);
  });
