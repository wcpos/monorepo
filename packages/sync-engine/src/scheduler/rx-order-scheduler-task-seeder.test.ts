...
import { seedOrderFilterSchedulerTask, seedOrderSchedulerTasks, seedTargetedOrderSchedulerTask } from './rx-order-scheduler-task-seeder';
...
describe('seedOrderSchedulerTasks', () => {
  ...
  it('seeds the supported background order custom-pull task without claiming it', async () => {
    ...
    await expect(seedOrderSchedulerTasks({ getRepository: mocks.getRepository, perPage: 100, nowMs: 10_000 })).resolves.toBe(result);
  ...
  it('seeds a deduped targeted order task without claiming it', async () => {
    ...
    await expect(seedTargetedOrderSchedulerTask({ getRepository: mocks.getRepository,
  ...
  it('seeds a status-only browser order filter descriptor without claiming it', async () => {
    ...
    await expect(seedOrderFilterSchedulerTask({ getRepository: mocks.getRepository,
      status: 'processing',
      search: '',
      limit: 50,
      priority: 750,
