...
import { productDocumentId, type ProductDocument, type StoredProductDocument, type WooProductPayload } from '@woo-rxdb-lab/shared';
import type { FetchTask, FetchTaskResult } from './replication-policy';
import { materializeTargeted, type Materialized } from '../materialization/record-materialization';
...
async function persistProductDocuments(input: ProductsSchedulerFetcherInput, records: Materialized<Record<string, unknown>>[]): Promise<void> {
  await input.repository.upsertMany(records.map(({ storedDocument }) => storedDocument as StoredProductDocument));
...
function productDocumentFromWooPayload(payload: WooProductPayload): Materialized<Record<string, unknown>> {
...
async function fetchTargetedProducts(input: ProductsSchedulerFetcherInput, task: FetchTask, context?: SchedulerFetcherContext): Promise<FetchTaskResult> {
  ...
  for (const idsBatch of chunk(targetedProductIds(task), batchSize)) {
    ...
    assertReturnedRequestedIds({ restLabel: 'product', payloadWooId: (payload: WooProductPayload) => Number(payload.id) }, idsBatch, payloads);
    const documents = payloads.map(productDocumentFromWooPayload);
    await persistProductDocuments(input, documents);
    fetchedDocumentIds.push(...documents.map(({ storedDocument }) => coverageRecordId(storedDocument as ProductDocument)));
...
async function fetchProductQuery(input: ProductsSchedulerFetcherInput, query: URLSearchParams, context?: SchedulerFetcherContext): Promise<WooProductPayload[]> {
  const url = `${input.baseUrl}/products?${query.toString()}`;
  const response = await httpGet(input, url, context);
  if (!response.ok) {
...
async function fetchProductSearch(input: ProductsSchedulerFetcherInput, task: FetchTask, search: string, context?: SchedulerFetcherContext): Promise<FetchTaskResult> {
  const limit = taskLimit(task);
  const searchPayloads = await fetchProductQuery(input, productSearchParams(search, limit), context);
  ...
  await recordCoverage('products', input, task, documents.map(({ storedDocument }) => coverageRecordId(storedDocument as ProductDocument)), complete);

  return { taskId: task.id, documentCount: documents.length, requestCount: 2, completed: complete };
}

export function createProductsSchedulerFetcher(input: ProductsSchedulerFetcherInput): SchedulerFetcher {
