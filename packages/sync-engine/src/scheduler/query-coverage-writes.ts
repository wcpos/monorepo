...
export function deriveQueryCoverageCompleteness(input: QueryCoverageCompletenessEvidence): boolean {
  return input.totalMatchingRecords !== null && input.returnedRecordCount === input.totalMatchingRecords;
}
...
export type BuildCumulativeCoverageDocumentsFromQueryResultInput = BuildCoverageDocumentsFromQueryResultInput & {
  resetCumulativeExpectedIds?: boolean;
};

export function buildCoverageDocumentsFromQueryResult(input: BuildCoverageDocumentsFromQueryResultInput): PersistedCoverageDocumentSet {
