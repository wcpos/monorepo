...
import { runPersistedSchedulerTasks, type PersistedSchedulerTaskRunnerResult } from './rx-scheduler-task-runner';
import { RxSchedulerTaskStateRepository, type SchedulerTaskStateDatabase } from './rx-scheduler-task-state-repository';
import type { LocalCoverage } from '../local-coverage/local-coverage';
import { createSchedulerFetcherRegistry, type SchedulerTaskSupportCandidate } from './scheduler-fetcher-registry';
...
import { createReferenceCollectionFetcher, BRAND_REFERENCE_CONFIG, CATEGORY_REFERENCE_CONFIG, COUPON_REFERENCE_CONFIG, TAG_REFERENCE_CONFIG } from './rx-scheduler-reference-fetcher';
...
import { upsertManifestRows, type ManifestCollection } from '../local-coverage/rx-existence-manifest-repository';
import { EngineOrderRepository, type OrderRepositoryDatabase } from '../write-path/engine-order-repository';
...
export function isSupportedTaxRateSchedulerTask(task: SchedulerTaskSupportCandidate): boolean {
  return task.collection === 'taxRates'
    && task.queryKey === SUPPORTED_TAX_RATE_QUERY_KEY
    && task.mode === 'greedy'
    && hasNoTargetedIds(task);
}

export function isSupportedReferenceSchedulerTask(task: SchedulerTaskSupportCandidate, collection: string, queryKey: string): boolean {
  return task.collection === collection
...
type BulkUpsertCollection<T> = {
  bulkUpsert(documents: T[]): Promise<unknown>;
};

/** The generic pull-apply adapter every non-order fetcher writes through. */
export function collectionSchedulerRepository<T>(collection: BulkUpsertCollection<T>): { upsertMany(documents: T[]): Promise<void> } {
  return {
    async upsertMany(documents: T[]): Promise<void> {
      assertBulkSuccess(await collection.bulkUpsert(documents), 'engine-scheduler-drain upsert');
    },
  };
}

/** Structural: the collections the drain touches (superset of the repos it builds). */
export type SchedulerDrainDatabase = OrderRepositoryDatabase & SchedulerTaskStateDatabase & {
  products: BulkUpsertCollection<unknown>;
  customers: BulkUpsertCollection<unknown>;
  taxRates: BulkUpsertCollection<unknown>;
  categories: BulkUpsertCollection<unknown> & { find(query?: unknown): { exec(): Promise<Array<{ toJSON(): unknown }>> }; bulkRemove(ids: string[]): Promise<unknown> };
  brands: BulkUpsertCollection<unknown> & { find(query?: unknown): { exec(): Promise<Array<{ toJSON(): unknown }>> }; bulkRemove(ids: string[]): Promise<unknown> };
  tags: BulkUpsertCollection<unknown> & { find(query?: unknown): { exec(): Promise<Array<{ toJSON(): unknown }>> }; bulkRemove(ids: string[]): Promise<unknown> };
  coupons: BulkUpsertCollection<unknown> & { find(query?: unknown): { exec(): Promise<Array<{ toJSON(): unknown }>> }; bulkRemove(ids: string[]): Promise<unknown> };
...
export async function runEngineSchedulerDrain(input: RunEngineSchedulerDrainInput): Promise<PersistedSchedulerTaskRunnerResult> {
  const db = input.db;
  ...
  const coverageRepository = {
    recordQueryResult: (value: Parameters<LocalCoverage['recordQueryResult']>[0]) => input.coverage.recordQueryResult(value),
    recordRecords: (value: Parameters<LocalCoverage['recordRecords']>[0]) => input.coverage.recordRecords(value),
    recordCumulativeQueryResult: (value: Parameters<LocalCoverage['recordCumulativeQueryResult']>[0]) => input.coverage.recordCumulativeQueryResult(value),
    readLocalLaneCoverage: (collection: string, queryKey: string) => input.coverage.readLane(collection, queryKey),
  };
  const shared = {
  ...
  const fetcherRegistry = createSchedulerFetcherRegistry([
    ...
    ] as const).map(([name, config]) => ({
      name,
      supportsTask: (task: SchedulerTaskSupportCandidate) => isSupportedReferenceSchedulerTask(task, config.collection, config.queryKey),
