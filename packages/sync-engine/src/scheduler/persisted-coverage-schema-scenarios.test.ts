...
describe('persisted coverage schema scenarios', () => {
	...
	it('formats a reviewer-facing scenario summary', () => {
		expect(summarizePersistedCoverageSchemaScenario(persistedCoverageSchemaScenarios[0])).toBe(
			'2 fresh records, 1 fresh lane, 2 expected ids, retention keep 3 / refresh 0 / remove 0'
		);
	});
});
