 ...
 * `ids`/`remoteIds` keys are PRESENT with value `undefined`. */
...
describe('scheduler task state repository under dev-mode (z-schema) validation', () => {
	it('claimNew inserts a lane task whose ids/remoteIds are explicitly undefined (VD2 repro)', async () => {
		...
		// (the sameSchedulerTaskState CAS compare depends on absence).
		expect(Object.keys(stored!)).not.toContain('ids');
		expect(Object.keys(stored!)).not.toContain('remoteIds');
		expect(Object.keys(stored!)).not.toContain('rerunRequested');
	});

	it('upsert of a lane task passes validation and round-trips with ids/remoteIds absent', async () => {
	...
	it('a targeted task keeps its ids/remoteIds arrays intact through insert + read', async () => {
		const { repository, storedJson } = await taskStateRepository();
	...
	it('the in-flight rerun-requested write path also passes validation for lane tasks', async () => {
		const { repository, storedJson } = await taskStateRepository();
		// Claim an in-flight lane task whose ids/remoteIds are explicitly undefined on the
		...
		// in-flight branch, which must strip the present-but-undefined ids/remoteIds keys the
