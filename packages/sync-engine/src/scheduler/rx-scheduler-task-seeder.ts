...
export type SchedulerTaskSeederRepository = {
  ...
  claim(expectedState: PersistedSchedulerTaskState, queuedState: PersistedSchedulerTaskState): Promise<boolean>;
...
function queuedStateFromTask(task: FetchTask, nowMs: number, attempt = 0): PersistedSchedulerTaskState {
  return {
    taskId: task.id,
...
export async function seedPersistedSchedulerTasks(input: SeedPersistedSchedulerTasksInput): Promise<SeedPersistedSchedulerTasksResult> {
  ...
  for (const task of tasks) {
    ...
    if (existing.status === 'completed') {
      ...
      if (input.completedDedupeForMs > 0 && input.nowMs - existing.updatedAtMs <= input.completedDedupeForMs) {
        result.skippedCompleted += 1;
        continue;
      }

      const requeued = await input.repository.claim(existing, queuedStateFromTask(task, input.nowMs, existing.attempt));
