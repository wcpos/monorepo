...
import { coverageCompactionCadenceScenarios, summarizeCoverageCompactionCadenceScenarios } from './coverage-compaction-cadence-scenarios';
...
describe('coverageCompactionCadenceScenarios', () => {
  it('covers run, skip, wait, and takeover ownership decisions', () => {
    ...
    expect(summaries.map((summary) => summary.action)).toEqual(['run', 'skip', 'wait-for-owner', 'take-over', 'wait-for-failure-backoff']);
