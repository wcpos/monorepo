...
describe('createCustomerSchedulerFetcher', () => {
  it('fetches targeted customer tasks through Woo REST include and stores customer documents', async () => {
    const repository = { upsertMany: vi.fn(async (_documents: LocalCustomerDocument[]) => undefined) };
    const fetcher = vi.fn(async () => response([
      { id: 12, email: 'ada@example.test', date_modified_gmt: '2026-05-28T10:00:00', meta_data: uuidMeta(12) },
      { id: 34, email: 'grace@example.test', date_modified_gmt: '2026-05-28T10:05:00', meta_data: uuidMeta(34) },
    ]));
    ...
    const result = await schedulerFetcher(customerTask({
    ...
    expect(fetcher).toHaveBeenCalledWith('http://wcpos.local/wp-json/wc-rxdb-sync/v1/customers?include=12%2C34&per_page=2&orderby=include');
    expect(repository.upsertMany).toHaveBeenCalledWith([
      expect.objectContaining({ id: uuidFor(12), wooCustomerId: 12, payload: expect.objectContaining({ email: 'ada@example.test' }), sync: expect.objectContaining({ revision: '2026-05-28T10:00:00', source: 'woo-rest', partial: false }) }),
      expect.objectContaining({ id: uuidFor(34), wooCustomerId: 34, payload: expect.objectContaining({ email: 'grace@example.test' }), sync: expect.objectContaining({ revision: '2026-05-28T10:05:00', source: 'woo-rest', partial: false }) }),
    ]);
    expect(result).toEqual({ taskId: 'customers:ids:12,34:on-demand', documentCount: 2, requestCount: 1, completed: true });
  });
  ...
  it('stores the existing default customer target without issuing an invalid Woo include request', async () => {
    ...
    expect(repository.upsertMany).toHaveBeenCalledWith([
      expect.objectContaining({ id: 'customer:default', wooCustomerId: null, payload: {}, sync: expect.objectContaining({ revision: '', source: 'woo-rest', partial: true }) }),
    ]);
    ...
    expect(result).toEqual({ taskId: 'customers:ids:default:on-demand', documentCount: 1, requestCount: 0, completed: true });
  });

  it('fetches customer search lanes and records complete coverage only when Woo returns fewer rows than requested', async () => {
    const repository = { upsertMany: vi.fn(async (_documents: LocalCustomerDocument[]) => undefined) };
    const coverageRepository = { recordQueryResult: vi.fn(async () => undefined) };
    const fetcher = vi.fn(async () => response([{ id: 56, email: 'alex@example.test', date_modified_gmt: '2026-05-28T11:00:00', meta_data: uuidMeta(56) }]));
    const schedulerFetcher = createCustomerSchedulerFetcher({
      baseUrl: 'http://wcpos.local/wp-json/wc-rxdb-sync/v1',
      repository,
      coverageRepository,
    ...
    expect(fetcher).toHaveBeenCalledWith('http://wcpos.local/wp-json/wc-rxdb-sync/v1/customers?search=alex&per_page=25&page=1&orderby=id&order=desc');
    expect(coverageRepository.recordQueryResult).toHaveBeenCalledWith({
      collection: 'customers',
      queryKey: 'customers:search=alex:limit=25',
    ...
    expect(result).toEqual({ taskId: 'customers:search:alex:windowed', documentCount: 1, requestCount: 1, completed: true });
  });
  ...
  it('rejects customer search tasks whose queryKey limit does not match the task limit', async () => {
    ...
    await expect(schedulerFetcher(customerTask({ queryKey: 'customers:search=alex:limit=10', limit: 25 })))
      .rejects.toThrow('Customer scheduler task limit does not match queryKey limit: customers:search=alex:limit=10');
    expect(fetcher).not.toHaveBeenCalled();
  ...
  it('fetches enough customer search pages to satisfy a task limit above the Woo page cap', async () => {
    const repository = { upsertMany: vi.fn(async (_documents: LocalCustomerDocument[]) => undefined) };
    const coverageRepository = { recordQueryResult: vi.fn(async () => undefined) };
    const firstPage = Array.from({ length: 100 }, (_, index) => ({ id: index + 1, email: `page-one-${index}@example.test`, date_modified_gmt: '2026-05-28T11:00:00', meta_data: uuidMeta(index + 1) }));
    const secondPage = Array.from({ length: 100 }, (_, index) => ({ id: index + 101, email: `page-two-${index}@example.test`, date_modified_gmt: '2026-05-28T11:05:00', meta_data: uuidMeta(index + 101) }));
    const fetcher = vi.fn()
    ...
    const result = await schedulerFetcher(customerTask({ limit: 150, queryKey: 'customers:search=alex:limit=150' }));
    ...
    expect(result).toEqual({ taskId: 'customers:search:alex:windowed', documentCount: 150, requestCount: 2, completed: true });
  });
  ...
  it('does not mark a search lane complete when the final fetched page is short but some returned rows were truncated by the task limit', async () => {
    ...
    const coverageRepository = { recordQueryResult: vi.fn(async () => undefined) };
    const firstPage = Array.from({ length: 100 }, (_, index) => ({ id: index + 1, email: `page-one-${index}@example.test`, date_modified_gmt: '2026-05-28T11:00:00', meta_data: uuidMeta(index + 1) }));
    const secondPage = Array.from({ length: 75 }, (_, index) => ({ id: index + 101, email: `page-two-${index}@example.test`, date_modified_gmt: '2026-05-28T11:05:00', meta_data: uuidMeta(index + 101) }));
    const fetcher = vi.fn()
      .mockResolvedValueOnce(response(firstPage))
      .mockResolvedValueOnce(response(secondPage));
    const schedulerFetcher = createCustomerSchedulerFetcher({
