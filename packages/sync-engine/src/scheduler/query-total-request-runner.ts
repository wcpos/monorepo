...
import { RxQueryTotalCacheRepository, type QueryTotalCacheDatabase } from '../collections/rx-query-total-cache-repository';
import { RxQueryTotalRequestStateRepository, type QueryTotalRequestStateDatabase } from '../rx-query-total-request-state-repository';
...
import { planQueryTotalRequestLifecycle, sameQueryTotalRequestState, type QueryTotalRequestState } from './query-total-request-lifecycle';
import {
...
export type QueryTotalRequestRunnerDeps = {
  ...
  publishCacheEntries: (updater: (current: QueryTotalCacheEntry[]) => QueryTotalCacheEntry[]) => void;
  now?: () => number;
  isOffline?: () => boolean;
};
...
export function mergeQueryTotalCacheEntries(current: QueryTotalCacheEntry[], incoming: QueryTotalCacheEntry[]): QueryTotalCacheEntry[] {
...
function latestQueryTotalRequestState(states: QueryTotalRequestState[], queryKey: string): QueryTotalRequestState | null {
  return states
...
export function createQueryTotalRequestRunner(deps: QueryTotalRequestRunnerDeps): QueryTotalRequestRunner {
  ...
  const fail = (message: string, error: unknown) =>
    deps.onDiagnostic({ level: 'error', message: `${message}: ${describeUnknown(error)}`, detail: diagnosticDetail(error) });

  const request = (input: QueryTotalRequestInput): void => {
    const { database } = deps;
    const nowMs = now();
    const requestStateRepository = database ? new RxQueryTotalRequestStateRepository(database) : null;
    ...
    void (async () => {
      try {
        ...
        if (database) {
          try {
            ...
            if (freshEntries.length > 0) {
              ...
              deps.publishCacheEntries((current) => mergeQueryTotalCacheEntries(current, freshEntries));
            }
          } catch (error: unknown) {
        ...
        const requestStates = abandonedRequestState
          ? persistedRequestStates.filter((state) => !sameQueryTotalRequestState(state, abandonedRequestState))
        ...
        if (decision.action === 'use-cached-total') {
          info(`Using cached Woo query total for ${decision.queryKey}: ${decision.totalMatchingRecords}`);
          return;
        }
        ...
        const writeFailedState = (failedAtMs: number) => {
          ...
          requestStateRepository?.markFailed(requestState, failedState).catch((stateError: unknown) => {
            fail(`Query total request failure state write failed for ${decision.queryKey}`, stateError);
          });
        };
  ...
  const abandon = (input: { queryKey: string }): void => {
    ...
    requestStateRepository.remove(activeRequest.state)
      .catch((error: unknown) => {
      ...
      .finally(() => {
        ...
        if (abandonedState && activeRequest.state && sameQueryTotalRequestState(abandonedState, activeRequest.state)) {
