import { planCoverageCompactionCadence, type CoverageCompactionCadenceDecision, type CoverageCompactionCadenceInput } from './coverage-compaction-cadence';
...
export type CoverageCompactionCadenceScenarioSummary = Pick<CoverageCompactionCadenceDecision,
  'action' | 'reason' | 'expiredDocuments' | 'refreshableDocuments' | 'keepDocuments' | 'ownerId' | 'previousOwnerId' | 'nextDueAtMs'
> & {
  scenarioId: string;
  label: string;
};
...
export const coverageCompactionCadenceScenarios: CoverageCompactionCadenceScenario[] = [
  {
    ...
    description: 'Current tab can run compaction when expired persisted coverage is due and no lease is active.',
  ...
  },
  {
    ...
    description: 'Stale coverage inside the retention window remains available for refresh and should not trigger compaction.',
  ...
  },
  {
    ...
    input: { ...baseInput, documents: { records: [expiredRecord], lanes: [freshLane] }, lease: { ownerId: 'tab-b', acquiredAtMs: 900, expiresAtMs: 1_200 } },
  },
  {
    ...
    input: { ...baseInput, documents: { records: [expiredRecord], lanes: [freshLane] }, lease: { ownerId: 'tab-b', acquiredAtMs: 400, expiresAtMs: 900 } },
  },
  {
    id: 'expired-coverage-failure-backoff',
    label: 'Expired coverage · failure backoff',
    description: 'Current tab waits before retrying destructive compaction when the previous attempt failed recently.',
    input: { ...baseInput, documents: { records: [expiredRecord], lanes: [freshLane] }, failedCompaction: { failedAtMs: 900, retryAfterMs: 1_200 } },
  },
];
