...
function decision(input: QueryTotalDiscoveryInput, output: Omit<QueryTotalDiscoveryDecision, 'queryKey'>): QueryTotalDiscoveryDecision {
  return { queryKey: input.queryKey, ...output };
}

export function planQueryTotalDiscovery(input: QueryTotalDiscoveryInput): QueryTotalDiscoveryDecision {
  ...
  const totalMatchingRecords = input.queryHasFilters ? input.querySpecificTotal : input.catalogTotal;
