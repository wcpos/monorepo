...
import { type CoverageCleanupInput, planCoverageCleanup } from './coverage-cleanup';
	...
	it('ignores same-collection records that were never covered by the cleaned lane', () => {
		const decisions = planCoverageCleanup({
			...
			records: [
				...
				{
					collection: 'orders',
					id: 'woo-order:deep-link-8',
					fresh: true,
					coveredQueryKeys: ['orders:ids:deep-link'],
				},
			],
		});
