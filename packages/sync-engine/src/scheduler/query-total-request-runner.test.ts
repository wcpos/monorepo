...
import { createQueryTotalRequestRunner, mergeQueryTotalCacheEntries, type QueryTotalDiagnostic } from './query-total-request-runner';
...
function harness(over: Partial<Parameters<typeof createQueryTotalRequestRunner>[0]> = {}) {
  ...
  const runner = createQueryTotalRequestRunner({
    ...
    publishCacheEntries: (updater) => { entries = updater(entries); },
    now: () => 1_000_000,
    isOffline: () => false,
    ...over,
  });
...
describe('createQueryTotalRequestRunner', () => {
  it('fetches, publishes the cache entry, and reports it', async () => {
    ...
    runner.request({ queryKey: 'orders:browser:status=processing:limit=50', filters: { status: 'processing' } });
    await flush();

    expect(fetchWooQueryTotal).toHaveBeenCalledTimes(1);
    expect(fetchWooQueryTotal).toHaveBeenCalledWith(expect.objectContaining({
      request: expect.objectContaining({ endpoint: 'https://demo.local/wp-json/wc-rxdb-sync/v1/orders' }),
    }));
    expect(cacheEntries()).toEqual([
      expect.objectContaining({ queryKey: 'orders:browser:status=processing:limit=50', totalMatchingRecords: 42 }),
    ]);
    expect(diagnostics.some((d) => d.message.includes('Cached Woo query total') && d.message.includes('42'))).toBe(true);
  });

  it('uses a fresh cached total without fetching', async () => {
    const { runner, diagnostics, fetchWooQueryTotal } = harness({
      readCacheEntries: () => [{ queryKey: 'orders:q', totalMatchingRecords: 7, freshUntilMs: 2_000_000, updatedAtMs: 900_000 }],
    });
    ...
    expect(diagnostics.some((d) => d.message.includes('Using cached Woo query total for orders:q: 7'))).toBe(true);
  });
  ...
  it('dedupes a request while the same queryKey is in flight', async () => {
    ...
    expect(fetchWooQueryTotal).toHaveBeenCalledTimes(1);
    expect(diagnostics.some((d) => d.message.includes('Waiting for in-flight Woo query total'))).toBe(true);
    gate.resolve(1);
    await flush();
  });
  ...
  it('skips while offline', async () => {
    ...
    expect(fetchWooQueryTotal).not.toHaveBeenCalled();
    expect(diagnostics.some((d) => d.message.includes('Skipped Woo query total for orders:q: offline'))).toBe(true);
  });

  it('a failed fetch surfaces as a diagnostic and publishes nothing', async () => {
    const { runner, diagnostics, cacheEntries } = harness({
      fetchWooQueryTotal: vi.fn(async () => { throw new Error('X-WP-Total missing'); }),
    });
    ...
    expect(cacheEntries()).toEqual([]);
    expect(diagnostics.some((d) => d.level === 'error' && d.message.includes('Woo query total failed for orders:q'))).toBe(true);
  });
});
