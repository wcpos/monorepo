...
describe('schedulerScenarios', () => {
	it('defines a constrained POS startup scenario with greedy, targeted, initial, and background requirements', () => {
		...
		expect(scenario?.requirements.map((requirement) => requirement.id)).toEqual(
		...
		expect(
			scenario?.requirements.find((requirement) => requirement.id === 'taxRates.all')?.policy.mode
		).toBe('greedy');
		expect(
			scenario?.requirements.find((requirement) => requirement.id === 'products.backgroundPoll')
				?.policy.priority
		).toBeLessThan(
			scenario?.requirements.find((requirement) => requirement.id === 'orders.deepLink')?.policy
				.priority ?? 0
		);
	});
});
