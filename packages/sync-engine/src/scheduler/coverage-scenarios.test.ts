...
describe('coverageScenarios', () => {
  ...
  it('encodes the product lane missing-record tradeoff', () => {
    const productScenario = coverageScenarios.find((scenario) => scenario.id === 'product-initial-lane-missing-record');
  ...
  it('encodes stale complete-lane evidence when current totals prove more matches exist', () => {
    const staleLaneScenario = coverageScenarios.find((scenario) => scenario.id === 'product-lane-new-total-exceeds-coverage');
