...
describe('createTaxRateSchedulerFetcher', () => {
  it('fetches the greedy tax-rate startup lane page-by-page and records complete coverage after exhaustion', async () => {
    const repository = { upsertMany: vi.fn(async (_documents: LocalTaxRateDocument[]) => undefined) };
    ...
    expect(repository.upsertMany.mock.calls[0]?.[0]).toEqual([
      expect.objectContaining({ id: 'woo-tax-rate:1', wooTaxRateId: 1, sync: expect.objectContaining({ source: 'woo-rest', partial: false }) }),
      expect.objectContaining({ id: 'woo-tax-rate:2', wooTaxRateId: 2, sync: expect.objectContaining({ source: 'woo-rest', partial: false }) }),
    ]);
    ...
    expect(first).toEqual({ taskId: 'taxRates:all:greedy', documentCount: 2, requestCount: 1, completed: false });
    expect(second).toEqual({ taskId: 'taxRates:all:greedy', documentCount: 1, requestCount: 1, completed: true });
    expect(nextRun).toEqual({ taskId: 'taxRates:all:greedy', documentCount: 1, requestCount: 1, completed: true });
  });
});
