...
function assertGreedyTask(spec: { collection: string; greedyQueryKey: string }, task: FetchTask): void {
  ...
  if (task.queryKey !== spec.greedyQueryKey) {
    throw new Error(`${spec.collection} scheduler task queryKey is not supported: ${task.queryKey}`);
  }
...
export async function httpGet(input: { fetcher?: Fetcher }, url: string, context: SchedulerFetcherContext | undefined): Promise<Response> {
...
export function createGreedyCollectionFetcher<Doc, Payload>(
  ...
  return async (task: FetchTask, context?: SchedulerFetcherContext): Promise<FetchTaskResult> => {
    ...
    if (!response.ok) throw new Error(`Woo REST ${spec.collection} request failed: ${response.status}`);
    ...
    const allStorageIds = [...(fetchedStorageIdsByTask.get(task.id) ?? []), ...documents.map(spec.storageId)];
    fetchedStorageIdsByTask.set(task.id, allStorageIds);
    const allCoverageIds = [...(fetchedCoverageIdsByTask.get(task.id) ?? []), ...documents.map(spec.coverageRecordId)];
    ...
    return spec.prunable
      ? { taskId: task.id, documentCount: documents.length, requestCount: 1, completed, prunedCount }
...
export function assertReturnedRequestedIds<Payload>(
  ...
  if (missingIds.length > 0) {
    throw new Error(`Woo REST targeted ${spec.restLabel} response missing requested ${spec.restLabel} ids: ${missingIds.join(', ')}`);
  }
}
...
async function fetchTargeted<Doc, Payload>(
  ...
  const defaultDocuments = targets.filter((target) => target.wooId === null).map(spec.defaultDocument);
  ...
  for (const idsBatch of chunk(ids, batchSize)) {
    ...
    const url = `${input.baseUrl}/${spec.endpoint}?${query.toString()}`;
    const response = await httpGet(input, url, context);
    if (!response.ok) throw new Error(`Woo REST targeted ${spec.restLabel} request failed: ${response.status}`);
    const payloads = JSON.parse(await response.text()) as Payload[];
...
async function fetchSearch<Doc, Payload>(
  ...
  while (documentCount < task.limit) {
    ...
    const url = `${input.baseUrl}/${spec.endpoint}?${query.toString()}`;
    const response = await httpGet(input, url, context);
    if (!response.ok) throw new Error(`Woo REST ${spec.restLabel} search request failed: ${response.status}`);
  ...
  return async (task: FetchTask, context?: SchedulerFetcherContext): Promise<FetchTaskResult> => {
    ...
    if (parsedSearch !== null) {
      if (parsedSearch.queryLimit !== task.limit) {
        throw new Error(`${spec.label} scheduler task limit does not match queryKey limit: ${task.queryKey}`);
      }
