import { planPersistedCoverageRetention, type PersistedCoverageDocumentSet } from './persisted-coverage-schema';
...
export type CoverageCompactionCadenceAction = 'run' | 'skip' | 'wait-for-owner' | 'take-over' | 'wait-for-failure-backoff';
...
export function planCoverageCompactionCadence(input: CoverageCompactionCadenceInput): CoverageCompactionCadenceDecision {
  ...
  const nextDueAtMs = input.lastCompactedAtMs === null ? input.nowMs : input.lastCompactedAtMs + input.intervalMs;
  ...
  if (input.lease && input.lease.ownerId !== input.tabId && input.lease.expiresAtMs <= input.nowMs) {
    return {
      action: 'take-over',
