import {
	type CoverageWriteConflictSummary,
	mergePersistedCoverageWrites,
} from './coverage-write-conflicts';
...
export const coverageWriteConflictScenarios: CoverageWriteConflictScenario[] = [
	{
		...
		description:
			'Concurrent tabs covering the same record through different lanes must not lose either lane key.',
		writes: [
			{
				records: [
					{
	...
	},
	{
		...
		description:
			'A stale record write can add coverage metadata without shortening a fresher record window.',
		writes: [
			{
				records: [
					{
	...
	},
	{
		...
		description:
			'A newer complete lane write should replace older incomplete lane metadata for the same query.',
		writes: [
			{
				records: [],
				lanes: [
	...
	},
	{
		...
		description:
			'Equal timestamps keep a deterministic complete snapshot without synthesizing expected IDs.',
		writes: [
			{
				records: [],
				lanes: [
				...
				lanes: [
					{
						...
						queryKey: 'orders:open',
						complete: false,
						expectedRecordIds: ['order-1', 'order-3'],
						freshUntilMs: 1_300,
						updatedAtMs: 1_200,
					},
				],
