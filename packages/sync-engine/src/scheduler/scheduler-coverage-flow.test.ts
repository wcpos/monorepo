...
import { runSchedulerCoverageFlow, type SchedulerCoverageFlowRequirement } from './scheduler-coverage-flow';
...
function flowRequirement(input: SchedulerCoverageFlowRequirement): SchedulerCoverageFlowRequirement {
  return input;
}
...
describe('runSchedulerCoverageFlow', () => {
  it('uses coverage decisions before scheduling remote work', async () => {
    ...
    const taxRates = requirement({
      ...
      policy: { mode: 'greedy', priority: 1000, batchSize: 10, offline: { read: 'fail-if-missing', write: 'reject' } },
    });
    ...
    const result = await runSchedulerCoverageFlow({
      ...
      requirements: [
        flowRequirement({
          ...
          coverageState: { records: [{ collection: 'customers', id: 'customer:default', fresh: true }], lanes: [] },
        }),
        flowRequirement({
    ...
    expect(result.servedLocal.map((decision) => decision.requirementId)).toEqual(['customers.default']);
    expect(result.scheduler.tasks.map((task) => task.requirementId)).toEqual(['taxRates.all']);
    expect(result.summary).toMatchObject({ servedLocal: 1, remoteTasks: 1, skipped: 0, totalRequests: 1, totalDocuments: 10 });
  });
  ...
  it('preserves user-triggered preemption after coverage filters remote requirements', async () => {
    ...
    const backgroundPoll = requirement({
      ...
      policy: { mode: 'windowed', priority: 100, batchSize: 25, offline: { read: 'serve-local', write: 'queue' } },
    });
    const fetcher = vi.fn(async (task) => ({ taskId: task.id, documentCount: task.limit, requestCount: 1, completed: true }));

    const result = await runSchedulerCoverageFlow({
      connectivity: 'online',
      requirements: [productSearch, backgroundPoll].map((item) => flowRequirement({
        requirement: item,
        coverageStrategy: 'record-and-lane',
        coverageState: { records: [], lanes: [] },
        expectedRecordIds: item.id === 'products.search.keyboard' ? ['product:keyboard'] : undefined,
      })),
      fetcher,
    });

    expect(result.scheduler.tasks.map((task) => task.requirementId)).toEqual(['products.search.keyboard', 'products.backgroundPoll']);
  });
  ...
  it('uses current query evidence before serving complete query lanes locally', async () => {
    const productSearch = requirement({
      ...
      policy: { mode: 'windowed', priority: 700, batchSize: 10, offline: { read: 'serve-local', write: 'queue' } },
    });
    const fetcher = vi.fn(async (task) => ({ taskId: task.id, documentCount: task.limit, requestCount: 1, completed: true }));
    ...
    expect(result.scheduler.tasks.map((task) => task.requirementId)).toEqual(['products.search.coffee']);
  });
  ...
  it('reports offline skips for requirements that need remote freshness', async () => {
    const deepLinkedOrder = requirement({
      ...
      ids: ['woo-order:114'],
      policy: { mode: 'on-demand', priority: 1000, batchSize: 1, offline: { read: 'fail-if-missing', write: 'queue' } },
    });
    const fetcher = vi.fn(async (task) => ({ taskId: task.id, documentCount: task.limit, requestCount: 1, completed: true }));
    ...
    expect(result.scheduler.skipped).toEqual([{ requirementId: 'orders.deepLink.114', reason: 'offline: required remote data is unavailable' }]);
