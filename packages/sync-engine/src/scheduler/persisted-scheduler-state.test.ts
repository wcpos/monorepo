...
import { planPersistedSchedulerTaskStates, type PersistedSchedulerTaskState } from './persisted-scheduler-state';
...
describe('planPersistedSchedulerTaskStates', () => {
  ...
  it('persists the wooIds numeric channel onto the claimed state (survives rehydrate)', () => {
    const result = planPersistedSchedulerTaskStates({
      ...
      ownerId: 'tab-a',
      claimForMs: 5_000,
      tasks: [task({ id: 'orders:ids:123,456:on-demand', requirementId: 'orders.deep-link', ids: ['woo-order:123', 'woo-order:456'], wooIds: [123, 456], mode: 'on-demand' })],
      existingStates: [],
    });
  ...
  it('waits when another tab owns an unexpired in-flight task', () => {
    const result = planPersistedSchedulerTaskStates({
      ...
      claimForMs: 5_000,
      tasks: [task({ id: 'orders:open:windowed', requirementId: 'orders.open' })],
      existingStates: [state({ taskId: 'orders:open:windowed', requirementId: 'orders.open', status: 'in-flight', ownerId: 'tab-b', claimedUntilMs: 3_000, attempt: 1 })],
    });

    expect(result.claimed).toEqual([]);
    expect(result.waiting).toEqual([{ taskId: 'orders:open:windowed', ownerId: 'tab-b', reason: 'active-owner' }]);
    expect(result.states[0]).toEqual(expect.objectContaining({ ownerId: 'tab-b', claimedUntilMs: 3_000, attempt: 1 }));
  });
  ...
  it('waits when the current tab already owns an unexpired in-flight task', () => {
    const result = planPersistedSchedulerTaskStates({
      ...
      claimForMs: 5_000,
      tasks: [task({ id: 'orders:open:windowed', requirementId: 'orders.open' })],
      existingStates: [state({ taskId: 'orders:open:windowed', requirementId: 'orders.open', status: 'in-flight', ownerId: 'tab-a', claimedUntilMs: 3_000, attempt: 1 })],
    });

    expect(result.claimed).toEqual([]);
    expect(result.waiting).toEqual([{ taskId: 'orders:open:windowed', ownerId: 'tab-a', reason: 'active-owner' }]);
    expect(result.states[0]).toEqual(expect.objectContaining({ ownerId: 'tab-a', claimedUntilMs: 3_000, attempt: 1 }));
  });
  ...
  it('takes over expired in-flight work after a reload or abandoned tab', () => {
    const result = planPersistedSchedulerTaskStates({
      ...
      claimForMs: 5_000,
      tasks: [task({ id: 'orders:open:windowed', requirementId: 'orders.open' })],
      existingStates: [state({ taskId: 'orders:open:windowed', requirementId: 'orders.open', status: 'in-flight', ownerId: 'tab-b', claimedUntilMs: 3_000, attempt: 2 })],
    });

    expect(result.claimed).toEqual([{ taskId: 'orders:open:windowed', reason: 'expired-owner' }]);
    expect(result.states[0]).toEqual(expect.objectContaining({ ownerId: 'tab-a', claimedUntilMs: 9_000, attempt: 3, status: 'in-flight' }));
  });
  ...
  it('honors failed task retry backoff before reclaiming', () => {
    const beforeRetry = planPersistedSchedulerTaskStates({
      ...
      claimForMs: 5_000,
      tasks: [task({ id: 'orders:open:windowed', requirementId: 'orders.open' })],
      existingStates: [state({ taskId: 'orders:open:windowed', requirementId: 'orders.open', status: 'failed', attempt: 2, retryAfterMs: 5_000 })],
    });

    expect(beforeRetry.claimed).toEqual([]);
    expect(beforeRetry.waiting).toEqual([{ taskId: 'orders:open:windowed', ownerId: null, reason: 'retry-backoff' }]);
    ...
    expect(afterRetry.states[0]).toEqual(expect.objectContaining({ status: 'in-flight', ownerId: 'tab-a', attempt: 3, retryAfterMs: null }));
  });
  ...
  it('can explicitly override failed task retry backoff for manual work', () => {
    ...
    expect(result.states[0]).toEqual(expect.objectContaining({ status: 'in-flight', ownerId: 'tab-a', attempt: 3, retryAfterMs: null }));
  });
  ...
  it('coalesces duplicate task ids before planning durable state', () => {
    ...
    expect(result.states[0]).toEqual(expect.objectContaining({ requirementId: 'orders.open', priority: 600 }));
  });
  ...
  it('keeps recently completed persisted work from being re-enqueued after reload', () => {
    const result = planPersistedSchedulerTaskStates({
      ...
      tasks: [task({ id: 'orders:open:windowed', requirementId: 'orders.open' })],
      existingStates: [state({ taskId: 'orders:open:windowed', requirementId: 'orders.open', status: 'completed', updatedAtMs: 3_000 })],
    });

    expect(result.claimed).toEqual([]);
    expect(result.completed).toEqual(['orders:open:windowed']);
    expect(result.states[0]).toEqual(expect.objectContaining({ status: 'completed', updatedAtMs: 3_000 }));
  });
  ...
  it('reclaims completed persisted work after the dedupe window expires', () => {
    const result = planPersistedSchedulerTaskStates({
      ...
      completedDedupeForMs: 2_000,
      tasks: [task({ id: 'orders:open:windowed', requirementId: 'orders.open' })],
      existingStates: [state({ taskId: 'orders:open:windowed', requirementId: 'orders.open', status: 'completed', updatedAtMs: 3_000, attempt: 1 })],
    });

    expect(result.completed).toEqual([]);
    expect(result.claimed).toEqual([{ taskId: 'orders:open:windowed', reason: 'queued' }]);
    expect(result.states[0]).toEqual(expect.objectContaining({ status: 'in-flight', ownerId: 'tab-a', claimedUntilMs: 11_000, attempt: 2 }));
  });
});
