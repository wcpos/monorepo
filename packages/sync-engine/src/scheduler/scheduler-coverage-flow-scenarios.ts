...
function coverage(
	requirementInput: ReplicationRequirement,
	options: Omit<SchedulerCoverageFlowRequirement, 'requirement' | 'coverageStrategy'> &
		Partial<Pick<SchedulerCoverageFlowRequirement, 'coverageStrategy'>>
...
export const schedulerCoverageFlowScenarios: SchedulerCoverageFlowScenario[] = [
	{
		...
		description:
			'Checks startup dependencies against local coverage before scheduling only missing remote work.',
		connectivity: 'online',
		requirements: [
			coverage(defaultCustomer, {
				coverageState: {
					records: [{ collection: 'customers', id: 'customer:default', fresh: true }],
					lanes: [],
				},
			}),
			coverage(deepLinkedOrder, {
				coverageState: {
	...
	},
	{
		...
		description:
			'Lets a query-triggered product search jump ahead of lower-priority background polling after coverage filters both as remote work.',
		connectivity: 'online',
		requirements: [
			coverage(productSearch, {
	...
	},
	{
		...
		description:
			'Serves a fresh targeted order locally and avoids scheduling a remote fetch for that deep-link requirement.',
		connectivity: 'online',
		requirements: [
			coverage(deepLinkedOrder, {
				coverageState: {
	...
	},
	{
		...
		description:
			'Serves covered local data while reporting missing fail-if-missing requirements as offline skips.',
	...
	},
	{
		...
		description:
			'Requires both a complete fresh lane and fresh expected records before serving a query locally.',
		connectivity: 'online',
		requirements: [
			coverage(productSearch, {
				coverageState: {
...
export function runSchedulerCoverageFlowScenario(
	scenario: SchedulerCoverageFlowScenario
): Promise<SchedulerCoverageFlowResult> {
	return runSchedulerCoverageFlow({
...
export async function summarizeSchedulerCoverageFlowScenario(
	scenario: SchedulerCoverageFlowScenario
): Promise<string> {
