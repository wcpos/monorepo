...
describe('queryRequirementLibraryScenarios', () => {
  ...
  it('coalesces product query declarations before scheduler coverage flow runs', async () => {
    const scenario = queryRequirementLibraryScenarios.find((item) => item.id === 'query-library-product-search-coalescing');
  ...
  it('keeps stale complete-lane query evidence remote after query-library declaration coalescing', async () => {
    const scenario = queryRequirementLibraryScenarios.find((item) => item.id === 'query-library-current-total-evidence');
  ...
  it('summarizes startup declarations across local coverage and remote scheduler work', async () => {
    const scenario = queryRequirementLibraryScenarios.find((item) => item.id === 'query-library-startup-declarations');

    expect(scenario).toBeDefined();
    await expect(summarizeQueryRequirementLibraryScenario(scenario!))
      .resolves.toBe('declarations 3 · coalesced 3 · served local 2 · remote tasks 1 · skipped 0 · requests 1 · docs 10');
  });
});
