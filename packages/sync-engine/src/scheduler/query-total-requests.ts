...
function cacheStatus(entry: QueryTotalCacheEntry | undefined, nowMs: number): QueryTotalCacheStatus {
...
export function planQueryTotalRequest(input: QueryTotalRequestPlannerInput): QueryTotalRequestDecision {
  ...
  return {
    ...
    reason: 'filtered exact-limit query needs a fresh Woo total and no equivalent request is in flight',
    totalMatchingRecords: null,
    cacheStatus: status,
    request: {
      queryKey: discovery.queryKey,
