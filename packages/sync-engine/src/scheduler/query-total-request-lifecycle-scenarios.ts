import {
	...
	type QueryTotalRequestLifecycleDecision,
	type QueryTotalRequestLifecycleInput,
...
export type QueryTotalRequestLifecycleScenarioSummary = Pick<
...
export const queryTotalRequestLifecycleScenarios: QueryTotalRequestLifecycleScenario[] = [
	{
		...
		description:
			'Fresh durable query-total evidence wins before any request ownership or retry state is considered.',
		input: {
			...baseInput,
			cacheEntries: [
				{
	...
	},
	{
		...
		description:
			'A second tab should not duplicate a query-total probe while another owner lease is still active.',
		input: {
			...baseInput,
			requestStates: [
				requestState({ status: 'in-flight', ownerId: 'tab-b', claimedUntilMs: 2_000 }),
			],
		},
	},
	{
		...
		description:
			'An expired owner lease can be claimed by the current tab and retried as the next attempt.',
		input: {
			...baseInput,
			requestStates: [
				requestState({ status: 'in-flight', ownerId: 'tab-b', claimedUntilMs: 1_400 }),
			],
		},
	},
	{
		...
		description:
			'A failed request whose retry-after time has not elapsed remains unowned but unavailable.',
		input: {
			...baseInput,
			requestStates: [
				requestState({
	...
	},
	{
		...
		description:
			'Offline tabs do not claim durable query-total work because they cannot contact Woo.',
	...
	},
	{
		...
		description:
			'Online exact-limit filtered queries without cache or durable lifecycle state can be claimed immediately.',
		input: baseInput,
	},
];
...
export function summarizeQueryTotalRequestLifecycleScenarios(): QueryTotalRequestLifecycleScenarioSummary[] {
	return queryTotalRequestLifecycleScenarios.map((scenario) => {
		...
		return {
			...
			requestEndpoint: decision.request
				? `${decision.request.method} ${decision.request.endpoint}`
				: null,
