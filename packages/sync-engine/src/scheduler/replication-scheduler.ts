...
function taskId(requirement: ReplicationRequirement): string {
	const idsPart =
		requirement.kind === 'targeted-records' && requirement.ids?.length
...
function shouldSkipOffline(
	...
	if (requirement.policy.offline.read === 'fail-if-missing')
		return 'offline: required remote data is unavailable';
...
export async function runSchedulerScenario(
	input: SchedulerScenarioInput
	...
	for (const task of tasks) {
		...
		const maxRequests =
			input.requirements.find((requirement) => requirement.id === task.requirementId)?.policy
				.maxRequests ?? 100;
		let completed = false;
		let attempts = 0;
