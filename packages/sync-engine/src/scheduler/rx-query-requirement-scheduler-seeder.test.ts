...
import type { SchedulerTaskSeederRepository } from '@wcpos/sync-engine/testing';
...
function declaration(
	overrides: Partial<QueryRequirementDeclaration> &
		Pick<QueryRequirementDeclaration, 'id' | 'collection' | 'queryKey'>
...
describe('seedSchedulerTasksFromQueryDeclarations', () => {
	it('seeds persisted scheduler tasks only for declarations that are not fresh in coverage', async () => {
			...
			records: [
				{
					collection: 'products',
					id: 'woo-product:1',
					coveredQueryKeys: ['products:search:keyboard'],
					freshUntilMs: 20_000,
					updatedAtMs: 9_000,
				},
			],
		...
		expect(repo.readForTaskIds).toHaveBeenCalledWith([
			'customers:customers:search=alex:limit=25:windowed',
		]);
		...
		expect(result.plannedTasks.map((task) => task.requirementId)).toEqual([
			'customers.search.alex',
		]);
		expect(
			result.coverageDecisions.map((decision) => [decision.requirementId, decision.action])
		).toEqual([
			['products.search.keyboard', 'serve-local'],
			['customers.search.alex', 'fetch-remote'],
		]);
		expect(result.seedResult).toEqual({
			inserted: 1,
			requeued: 0,
			skippedActive: 0,
			skippedCompleted: 0,
	...
	it('fetches when a complete fresh lane expects records that are not fresh locally', async () => {
		...
		expect(result.coverageDecisions).toEqual([
			expect.objectContaining({
				action: 'fetch-remote',
	...
	it('honors offline scheduler planning before seeding tasks', async () => {
		...
		expect(result.skipped).toEqual([
			{ requirementId: 'products.search.keyboard', reason: 'offline: served local data only' },
		]);
		expect(result.seedResult).toEqual({
			inserted: 0,
			requeued: 0,
