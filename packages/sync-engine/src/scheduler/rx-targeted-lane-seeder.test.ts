...
describe('seedTargetedLane', () => {
  ...
  it('normalizes (dedup + ascending sort) and builds the descriptor task shape', async () => {
    ...
    await expect(seedTargetedLane(VARIATIONS_LANE, {
  ...
  it('defaults priority, batch size, and dedupe window from the descriptor', async () => {
    ...
    await seedTargetedLane(VARIATIONS_LANE, {
      getRepository: mocks.getRepository, ids: [7], nowMs: 1_000 });
  ...
  it('forwards a non-positive completedDedupeForMs unchanged (disables completed-dedupe)', async () => {
    ...
    await seedTargetedLane(VARIATIONS_LANE, {
      getRepository: mocks.getRepository, ids: [7], completedDedupeForMs: 0, nowMs: 1_000 });
  ...
  it('defaults nowMs to a real clock when omitted', async () => {
    ...
    await seedTargetedLane(VARIATIONS_LANE, {
      getRepository: mocks.getRepository, ids: [7] });
  ...
  it('rejects empty and invalid ids with the descriptor label before queuing work', async () => {
    ...
    await expect(seedTargetedLane(VARIATIONS_LANE, {
      getRepository: mocks.getRepository, ids: [] })).rejects.toThrow(
      'Targeted variation scheduler task requires at least one variation id',
    );
    await expect(seedTargetedLane(VARIATIONS_LANE, {
      getRepository: mocks.getRepository, ids: [0] })).rejects.toThrow(
      'Targeted variation scheduler task requires positive integer variation ids',
    );
    await expect(seedTargetedLane(VARIATIONS_LANE, {
      getRepository: mocks.getRepository, ids: [1.5] })).rejects.toThrow(
      'positive integer variation ids',
    );
    await expect(seedTargetedLane(VARIATIONS_LANE, {
      getRepository: mocks.getRepository, ids: [5], batchSize: 0 })).rejects.toThrow(
      'Targeted variation scheduler task batch size must be a positive integer',
    );
  ...
  it('splits by batch size into ordered chunks covering every id', async () => {
    ...
    await seedTargetedLane(VARIATIONS_LANE, {
      getRepository: mocks.getRepository, ids: [5, 4, 3, 2, 1], batchSize: 2, nowMs: 1_000 });
    ...
    for (const task of seederInput.tasks as Array<{ limit: number }>) {
  ...
  it('reproduces the product and order lane shapes from descriptors', async () => {
    ...
    fakeWiring();
    await seedTargetedLane(productLane, { getRepository: mocks.getRepository, ids: [456, 123, 123], priority: 950, batchSize: 50, completedDedupeForMs: 30_000, nowMs: 12_000 });
    ...
    await seedTargetedLane(orderLane, { getRepository: mocks.getRepository, ids: [456, 123, 123], priority: 950, batchSize: 50, completedDedupeForMs: 30_000, nowMs: 12_000 });
    expect(mocks.seedPersistedSchedulerTasks.mock.calls[0][0].tasks).toEqual([{
