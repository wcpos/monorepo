...
export type PersistedSchedulerTaskRunnerRepository = {
  ...
  markFailed(expectedState: PersistedSchedulerTaskState, failedState: PersistedSchedulerTaskState): Promise<boolean>;
};
...
function completedState(state: PersistedSchedulerTaskState, completedAtMs: number): PersistedSchedulerTaskState {
  return {
    ...state,
    status: 'completed',
    ownerId: null,
 ...
 */
function requeuedState(state: PersistedSchedulerTaskState, requeuedAtMs: number): PersistedSchedulerTaskState {
  return {
    ...state,
    status: 'queued',
    ownerId: null,
...
function recordFetchResult(result: PersistedSchedulerTaskRunnerResult, fetchResult: FetchTaskResult): void {
  result.totalDocuments += fetchResult.documentCount;
  result.totalRequests += fetchResult.requestCount;
}

function maxRequestsForTask(input: PersistedSchedulerTaskRunnerInput, task: FetchTask, state: PersistedSchedulerTaskState): number {
  return input.maxRequestsForTask?.(task, state) ?? input.maxRequestsPerTask ?? 100;
}

function hasActiveCurrentOwner(state: PersistedSchedulerTaskState, input: PersistedSchedulerTaskRunnerInput, nowMs: number): boolean {
...
function renewState(state: PersistedSchedulerTaskState, input: PersistedSchedulerTaskRunnerInput, renewedAtMs: number): PersistedSchedulerTaskState {
  return {
    ...state,
...
export async function runPersistedSchedulerTasks(input: PersistedSchedulerTaskRunnerInput): Promise<PersistedSchedulerTaskRunnerResult> {
  ...
  taskStates.sort((left, right) => right.priority - left.priority || left.taskId.localeCompare(right.taskId));
  ...
  for (const runnableState of taskStates) {
    ...
    try {
      ...
      const failed = await input.repository.markFailed(activeState, failedState(activeState, input, failedAtMs));
      if (!failed) {
