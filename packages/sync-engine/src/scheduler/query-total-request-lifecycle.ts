...
function findLatestState(
	...
	queryKey: string
): QueryTotalRequestState | undefined {
	return states
...
export function planQueryTotalRequestLifecycle(
	input: QueryTotalRequestLifecycleInput
): QueryTotalRequestLifecycleDecision {
	const baseDecision = planQueryTotalRequest({
		discovery: input.discovery,
	...
	if (existingState?.status === 'in-flight') {
		if (existingState.claimedUntilMs !== null && existingState.claimedUntilMs > input.nowMs) {
			return {
				...
				reason:
					existingState.ownerId === input.ownerId
				...
				request: null,
				ownerStatus:
					existingState.ownerId === input.ownerId ? 'owned-by-current-tab' : 'owned-by-other-tab',
