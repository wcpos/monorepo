import {
	...
	type QueryTotalDiscoveryInput,
} from './query-total-discovery';
...
export type QueryTotalDiscoveryScenarioSummary = Pick<
...
export const queryTotalDiscoveryScenarios: QueryTotalDiscoveryScenario[] = [
	{
		...
		description:
			'A settled filtered result below the active limit proves all matching local records are visible.',
		input: {
			...baseInput,
			queryKey: 'orders:browser:status=processing:search=:limit=50',
			returnedRecordCount: 12,
	...
	},
	{
		...
		description:
			'An unfiltered exact-limit result can use the trusted Woo catalog total when it matches the visible count.',
		input: {
			...baseInput,
	...
	},
	{
		...
		description:
			'A filtered exact-limit result needs a query-specific Woo total before expected ids are trusted.',
		input: {
			...baseInput,
			queryKey: 'orders:browser:status=processing:search=:limit=50',
			returnedRecordCount: 50,
	...
	},
	{
		...
		description:
			'A query-specific total equal to the visible count proves the filtered lane is complete.',
		input: {
			...baseInput,
			queryKey: 'orders:browser:status=processing:search=:limit=50',
			returnedRecordCount: 50,
			queryHasFilters: true,
			querySpecificTotal: 50,
		},
	},
	{
		...
		description:
			'A query-specific total larger than the visible count proves more matches exist and the lane stays incomplete.',
		input: {
			...baseInput,
			queryKey: 'orders:browser:status=processing:search=:limit=50',
			returnedRecordCount: 50,
