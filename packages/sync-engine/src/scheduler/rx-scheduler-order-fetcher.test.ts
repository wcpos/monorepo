...
import { normalizeCheckpoint, type SyncCheckpoint, type PullResponse } from '@woo-rxdb-lab/shared';
...
function customPullDoc(wooId: number): PullResponse['documents'][number] {
  return {
    id: uuidFor(wooId),
    wooOrderId: wooId,
    payload: { id: wooId, date_modified_gmt: '2026-05-20T10:00:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(wooId) }] },
    sync: { revision: '', partial: false, source: 'custom-pull', checkpoint: normalizeCheckpoint({ orderId: wooId }) },
...
describe('createOrdersSchedulerFetcher', () => {
  it('runs one custom-pull batch for an order scheduler task and reports whether more batches remain', async () => {
    ...
    const fetcher = vi.fn(async () => response({ documents, checkpoint: nextCheckpoint, hasMore: true }));
    ...
    expect(fetcher).toHaveBeenCalledWith('http://wcpos.local/wp-json/wc-rxdb-sync/v1/orders/pull?limit=25&updated_at_gmt=2026-05-20T10%3A00%3A00.000Z&order_id=10&sequence=10&include_deletes=true');
  ...
  it('client-assembles custom-pull documents from the payload, ignoring the server-built envelope id and wooOrderId', async () => {
    ...
    const serverDoc = {
      id: 'woo-order:11',
      wooOrderId: 999,
      payload: { id: 11, date_modified_gmt: '2026-05-20T10:00:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(11) }] },
      sync: { revision: '', partial: false, source: 'custom-pull', checkpoint: normalizeCheckpoint({ orderId: 11 }) },
      local: { dirty: false, pendingMutationIds: [] },
    } as PullResponse['documents'][number];
    const upserted: PullResponse['documents'] = [];
    const repository = { upsertMany: vi.fn(async (docs: PullResponse['documents']) => { upserted.push(...docs); }) };
    const checkpointStore = {
      readCustomPullCheckpoint: vi.fn(async () => checkpoint),
      writeCustomPullCheckpoint: vi.fn(async () => undefined),
    };
    const fetcher = vi.fn(async () => response({ documents: [serverDoc], checkpoint: nextCheckpoint, hasMore: false }));
    const schedulerFetcher = createOrdersSchedulerFetcher({
      baseUrl: 'http://wcpos.local/wp-json/wc-rxdb-sync/v1',
      repository,
      checkpointStore,
  ...
  it('marks the custom-pull lane complete when the final greedy batch exhausts the remote orders catalog', async () => {
    ...
    const fetcher = vi.fn(async () => response({ documents, checkpoint: nextCheckpoint, hasMore: false }));
    const schedulerFetcher = createOrdersSchedulerFetcher({
      baseUrl: 'http://wcpos.local/wp-json/wc-rxdb-sync/v1',
      repository,
      checkpointStore,
    ...
    expect(coverageRepository.recordQueryResult).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('does not mark a greedy terminal custom-pull batch complete when it starts from an advanced checkpoint', async () => {
    ...
    expect(coverageRepository.recordCumulativeQueryResult).toHaveBeenCalledWith(expect.objectContaining({
      queryKey: 'orders:custom-pull',
      complete: false,
    }));
  });
  ...
  it('does not advance the custom-pull checkpoint when coverage write fails after storing batch records', async () => {
    ...
    const coverageRepository = {
      recordQueryResult: vi.fn(async () => { throw new Error('coverage unavailable'); }),
    };
    const fetcher = vi.fn(async () => response({ documents, checkpoint: nextCheckpoint, hasMore: false }));
    const schedulerFetcher = createOrdersSchedulerFetcher({
      baseUrl: 'http://wcpos.local/wp-json/wc-rxdb-sync/v1',
    ...
    await expect(schedulerFetcher(orderTask({ mode: 'greedy' }))).rejects.toThrow('coverage unavailable');
  ...
  it('fetches targeted order tasks through Woo REST include and stores full order documents', async () => {
    ...
    const fetcher = vi.fn(async () => response([
      { id: 123, date_modified_gmt: '2026-05-20T10:10:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(123) }] },
      { id: 456, date_modified_gmt: '2026-05-20T10:11:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(456) }] },
    ]));
    const schedulerFetcher = createOrdersSchedulerFetcher({
      baseUrl: 'http://wcpos.local/wp-json/wc-rxdb-sync/v1',
      repository,
    ...
    expect(fetcher).toHaveBeenCalledWith('http://wcpos.local/wp-json/wc-rxdb-sync/v1/orders?include=123%2C456&per_page=2&orderby=include');
    expect(repository.upsertMany).toHaveBeenCalledWith([
      {
        id: uuidFor(123),
        wooOrderId: 123,
        payload: { id: 123, date_modified_gmt: '2026-05-20T10:10:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(123) }] },
        sync: {
      ...
      },
      {
        id: uuidFor(456),
        wooOrderId: 456,
        payload: { id: 456, date_modified_gmt: '2026-05-20T10:11:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(456) }] },
        sync: {
  ...
  it('fails a targeted order task that is missing its wooIds channel (contract error, no reverse-parse)', async () => {
    ...
    await expect(schedulerFetcher(orderTask({
      id: 'orders:ids:123,456:on-demand',
      requirementId: 'orders.deep-link',
      queryKey: 'orders:ids:123,456',
      ids: ['woo-order:123', 'woo-order:456'],
      limit: 2,
      mode: 'on-demand',
    }))).rejects.toThrow('Targeted order scheduler task is missing its wooIds channel: orders:ids:123,456:on-demand');
  ...
  it('does NOT overwrite a targeted order that has queued local mutations, but keeps it covered', async () => {
    ...
    expect(repository.upsertMany).toHaveBeenCalledWith([expect.objectContaining({ id: uuidFor(456) })]);
    expect(repository.upsertMany).not.toHaveBeenCalledWith(expect.arrayContaining([expect.objectContaining({ id: uuidFor(123) })]));
    // ...but 123 stays in coverage so the window isn't reported incomplete and re-pulled.
    expect(coverageRepository.recordQueryResult).toHaveBeenCalledWith(
      expect.objectContaining({ records: expect.arrayContaining([{ id: 'woo-order:123' }, { id: 'woo-order:456' }]) }),
    );
  });
  ...
  it('re-reads pending mutations per batch so an order queued mid-pull is not overwritten', async () => {
    ...
    const fetcher = vi
      .fn()
      .mockResolvedValueOnce(response([{ id: 123, date_modified_gmt: '2026-05-20T10:10:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(123) }] }]))
      .mockResolvedValueOnce(response([{ id: 456, date_modified_gmt: '2026-05-20T10:11:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(456) }] }]));
    ...
    expect(repository.upsertMany).toHaveBeenNthCalledWith(1, [expect.objectContaining({ id: uuidFor(123) })]);
  ...
  it('does NOT overwrite a browser-window order that has queued local mutations (numeric pending id)', async () => {
    const repository = { upsertMany: vi.fn(async () => undefined) };
    const fetcher = vi.fn(async () => response([
      { id: 123, status: 'processing', date_modified_gmt: '2026-05-20T10:10:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(123) }] },
      { id: 456, status: 'processing', date_modified_gmt: '2026-05-20T10:11:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(456) }] },
    ]));
    const schedulerFetcher = createOrdersSchedulerFetcher({
      baseUrl: 'http://wcpos.local/wp-json/wc-rxdb-sync/v1',
      repository,
    ...
    expect(repository.upsertMany).toHaveBeenCalledWith([expect.objectContaining({ id: uuidFor(123) })]);
    expect(repository.upsertMany).not.toHaveBeenCalledWith(expect.arrayContaining([expect.objectContaining({ id: uuidFor(456) })]));
  });
  ...
  it('passes scheduler abort signals to Woo REST order requests', async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith('http://wcpos.local/wp-json/wc-rxdb-sync/v1/orders?include=123&per_page=1&orderby=include', {
      signal: abortController.signal,
    });
  });
  ...
  it('chunks targeted order tasks at Woo REST page-size boundaries before marking them complete', async () => {
    ...
    const firstPayloads = Array.from({ length: 100 }, (_, index) => ({ id: index + 1, date_modified_gmt: `2026-05-20T10:${String(index % 60).padStart(2, '0')}:00`, meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(index + 1) }] }));
    const secondPayloads = [{ id: 101, date_modified_gmt: '2026-05-20T11:41:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(101) }] }];
    const repository = {
    ...
    expect(firstUrl).toContain(`include=${Array.from({ length: 100 }, (_, index) => index + 1).join('%2C')}`);
    expect(firstUrl).toContain('per_page=100');
    expect(fetcher.mock.calls[1][0]).toBe('http://wcpos.local/wp-json/wc-rxdb-sync/v1/orders?include=101&per_page=1&orderby=include');
  ...
  it('honors the targeted task limit when it is smaller than the Woo REST page-size cap', async () => {
    ...
    const fetcher = vi.fn()
      .mockResolvedValueOnce(response([{ id: 1, date_modified_gmt: '2026-05-20T10:01:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(1) }] }, { id: 2, date_modified_gmt: '2026-05-20T10:02:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(2) }] }]))
      .mockResolvedValueOnce(response([{ id: 3, date_modified_gmt: '2026-05-20T10:03:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(3) }] }, { id: 4, date_modified_gmt: '2026-05-20T10:04:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(4) }] }]))
      .mockResolvedValueOnce(response([{ id: 5, date_modified_gmt: '2026-05-20T10:05:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(5) }] }]));
    const schedulerFetcher = createOrdersSchedulerFetcher({
      baseUrl: 'http://wcpos.local/wp-json/wc-rxdb-sync/v1',
      repository,
      checkpointStore: {
  ...
  it('fails targeted order tasks when Woo omits a requested order id', async () => {
    ...
    const fetcher = vi.fn(async () => response([{ id: 123, date_modified_gmt: '2026-05-20T10:10:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(123) }] }]));
    const schedulerFetcher = createOrdersSchedulerFetcher({
      baseUrl: 'http://wcpos.local/wp-json/wc-rxdb-sync/v1',
      repository,
      checkpointStore: {
  ...
  it('fetches status-only browser order query tasks through Woo REST filter descriptors', async () => {
    ...
    const fetcher = vi.fn(async () => response([
      { id: 789, status: 'processing', date_modified_gmt: '2026-05-20T10:12:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(789) }] },
    ]));
    const schedulerFetcher = createOrdersSchedulerFetcher({
      baseUrl: 'http://wcpos.local/wp-json/wc-rxdb-sync/v1',
      repository,
    ...
    expect(fetcher).toHaveBeenCalledWith('http://wcpos.local/wp-json/wc-rxdb-sync/v1/orders?status=processing&per_page=25&page=1&orderby=id&order=desc');
    expect(repository.upsertMany).toHaveBeenCalledWith([expect.objectContaining({
      id: uuidFor(789),
      wooOrderId: 789,
      payload: { id: 789, status: 'processing', date_modified_gmt: '2026-05-20T10:12:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(789) }] },
  ...
  it('records capped browser order query coverage as incomplete without exhaustion evidence', async () => {
    ...
    const page = Array.from({ length: 50 }, (_, index) => ({ id: 1_000 - index, status: 'processing', date_modified_gmt: '2026-05-20T10:12:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(1_000 - index) }] }));
  ...
  it('records partially sliced browser order pages as incomplete when Woo returns rows beyond the descriptor limit', async () => {
    ...
    const firstPage = Array.from({ length: 100 }, (_, index) => ({ id: 1_000 - index, status: 'processing', date_modified_gmt: '2026-05-20T10:12:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(1_000 - index) }] }));
    const secondPage = Array.from({ length: 75 }, (_, index) => ({ id: 900 - index, status: 'processing', date_modified_gmt: '2026-05-20T10:13:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(900 - index) }] }));
    const fetcher = vi.fn()
  ...
  it('records browser order query coverage across fetched Woo REST pages', async () => {
    ...
    const firstPage = Array.from({ length: 100 }, (_, index) => ({ id: 1_000 - index, status: 'processing', date_modified_gmt: '2026-05-20T10:12:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(1_000 - index) }] }));
    const secondPage = Array.from({ length: 2 }, (_, index) => ({ id: 900 - index, status: 'processing', date_modified_gmt: '2026-05-20T10:13:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(900 - index) }] }));
    const fetcher = vi.fn()
      .mockResolvedValueOnce(response(firstPage))
      .mockResolvedValueOnce(response(secondPage));
    const schedulerFetcher = createOrdersSchedulerFetcher({
  ...
  it('fetches bounded multi-page status-only browser order query descriptors', async () => {
    ...
    const firstPage = Array.from({ length: 100 }, (_, index) => ({ id: 1_000 - index, status: 'processing', date_modified_gmt: '2026-05-20T10:12:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(1_000 - index) }] }));
    const secondPage = Array.from({ length: 50 }, (_, index) => ({ id: 900 - index, status: 'processing', date_modified_gmt: '2026-05-20T10:13:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(900 - index) }] }));
    const fetcher = vi.fn()
      .mockResolvedValueOnce(response(firstPage))
      .mockResolvedValueOnce(response(secondPage));
    const schedulerFetcher = createOrdersSchedulerFetcher({
    ...
    expect(fetcher).toHaveBeenCalledTimes(2);
    expect(fetcher).toHaveBeenNthCalledWith(1, 'http://wcpos.local/wp-json/wc-rxdb-sync/v1/orders?status=processing&per_page=100&page=1&orderby=id&order=desc');
    expect(fetcher).toHaveBeenNthCalledWith(2, 'http://wcpos.local/wp-json/wc-rxdb-sync/v1/orders?status=processing&per_page=100&page=2&orderby=id&order=desc');
    expect(repository.upsertMany).toHaveBeenCalledTimes(2);
    expect(result).toEqual({
      taskId: 'orders:browser:processing-large:windowed',
      documentCount: 150,
  ...
  it('fetches browser order search descriptors through Woo REST search requests', async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith('http://wcpos.local/wp-json/wc-rxdb-sync/v1/orders?status=processing&search=hat&per_page=50&page=1&orderby=id&order=desc');
    expect(repository.upsertMany).toHaveBeenCalledWith([expect.objectContaining({
      id: uuidFor(789),
      wooOrderId: 789,
      payload: { id: 789, status: 'processing', date_modified_gmt: '2026-05-20T10:12:00', meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(789) }] },
  ...
  it('fails a greedy custom-pull task when hasMore stays true without checkpoint progress', async () => {
    ...
    await schedulerFetcher(orderTask({ mode: 'greedy' }));
    await schedulerFetcher(orderTask({ mode: 'greedy' }));
    await expect(schedulerFetcher(orderTask({ mode: 'greedy' }))).rejects.toThrow('Custom pull stalled: checkpoint did not advance while hasMore=true');
  });
