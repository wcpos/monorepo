...
import type { BuildCoverageDocumentsFromQueryResultInput, BuildCumulativeCoverageDocumentsFromQueryResultInput } from './query-coverage-writes';
import type { SchedulerFetcher, SchedulerFetcherContext } from './replication-scheduler';
import { shouldApplyPulledDocument, syncCustomPullBatchIntoRepository, type CustomPullCheckpointStore, type CustomPullRepository } from '@woo-rxdb-lab/sync-core';
import { checkpointInstantMs, normalizeCheckpoint, orderDocumentId, type OrderDocument, type SyncCheckpoint, type WooOrderPayload } from '@woo-rxdb-lab/shared';
import { materializeLocalOnly } from '../materialization/record-materialization';
import { parseOrderBrowserSchedulerDescriptor, WOO_REST_MAX_PER_PAGE } from './order-browser-scheduler-descriptor';
...
export type OrdersSchedulerCoverageRepository = {
  ...
  recordRecords?(input: Omit<BuildCoverageDocumentsFromQueryResultInput, 'complete'>): Promise<void>;
  recordCumulativeQueryResult?(input: BuildCumulativeCoverageDocumentsFromQueryResultInput): Promise<void>;
  readLocalLaneCoverage?(collection: string, queryKey: string, nowMs: number): Promise<{ complete: boolean; fresh: boolean } | null>;
};
...
function browserOrderQueryDescriptor(task: FetchTask): { status: string; search: string; limit: number; perPage: number } | null {
  const decision = parseOrderBrowserSchedulerDescriptor(task.queryKey);
  if (!decision) return null;
  if ('skipReason' in decision) {
    throw new Error(decision.skipReason === 'descriptor is not supported'
...
function orderCoverageInput(input: OrdersSchedulerFetcherInput, task: FetchTask, documentIds: string[], complete: boolean): BuildCoverageDocumentsFromQueryResultInput {
  return {
    collection: 'orders',
...
async function recordOrderFetchedRecords(input: OrdersSchedulerFetcherInput, task: FetchTask, documentIds: string[]): Promise<void> {
  if (!input.coverageRepository?.recordRecords) return;
  const { complete: _complete, ...coverageInput } = orderCoverageInput(input, task, documentIds, false);
...
async function writeFullBaselineMarker(input: OrdersSchedulerFetcherInput, task: FetchTask): Promise<void> {
...
async function clearFullBaselineMarker(input: OrdersSchedulerFetcherInput, task: FetchTask): Promise<void> {
  if (!input.coverageRepository) return;
  await input.coverageRepository.recordQueryResult({
    collection: 'orders',
    queryKey: fullBaselineMarkerQueryKey(task),
...
async function hasFullBaselineMarker(input: OrdersSchedulerFetcherInput, task: FetchTask): Promise<boolean> {
  if (!input.coverageRepository?.readLocalLaneCoverage) return false;
  const lane = await input.coverageRepository.readLocalLaneCoverage('orders', fullBaselineMarkerQueryKey(task), coverageNowMs(input));
  return Boolean(lane?.complete && lane.fresh);
}

async function recordOrderFetchCoverage(input: OrdersSchedulerFetcherInput, task: FetchTask, documentIds: string[], complete: boolean): Promise<void> {
  if (!input.coverageRepository) return;
  await input.coverageRepository.recordQueryResult(orderCoverageInput(input, task, documentIds, complete));
}

async function recordCumulativeOrderFetchCoverage(input: OrdersSchedulerFetcherInput, task: FetchTask, documentIds: string[], complete: boolean, resetCumulativeExpectedIds: boolean): Promise<void> {
  if (!input.coverageRepository) return;
  const coverageInput = orderCoverageInput(input, task, documentIds, complete && Boolean(input.coverageRepository.recordCumulativeQueryResult));
  if (!input.coverageRepository.recordCumulativeQueryResult) {
    await input.coverageRepository.recordQueryResult(coverageInput);
    return;
  }
  await input.coverageRepository.recordCumulativeQueryResult({ ...coverageInput, resetCumulativeExpectedIds });
...
async function fetchBrowserOrderQuery(input: OrdersSchedulerFetcherInput, task: FetchTask, descriptor: { status: string; search: string; limit: number; perPage: number }, context?: SchedulerFetcherContext): Promise<FetchTaskResult> {
  if (task.collection !== 'orders') {
    throw new Error(`Orders scheduler fetcher cannot run ${task.collection} tasks`);
  }
  ...
  while (documentCount < descriptor.limit) {
    ...
    const applicable = pending ? documents.filter((document) => shouldApplyPulledDocument(document, pending)) : documents;
...
async function fetchTargetedOrders(input: OrdersSchedulerFetcherInput, task: FetchTask, context?: SchedulerFetcherContext): Promise<FetchTaskResult> {
  if (task.collection !== 'orders') {
    throw new Error(`Orders scheduler fetcher cannot run ${task.collection} tasks`);
  }
  ...
  for (const idsBatch of chunk(ids, batchSize)) {
    ...
    const payloads = JSON.parse(await response.text()) as WooOrderPayload[];
    assertReturnedRequestedIds({ restLabel: 'order', payloadWooId: (payload: WooOrderPayload) => Number(payload.id) }, idsBatch, payloads);
...
function checkpointAdvanced(previous: SyncCheckpoint, next: SyncCheckpoint): boolean {
  ...
  return checkpointInstantMs(next.updatedAtGmt) !== checkpointInstantMs(previous.updatedAtGmt)
...
function isInitialCheckpoint(checkpoint: SyncCheckpoint): boolean {
  ...
  return checkpoint.updatedAtGmt === initialCheckpoint.updatedAtGmt
...
export function createOrdersSchedulerFetcher(input: OrdersSchedulerFetcherInput): SchedulerFetcher {
  ...
  return async (task: FetchTask, context?: SchedulerFetcherContext): Promise<FetchTaskResult> => {
    ...
    const greedyTaskStartedAtBaseline = task.mode === 'greedy' && isInitialCheckpoint(previousCheckpoint);
    if (greedyTaskStartedAtBaseline) {
      fullBaselineGreedyTasks.add(task.id);
      await writeFullBaselineMarker(input, task);
    }
    const canCompleteAllOrdersLane = task.mode === 'greedy' && (fullBaselineGreedyTasks.has(task.id) || await hasFullBaselineMarker(input, task));
    const result = await syncCustomPullBatchIntoRepository({
      ...
      ...(input.pendingMutationOrderIds
        ...
        : {}),
      afterUpsert: (documents, result) => recordCumulativeOrderFetchCoverage(input, task, documents.map(orderCoverageRecordId), canCompleteAllOrdersLane && !result.hasMore, greedyTaskStartedAtBaseline),
    });
    if (!result.hasMore) {
      fullBaselineGreedyTasks.delete(task.id);
    }
    stalledBatches = result.hasMore && !checkpointAdvanced(previousCheckpoint, result.checkpoint)
      ? stalledBatches + 1
      : 0;
