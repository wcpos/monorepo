import { type CoverageDecision, evaluateCoverageRequirement } from './coverage-model';
import {
...
import {
	type SchedulerTaskSeederRepository,
...
function validSchedulerTasks(tasks: FetchTask[]): {
	validTasks: FetchTask[];
	invalidTasks: InvalidQueryDeclarationTask[];
} {
...
export async function seedSchedulerTasksFromQueryDeclarations(
	input: SeedSchedulerTasksFromQueryDeclarationsInput
): Promise<SeedSchedulerTasksFromQueryDeclarationsResult> {
	const documents = await input.coverageRepository.readSnapshot();
	const declarations = buildDeclarationsFromPersistedCoverage({
		documents,
		nowMs: input.nowMs,
	...
	const coverageDecisions = requirements.map((item) =>
	...
	const requirementById = new Map(
		requirements.map((item) => [item.requirement.id, item.requirement] as const)
	);
