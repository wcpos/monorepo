...
import { CATEGORY_REFERENCE_CONFIG, createReferenceCollectionFetcher } from './rx-scheduler-reference-fetcher';
...
function response(payloads: WooReferencePayload[]): Response {
  return new Response(JSON.stringify(payloads), { status: 200, headers: { 'content-type': 'application/json' } });
}
...
const cat = (id: number): WooReferencePayload => ({ id, name: `Category ${id}`, meta_data: [{ key: '_woocommerce_pos_uuid', value: uuidFor(id) }] });
...
describe('createReferenceCollectionFetcher set-difference deletion', () => {
  ...
  it('keeps the three id-spaces distinct: storage=uuid, coverage=woo-prefix, prune=uuid (divergence regression)', async () => {
    ...
    expect(repository.upsertMany).toHaveBeenCalledWith([expect.objectContaining({ id: uuidFor(1), wooId: 1 })]);
