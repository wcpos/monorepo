...
export function planPersistedSchedulerTaskStates(input: PersistedSchedulerTaskStatePlanInput): PersistedSchedulerTaskStatePlan {
  const existingByTaskId = new Map(input.existingStates.map((state) => [state.taskId, state] as const));
  ...
  const orderedTasks = [...input.tasks].sort((left, right) => right.priority - left.priority || left.id.localeCompare(right.id));
  ...
  for (const task of plannedTasks.values()) {
    ...
    if (baseState.status === 'completed' && input.nowMs - baseState.updatedAtMs <= (input.completedDedupeForMs ?? 0)) {
    ...
    if (baseState.status === 'in-flight' && (baseState.claimedUntilMs ?? 0) > input.nowMs) {
      waiting.push({ taskId: baseState.taskId, ownerId: baseState.ownerId, reason: 'active-owner' });
      statesByTaskId.set(baseState.taskId, baseState);
      continue;
    }

    if (baseState.status === 'failed' && !input.ignoreRetryBackoff && baseState.retryAfterMs !== null && baseState.retryAfterMs > input.nowMs) {
...
function claimReason(state: PersistedSchedulerTaskState, existed: boolean): PersistedSchedulerClaimReason {
