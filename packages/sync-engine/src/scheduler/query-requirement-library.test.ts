...
function queryDeclaration(overrides: Partial<QueryRequirementDeclaration> & Pick<QueryRequirementDeclaration, 'componentId' | 'id' | 'collection' | 'queryKey'>): QueryRequirementDeclaration {
  return {
    ...
    policy: overrides.policy ?? { mode: 'windowed', priority: 100, batchSize: 10, offline: { read: 'serve-local', write: 'queue' } },
  };
}
...
describe('buildSchedulerCoverageRequirementsFromDeclarations', () => {
  it('converts component-declared queries into scheduler coverage requirements', () => {
    const requirements = buildSchedulerCoverageRequirementsFromDeclarations({
      ...
      declarations: [queryDeclaration({ componentId: 'ProductSearch', id: 'products.search.keyboard', collection: 'products', queryKey: 'products:search:keyboard', expectedRecordIds: ['product:keyboard'] })],
    });

    expect(requirements).toHaveLength(1);
    expect(requirements[0].requirement).toMatchObject({ id: 'products.search.keyboard', collection: 'products', kind: 'query', queryKey: 'products:search:keyboard' });
  ...
  it('carries a targeted declaration\'s wooIds onto the requirement — the only channel a targeted fetcher reads', () => {
    ...
    const requirements = buildSchedulerCoverageRequirementsFromDeclarations({
      coverageState: freshCoverage,
      declarations: [queryDeclaration({ componentId: 'DeepLinkRouter', id: 'orders.deepLink.114', collection: 'orders', kind: 'targeted-records', queryKey: 'orders:ids:114', ids: ['woo-order:114'], wooIds: [114] })],
    });

    expect(requirements).toHaveLength(1);
    expect(requirements[0].requirement).toMatchObject({ kind: 'targeted-records', ids: ['woo-order:114'], wooIds: [114] });
  });
  ...
  it('coalesces same-ids targeted declarations to the union of their wooIds — first-without never wins', () => {
    ...
    const requirements = buildSchedulerCoverageRequirementsFromDeclarations({
      coverageState: freshCoverage,
      declarations: [
        queryDeclaration({ componentId: 'LegacyDeepLink', id: 'orders.deepLink.114', collection: 'orders', kind: 'targeted-records', queryKey: 'orders:ids:114', ids: ['woo-order:114'] }),
        queryDeclaration({ componentId: 'OrderDetailsRoute', id: 'orders.deepLink.114', collection: 'orders', kind: 'targeted-records', queryKey: 'orders:ids:114', ids: ['woo-order:114'], wooIds: [114] }),
      ],
    });
  ...
  it('coalesces duplicate declarations while preserving requesting component ids and highest priority', () => {
    const requirements = buildSchedulerCoverageRequirementsFromDeclarations({
      coverageState: freshCoverage,
      declarations: [
        queryDeclaration({ componentId: 'ProductGrid', id: 'products.search.keyboard', collection: 'products', queryKey: 'products:search:keyboard', expectedRecordIds: ['product:alpha'], policy: { mode: 'windowed', priority: 100, batchSize: 10, offline: { read: 'serve-local', write: 'queue' } } }),
        queryDeclaration({ componentId: 'ProductSearchBox', id: 'products.search.keyboard', collection: 'products', queryKey: 'products:search:keyboard', expectedRecordIds: ['product:keyboard'], policy: { mode: 'windowed', priority: 900, batchSize: 10, offline: { read: 'serve-local', write: 'queue' } } }),
      ],
    });
  ...
  it('merges duplicate declarations to the strictest coverage strategy independent of declaration order', async () => {
    ...
    const result = await runQueryRequirementFlow({
      ...
      coverageState: freshCoverage,
      declarations: [
        queryDeclaration({ componentId: 'ProductLaneConsumer', id: 'products.search.keyboard', collection: 'products', queryKey: 'products:search:keyboard', coverageStrategy: 'lane-only' }),
        queryDeclaration({ componentId: 'ProductRecordConsumer', id: 'products.search.keyboard', collection: 'products', queryKey: 'products:search:keyboard', expectedRecordIds: ['product:missing'], coverageStrategy: 'record-and-lane' }),
      ],
      fetcher,
    });
    ...
    expect(result.coverageFlow.scheduler.tasks.map((task) => task.requirementId)).toEqual(['products.search.keyboard']);
    ...
    expect(reverseOrderResult.coverageFlow.scheduler.tasks.map((task) => task.requirementId)).toEqual(['products.search.keyboard']);

    const mixedRecordAndLaneResult = await runQueryRequirementFlow({
      connectivity: 'online',
      coverageState: freshCoverage,
      declarations: [
        queryDeclaration({ componentId: 'ProductRecordConsumer', id: 'products.search.keyboard', collection: 'products', queryKey: 'products:search:keyboard', expectedRecordIds: ['product:missing'], coverageStrategy: 'record-only' }),
        queryDeclaration({ componentId: 'ProductLaneConsumer', id: 'products.search.keyboard', collection: 'products', queryKey: 'products:search:keyboard', coverageStrategy: 'lane-only' }),
      ],
      fetcher,
    });
    ...
    expect(mixedRecordAndLaneResult.coverageFlow.scheduler.tasks.map((task) => task.requirementId)).toEqual(['products.search.keyboard']);
  });
  ...
  it('coalesces current query evidence so record-and-lane gates can reject stale lane assumptions', async () => {
    ...
    expect(result.coverageFlow.scheduler.tasks.map((task) => task.requirementId)).toEqual(['products.search.coffee']);
  });
  ...
  it('keeps targeted record declarations local when coverage has the fresh record', async () => {
    ...
    const result = await runQueryRequirementFlow({
      connectivity: 'online',
      coverageState: freshCoverage,
      declarations: [queryDeclaration({ componentId: 'OrderDeepLink', id: 'orders.deepLink.114', collection: 'orders', kind: 'targeted-records', queryKey: 'orders:ids:114', ids: ['woo-order:114'], policy: { mode: 'on-demand', priority: 950, batchSize: 1, offline: { read: 'fail-if-missing', write: 'queue' } } })],
      fetcher,
    });

    expect(result.coverageFlow.servedLocal.map((decision) => decision.requirementId)).toEqual(['orders.deepLink.114']);
