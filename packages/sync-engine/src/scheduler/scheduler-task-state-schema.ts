...
export type SchedulerTaskStateDocument = SchedulerTaskStateCommon & PersistedSchedulerSchemaVersionMarker<4>;
/** v3: carries the optional `rerunRequested` coalescing flag (#318) — field-identical to v4. */
export type SchedulerTaskStateV3Document = SchedulerTaskStateCommon & PersistedSchedulerSchemaVersionMarker<3>;
/** v2: before the `rerunRequested` coalescing flag existed (carried the numeric `wooIds` channel). */
export type SchedulerTaskStateV2Document = Omit<SchedulerTaskStateCommon, 'rerunRequested'> & PersistedSchedulerSchemaVersionMarker<2>;
/** v1: before the `wooIds` numeric channel existed. */
export type SchedulerTaskStateV1Document = Omit<SchedulerTaskStateCommon, 'wooIds' | 'rerunRequested'> & PersistedSchedulerSchemaVersionMarker<1>;
/** v0: pre-marker shape (raw state, before stateKey/collectionName + wooIds). */
export type SchedulerTaskStateV0Document = Omit<PersistedSchedulerTaskState, 'wooIds' | 'rerunRequested'>;

export function migrateSchedulerTaskStateV1(document: SchedulerTaskStateV0Document): SchedulerTaskStateV1Document {
  const { collection, ...rest } = document;
  return markPersistedSchedulerDocument({ ...rest, stateKey: schedulerTaskStateKey(document.taskId), collectionName: collection }, 1);
}

/** v1 → v2: additive — `wooIds` is optional, so a v1 doc simply gains the (absent) field. */
export function migrateSchedulerTaskStateV2(document: SchedulerTaskStateV1Document): SchedulerTaskStateV2Document {
...
export function migrateSchedulerTaskStateV3(document: SchedulerTaskStateV2Document): SchedulerTaskStateV3Document {
  const { schemaVersion: _schemaVersion, ...rest } = document;
 ...
 * are unaffected.
 */
export function migrateSchedulerTaskStateV4(document: SchedulerTaskStateV3Document): SchedulerTaskStateDocument | null {
  if (
...
export const schedulerTaskStateSchema = {
  ...
  required: ['stateKey', 'taskId', 'requirementId', 'collectionName', 'queryKey', 'limit', 'priority', 'mode', 'status', 'ownerId', 'claimedUntilMs', 'attempt', 'retryAfterMs', 'updatedAtMs', 'schemaVersion'],
  indexes: [
    ['status'],
