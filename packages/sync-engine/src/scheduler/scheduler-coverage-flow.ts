...
export async function runSchedulerCoverageFlow(
	input: SchedulerCoverageFlowInput
): Promise<SchedulerCoverageFlowResult> {
	const coverageDecisions = input.requirements.map((item) =>
		evaluateCoverageRequirement({
	...
	const requirementById = new Map(
		input.requirements.map((item) => [item.requirement.id, item.requirement])
	);
