...
describe('seedPersistedSchedulerTasks', () => {
	...
	it('disables completed-dedupe on a non-positive window — re-seeds even at delta 0 (fresh change-signal pull)', async () => {
		...
		const existing = state({
			taskId: 'products:ids:7',
			requirementId: 'products.targeted.7',
			status: 'completed',
			updatedAtMs: 10_000,
			attempt: 0,
		});
	...
	it('flags an in-flight row for coalesced re-run when coalesceInFlight is set, leaving queued/failed runnable', async () => {
		...
		const queued = state({
			taskId: 'orders:queued',
			requirementId: 'orders.queued',
			status: 'queued',
		});
		const inFlight = state({
			taskId: 'orders:in-flight',
			requirementId: 'orders.in-flight',
			status: 'in-flight',
			ownerId: 'tab-a',
		...
		const failed = state({
			taskId: 'orders:failed',
			requirementId: 'orders.failed',
			status: 'failed',
			retryAfterMs: 9_000,
			attempt: 2,
		});
