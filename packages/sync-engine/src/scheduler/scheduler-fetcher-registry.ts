...
export type SchedulerTaskSupportCandidate = Pick<FetchTask, 'collection' | 'queryKey' | 'ids' | 'mode' | 'limit'>;
...
export type SchedulerFetcherRegistry = {
  supportsTask(task: SchedulerTaskSupportCandidate): boolean;
  fetcher: SchedulerFetcher;
  supportedRepository(repository: PersistedSchedulerTaskRunnerRepository): PersistedSchedulerTaskRunnerRepository;
};
...
export function createSchedulerFetcherRegistry(registrations: SchedulerFetcherRegistration[]): SchedulerFetcherRegistry {
  function matchingRegistration(task: SchedulerTaskSupportCandidate): SchedulerFetcherRegistration | null {
  ...
  return {
    ...
    fetcher: async (task, context) => {
      ...
      if (!registration) {
        throw new Error(`No scheduler fetcher registered for ${task.collection} task ${task.queryKey}`);
      }
      return registration.fetcher(task, context);
    },
    supportedRepository: (repository) => ({
      readRunnable: async (nowMs) => (await repository.readRunnable(nowMs)).filter((state) => matchingRegistration(taskCandidateFromState(state)) !== null),
