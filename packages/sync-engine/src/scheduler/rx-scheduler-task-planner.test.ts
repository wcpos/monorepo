...
import { planAndPersistSchedulerTasks, type SchedulerTaskPlannerRepository } from './rx-scheduler-task-planner';
...
function repository(existingStates: PersistedSchedulerTaskState[] = [], claimNew = true, claim = true): SchedulerTaskPlannerRepository {
  return {
...
describe('planAndPersistSchedulerTasks', () => {
  ...
  it('uses guarded claims for existing runnable states and omits claims lost to another owner', async () => {
    const existing = state({ status: 'in-flight', ownerId: 'tab-b', claimedUntilMs: 900, attempt: 2, retryAfterMs: null, updatedAtMs: 800 });
    ...
    expect(result.plan.claimed).toEqual([{ taskId: 'orders:custom-pull:windowed', reason: 'expired-owner' }]);
