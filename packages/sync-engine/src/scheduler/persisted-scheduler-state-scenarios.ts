...
function existingOpenOrdersState(overrides: Partial<PersistedSchedulerTaskState>): PersistedSchedulerTaskState {
  return {
...
export const persistedSchedulerStateScenarios: PersistedSchedulerStateScenario[] = [
  {
    ...
    description: 'A freshly scheduled query lane is persisted and claimed by the current tab so it can survive reload bookkeeping.',
    nowMs: 2_000,
    ownerId: 'tab-a',
    claimForMs: 5_000,
  ...
  },
  {
    ...
    description: 'Another tab has a non-expired lease, so this tab waits instead of duplicating remote Woo work.',
    nowMs: 2_000,
    ownerId: 'tab-a',
    claimForMs: 5_000,
    tasks: [openOrdersTask],
    existingStates: [existingOpenOrdersState({ status: 'in-flight', ownerId: 'tab-b', claimedUntilMs: 3_000, attempt: 1 })],
  },
  {
    id: 'expired-owner-takeover',
    label: 'Existing task · expired owner',
    description: 'An abandoned tab left an expired in-flight lease, so the current tab can take over and increment the attempt.',
    nowMs: 4_000,
    ownerId: 'tab-a',
    claimForMs: 5_000,
    tasks: [openOrdersTask],
    existingStates: [existingOpenOrdersState({ status: 'in-flight', ownerId: 'tab-b', claimedUntilMs: 3_000, attempt: 2 })],
  },
  {
    ...
    claimForMs: 5_000,
    tasks: [openOrdersTask],
    existingStates: [existingOpenOrdersState({ status: 'failed', attempt: 2, retryAfterMs: 5_000 })],
  },
  {
    id: 'completed-task-dedupe',
    label: 'Completed task · reload dedupe',
    description: 'Completed durable work is retained so a reload does not immediately enqueue duplicate remote fetch work.',
    nowMs: 4_000,
    ownerId: 'tab-a',
    claimForMs: 5_000,
...
export function runPersistedSchedulerStateScenario(scenario: PersistedSchedulerStateScenario): PersistedSchedulerTaskStatePlan {
...
export function summarizePersistedSchedulerStateScenarios(
  ...
  return scenarios.map((scenario) => {
    ...
    return {
      ...
      waiting: result.waiting.map((wait) => `${requirementLabel(result, wait.taskId)}:${wait.reason}:${wait.ownerId ?? '—'}`),
      completed: result.completed.map((taskId) => requirementLabel(result, taskId)),
      nextStatuses: result.states.map((state) => `${state.requirementId}:${state.status}:${state.ownerId ?? '—'}`),
    };
  });
}

function summarizeAction(result: PersistedSchedulerTaskStatePlan): PersistedSchedulerStateScenarioSummary['action'] {
