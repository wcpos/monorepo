...
import type { ConnectivityMode, ReplicationPolicy, ReplicationRequirement } from './replication-policy';
...
function strictestCoverageStrategy(left: CoverageStrategy, right: CoverageStrategy): CoverageStrategy {
...
function uniqueNumbersInOrder(values: Array<number | undefined>): number[] {
 ...
 */
function mergedWooIds(existingWooIds: number[] | undefined, declarationWooIds: number[] | undefined): number[] | undefined {
  const wooIds = uniqueNumbersInOrder([...(existingWooIds ?? []), ...(declarationWooIds ?? [])]);
  return wooIds.length > 0 ? wooIds : undefined;
}

function maxKnownTotal(values: Array<number | null | undefined>): number | null | undefined {
...
function coalescingKey(declaration: QueryRequirementDeclaration): string {
  const idsPart = declaration.kind === 'targeted-records' ? [...declaration.ids ?? []].sort().join(',') : '';
  return [declaration.collection, declaration.queryKey, declaration.kind ?? 'query', declaration.policy.mode, idsPart].join('|');
}
...
function mergeDeclaration(existing: CoalescedDeclaration, declaration: QueryRequirementDeclaration): CoalescedDeclaration {
  const existingPriority = existing.requirement.policy.priority;
  const nextPolicy = declaration.policy.priority > existingPriority ? declaration.policy : existing.requirement.policy;
  ...
  return {
    ...
    coverageStrategy: strictestCoverageStrategy(existing.coverageStrategy, declaration.coverageStrategy),
    coverageState: existing.coverageState,
    expectedRecordIds: uniqueInOrder([...(existing.expectedRecordIds ?? []), ...(declaration.expectedRecordIds ?? [])]),
    currentRecordIds: uniqueInOrder([...(existing.currentRecordIds ?? []), ...(declaration.currentRecordIds ?? [])]),
    totalMatchingRecords: maxKnownTotal([existing.totalMatchingRecords, declaration.totalMatchingRecords]),
    declaredBy: uniqueInOrder([...existing.declaredBy, declaration.componentId]),
  };
}

export function buildSchedulerCoverageRequirementsFromDeclarations(input: BuildSchedulerCoverageRequirementsInput): SchedulerCoverageFlowRequirementWithSources[] {
...
export async function runQueryRequirementFlow(input: QueryRequirementFlowInput): Promise<QueryRequirementFlowResult> {
