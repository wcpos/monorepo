...
describe('schedulerCoverageFlowScenarios', () => {
  ...
  it('runs the product search preemption scenario with search before background poll', async () => {
    const scenario = schedulerCoverageFlowScenarios.find((item) => item.id === 'product-search-preemption');
  ...
  it('summarizes served local, remote, skipped, request, and document counts', async () => {
    const scenario = schedulerCoverageFlowScenarios.find((item) => item.id === 'offline-local-only');

    expect(scenario).toBeDefined();
    await expect(summarizeSchedulerCoverageFlowScenario(scenario!))
      .resolves.toBe('served local 1 · remote tasks 0 · skipped 1 · requests 0 · docs 0');
  });
});
