import { expectedRecordIdsForLane, toLocalCoverageState, type PersistedCoverageDocumentSet } from './persisted-coverage-schema';
import { runQueryRequirementFlow, type QueryRequirementDeclaration, type QueryRequirementFlowResult } from './query-requirement-library';
...
export function buildDeclarationsFromPersistedCoverage(input: {
  ...
  return input.declarations.map((declaration) => declarationWithPersistedExpectedIds(declaration, input.documents, input.nowMs));
}

export async function runPersistedCoverageQueryRequirementFlow(input: PersistedCoverageQueryRequirementFlowInput): Promise<QueryRequirementFlowResult> {
