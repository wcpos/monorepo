...
import {
	createRxKeyedRepository,
	type RxKeyedCollection,
} from '../collections/rx-keyed-repository';
...
function sameSchedulerTaskState(
	left: PersistedSchedulerTaskState,
	right: PersistedSchedulerTaskState
): boolean {
	return (
...
export class RxSchedulerTaskStateRepository {
	...
	async completeOrRequeue(
		...
		if (
			expectedState.taskId !== completedState.taskId ||
			expectedState.taskId !== requeuedState.taskId
		)
			return 'claim-lost';
	...
	private async replaceIfCurrent(
		expectedState: PersistedSchedulerTaskState,
		nextState: PersistedSchedulerTaskState
	): Promise<boolean> {
		return this.keyed.replaceIfCurrent(
			...
			(currentDocument, nextDocument) =>
				currentDocument.rerunRequested ? { ...nextDocument, rerunRequested: true } : nextDocument
		);
	}
}
