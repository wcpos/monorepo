...
import { schedulerTaskStateKey, schedulerTaskStateSchema, type SchedulerTaskStateDocument } from '@woo-rxdb-lab/sync-engine-rxdb/testing';

type StoredTaskState = SchedulerTaskStateDocument & { _deleted?: boolean };

function taskState(overrides: Partial<PersistedSchedulerTaskState> = {}): PersistedSchedulerTaskState {
  return {
...
function createCollection(documents: StoredTaskState[] = []) {
  ...
  const find = vi.fn((_query?: unknown) => ({
    exec: vi.fn(async () => [...stored.values()]
  ...
  return { collection: { insert, bulkUpsert, find, findOne }, stored, insert, bulkUpsert, find, findOne };
}

function repositoryFor(documents: PersistedSchedulerTaskState[] = []) {
  const fixture = createCollection(documents.map(storedTaskState));
  const repository = new RxSchedulerTaskStateRepository({ schedulerTaskStates: fixture.collection } as never);
...
describe('RxSchedulerTaskStateRepository', () => {
  ...
  it('allows task ids large enough for targeted batches built from existing field limits', () => {
    ...
    const twoRecordTargetedTaskIdLength = maxCollectionLength
      + 1
      + maxQueryKeyLength
      + 1
      + maxModeLength
    ...
    expect(stateKey.length).toBeLessThanOrEqual(schedulerTaskStateSchema.properties.stateKey.maxLength);
  ...
  it('round-trips the wooIds numeric channel through persisted storage (uuid-ready targeted fetch)', async () => {
    const { repository } = repositoryFor([
      taskState({ taskId: 'orders:ids:deep-link:on-demand', ids: ['woo-order:123', 'woo-order:456'], wooIds: [123, 456] }),
    ]);
  ...
  it('returns runnable queued, expired-owner, and retry-ready task states sorted by priority then task id', async () => {
    const { repository, find } = repositoryFor([
      taskState({ taskId: 'active-owner', priority: 900, status: 'in-flight', claimedUntilMs: 2_000, retryAfterMs: null }),
      taskState({ taskId: 'expired-low', priority: 100, status: 'in-flight', claimedUntilMs: 1_000, retryAfterMs: null }),
      taskState({ taskId: 'expired-high', priority: 900, status: 'in-flight', claimedUntilMs: 1_000, retryAfterMs: null }),
      taskState({ taskId: 'backoff-active', priority: 800, status: 'failed', ownerId: null, claimedUntilMs: null, retryAfterMs: 2_000 }),
      taskState({ taskId: 'retry-ready', priority: 700, status: 'failed', ownerId: null, claimedUntilMs: null, retryAfterMs: 1_000 }),
      taskState({ taskId: 'queued', priority: 700, status: 'queued', ownerId: null, claimedUntilMs: null, retryAfterMs: null }),
      taskState({ taskId: 'completed', priority: 1_000, status: 'completed', ownerId: null, claimedUntilMs: null, retryAfterMs: null }),
    ]);
  ...
  it('claims a runnable scheduler task only when stored state still matches expected state', async () => {
    const expired = taskState({ taskId: 'orders:a', ownerId: 'tab-a', attempt: 1, claimedUntilMs: 1_000, updatedAtMs: 1_000 });
    const claimed = taskState({ ...expired, ownerId: 'tab-b', attempt: 2, claimedUntilMs: 2_000, updatedAtMs: 1_500 });
  ...
  it('does not claim over a newer scheduler task owner attempt', async () => {
    const older = taskState({ taskId: 'orders:a', ownerId: 'tab-a', attempt: 1, claimedUntilMs: 1_000, updatedAtMs: 1_000 });
    const newer = taskState({ taskId: 'orders:a', ownerId: 'tab-c', attempt: 3, claimedUntilMs: 3_000, updatedAtMs: 2_000 });
    const claimedFromOlder = taskState({ ...older, ownerId: 'tab-b', attempt: 2, claimedUntilMs: 2_000, updatedAtMs: 1_500 });
  ...
  it('does not replace a scheduler task with a different task id', async () => {
    ...
    const crossTaskClaim = taskState({ ...expected, taskId: 'orders:b', ownerId: 'tab-b', attempt: 2 });
  ...
  it('marks a scheduler task failed only when the stored state still matches expected state', async () => {
    const inFlight = taskState({ taskId: 'orders:a', ownerId: 'tab-a', attempt: 1, claimedUntilMs: 1_000, updatedAtMs: 1_000 });
    const failed = taskState({ ...inFlight, status: 'failed', ownerId: null, claimedUntilMs: null, retryAfterMs: 2_000, updatedAtMs: 1_500 });
  ...
  it('marks a scheduler task completed only when the stored state still matches expected state', async () => {
    const inFlight = taskState({ taskId: 'orders:a', ownerId: 'tab-a', attempt: 1, claimedUntilMs: 1_000, updatedAtMs: 1_000 });
    const completed = taskState({ ...inFlight, status: 'completed', ownerId: null, claimedUntilMs: null, retryAfterMs: null, updatedAtMs: 1_500 });
    const { repository, stored } = repositoryFor([inFlight]);
  ...
  it('does not remove or mark failed state when targeted ids differ', async () => {
    ...
    const failedState = taskState({ ...expectedState, status: 'failed', ownerId: null, claimedUntilMs: null, retryAfterMs: 2_000 });
    ...
    expect(stored.get(schedulerTaskStateKey('orders:targeted'))).toEqual(storedTaskState(storedState));
  });
  ...
  it('does not remove or mark failed state when only the wooIds numeric channel differs', async () => {
    ...
    const expectedState = taskState({ taskId: 'orders:targeted', ids: ['woo-order:1'], wooIds: [2] });
    const failedState = taskState({ ...expectedState, status: 'failed', ownerId: null, claimedUntilMs: null, retryAfterMs: 2_000 });
    const { repository, stored } = repositoryFor([storedState]);
...
describe('RxSchedulerTaskStateRepository — change-signal coalescing (#318)', () => {
  ...
  describe('requestRerunOrReseed', () => {
    it('flags an actively in-flight task to re-run, touching NO field the completion CAS compares', async () => {
      const inFlight = taskState({ status: 'in-flight', claimedUntilMs: 6_000, updatedAtMs: 1_000, attempt: 2 });
      const { repository, stored } = repositoryFor([inFlight]);

      const outcome = await repository.requestRerunOrReseed(inFlight, taskState({ status: 'queued', attempt: 2 }), nowMs);
    ...
    it('leaves a lease-expired task untouched (it will be re-claimed and run on its own)', async () => {
      ...
      expect(await repository.requestRerunOrReseed(expired, taskState({ status: 'queued' }), nowMs)).toBe('skipped');
    ...
    it('leaves a FAILED task untouched — preserving its retry backoff (codex P2)', async () => {
      const failed = taskState({ status: 'failed', ownerId: null, claimedUntilMs: null, retryAfterMs: 9_000, attempt: 3 });
      ...
      expect(await repository.requestRerunOrReseed(failed, taskState({ status: 'queued' }), nowMs)).toBe('skipped');
    ...
    it('re-queues directly when the task already completed (race: owner finished before our write, no future run)', async () => {
      ...
      const reseeded = taskState({ status: 'queued', attempt: 0, ownerId: null, claimedUntilMs: null, updatedAtMs: nowMs });
    ...
    it('returns skipped when the task row is gone', async () => {
      const { repository } = repositoryFor([]);
      expect(await repository.requestRerunOrReseed(taskState(), taskState(), nowMs)).toBe('skipped');
    });
  });
  ...
  describe('completeOrRequeue', () => {
    it('completes normally when no re-run was requested', async () => {
      ...
      const completed = taskState({ status: 'completed', ownerId: null, claimedUntilMs: null, updatedAtMs: nowMs });
      const { repository, stored } = repositoryFor([inFlight]);

      expect(await repository.completeOrRequeue(inFlight, completed, taskState({ status: 'queued' }))).toBe('completed');
    ...
    it('re-queues (clearing the flag) when a re-run was requested mid-flight, even though the runner\'s expected state predates the flag', async () => {
      ...
      const completed = taskState({ status: 'completed', ownerId: null, claimedUntilMs: null });
      const requeued = taskState({ status: 'queued', attempt: 0, ownerId: null, claimedUntilMs: null, updatedAtMs: nowMs });
      ...
      const outcome = await repository.completeOrRequeue(taskState({ status: 'in-flight' }), completed, requeued);
  ...
  it('END-TO-END: a change seeded mid-flight survives the completion CAS and forces a fresh re-run instead of a silent drop', async () => {
    ...
    expect(await repository.requestRerunOrReseed(claimed, taskState({ status: 'queued' }), nowMs)).toBe('rerun-requested');
  ...
  it('preserves the rerun flag across a lease renewal (claim) — a multi-page greedy task is not silently completed', async () => {
    ...
    const outcome = await repository.completeOrRequeue(
      renewed,
      taskState({ status: 'completed', ownerId: null, claimedUntilMs: null }),
      taskState({ status: 'queued', attempt: 0, ownerId: null, claimedUntilMs: null, updatedAtMs: nowMs }),
    );
    expect(outcome).toBe('requeued');
  ...
  it('preserves the rerun flag across a mark-failed → retry so the coalesced change survives a failed page', async () => {
    const claimed = taskState({ status: 'in-flight', claimedUntilMs: 6_000, updatedAtMs: 1_000, attempt: 1 });
    const { repository, stored } = repositoryFor([claimed]);
    ...
    const failed = taskState({ status: 'failed', ownerId: null, claimedUntilMs: null, retryAfterMs: 9_000, attempt: 2, updatedAtMs: 1_000 });
