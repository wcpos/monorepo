...
export const coverageStrategies: CoverageStrategy[] = ['record-only', 'lane-only', 'record-and-lane'];

function requiredRecordIds(requirement: ReplicationRequirement, expectedRecordIds: string[] = []): string[] {
...
function freshRecordIds(state: LocalCoverageState, collection: string): Set<string> {
  return new Set(state.records.filter((record) => record.collection === collection && record.fresh).map((record) => record.id));
}

function missingFreshRecordIds(state: LocalCoverageState, requirement: ReplicationRequirement, expectedRecordIds?: string[]): string[] {
...
function findFreshCompleteLane(state: LocalCoverageState, requirement: ReplicationRequirement): LocalLaneCoverage | undefined {
...
function currentQueryEvidenceFailure(input: CoverageEvaluationInput): string | null {
  if (input.requirement.kind === 'targeted-records' || input.expectedRecordIds === undefined) return null;
  ...
  if (input.totalMatchingRecords !== null && typeof input.totalMatchingRecords !== 'undefined' && input.totalMatchingRecords > input.expectedRecordIds.length) {
...
function recordDecision(input: CoverageEvaluationInput): CoverageDecision {
  const missingRecordIds = missingFreshRecordIds(input.state, input.requirement, input.expectedRecordIds);
  if (missingRecordIds.length === 0 && requiredRecordIds(input.requirement, input.expectedRecordIds).length > 0) {
    return {
      ...
      missingRecordIds: [],
      risks: input.requirement.kind === 'query' ? ['record metadata alone does not prove the full query lane is complete'] : [],
    };
  }
  ...
  return {
    ...
    requirementId: input.requirement.id,
    action: 'fetch-remote',
    reason: missingRecordIds.length > 0 ? 'one or more required records are missing or stale' : 'record-only coverage cannot prove query completeness without expected record ids',
    missingRecordIds,
    risks: [],
  };
}
...
export function evaluateCoverageRequirement(input: CoverageEvaluationInput): CoverageDecision {
  ...
  return {
    ...
    requirementId: input.requirement.id,
    action: 'fetch-remote',
    reason: lane ? 'lane is complete but expected records are missing or stale' : 'no matching complete fresh lane coverage',
    missingRecordIds,
    risks: [],
  };
}

export function runCoverageComparison(cases: CoverageEvaluationCase[], strategies: CoverageStrategy[] = coverageStrategies): CoverageComparisonResult[] {
  return cases.flatMap((coverageCase) => strategies.map((strategy) => ({
