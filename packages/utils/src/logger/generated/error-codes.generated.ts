...
export const ERROR_CATALOGUE: Record<ErrorCode, CatalogueEntry> = {
	...
	PAYMENT101: {
		...
		domain: 'PAYMENT',
		severity: 'info',
		actionHint: 'No action needed — the payment succeeded; do not charge again.',
	...
	PAYMENT121: {
		...
		domain: 'PAYMENT',
		severity: 'error',
		actionHint: 'Do not charge again — reload the app and the till finishes the order.',
		dataSafety: 'money-moved',
	...
	CLIENT101: {
		...
		domain: 'CLIENT',
		severity: 'error',
		actionHint: "Restart; if it still won't start, open the help page before clearing local data.",
		dataSafety: 'local-only',
	...
	PAYMENT511: {
		...
		domain: 'PAYMENT',
		severity: 'error',
		actionHint: 'Record the balance owed from the money taken, then refund the difference.',
		dataSafety: 'money-moved',
	...
	SYNC221: {
		...
		severity: 'warn',
		actionHint:
			'Compare with the store, then choose Send again or Use server version in Store health → Database.',
		dataSafety: 'order-safe',
	...
	AUTH411: {
		...
		severity: 'error',
		actionHint:
			'Enter the store address; if it is already filled in, ask the site admin to update WCPOS.',
		dataSafety: 'no-impact',
