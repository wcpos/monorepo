...
description: "This record on the device does not match your store and needs downloading again."
---
...
This record on the device does not match your store and needs downloading again.
...
1. Expand the log entry - its context names the collection, the record id and the detector that spotted the difference.
2. Run a targeted download for the affected collection so the device replaces its copy with the store's.
3. Compare the record in WP Admin with what the POS shows; a price or stock difference is the visible symptom.
4. If the same records keep reappearing after a download, export debug info and contact support - repeated escalation means the download is not resolving the difference.
