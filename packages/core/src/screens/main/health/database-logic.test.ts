...
describe('database page logic', () => {
	...
	it('never computes a percentage for windowed collections even with a fresh census', () => {
		...
		// Variations download with their products (policy, not lag) — same
		// windowed treatment now that the plugin route gives a real total.
		const variations = deriveCollectionRow('variations', 1_854, census(4_020));
		expect(variations.windowed).toBe(true);
		expect(variations.percentLocal).toBeNull();
		expect(variations.serverTotal).toBe(4_020);
	});
