...
describe('useCheckoutSession', () => {
	...
	it('releases a failed background refresh after a stock rejection', async () => {
		const release = jest.fn();
		const ready = Promise.reject(new Error('refresh failed'));
		mockEngineRequire.mockReturnValue({ ready, release });
		mockGet.mockResolvedValueOnce({
			data: [
				{
					id: 'stripe_terminal_for_woocommerce',
		...
		mockPost.mockResolvedValueOnce({ data: { status: 'ready' } }).mockRejectedValueOnce({
			response: {
				data: {
					...
					data: {
						items: [{ product_id: 10, variation_id: 0, available: 1 }],
					},
				},
			},
		});
		...
		await expect(ready).rejects.toThrow('refresh failed');
