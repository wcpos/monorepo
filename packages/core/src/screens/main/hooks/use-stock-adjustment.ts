...
export function stockAdjustment(runtime: QueryRuntime, lineItems: LineItems): void {
...
export const useStockAdjustment = () => {
	...
	return {
		stockAdjustment: React.useCallback(
			(lineItems: LineItems) => stockAdjustment(runtime, lineItems),
			[runtime]
		),
	};
};
