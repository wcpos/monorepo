...
import { Button, ButtonText, buttonVariants } from '../button';
			...
			components={{
				...
				CaptionLabel: ({ children }) => (
					<Button
						...
						variant="ghost"
						size="sm"
						onPress={() => setNavView((prev) => (prev === 'days' ? 'years' : 'days'))}
					>
						<ButtonText>
							...
							<div className={cn('grid grid-cols-4 gap-y-2', className)} {...props}>
								{Array.from({ length: displayYears.to - displayYears.from + 1 }, (_, i) => {
									...
									return (
										...
										>
											<ButtonText>{displayYears.from + i}</ButtonText>
										</Button>
									);
								})}
							</div>
