...
The defect is the gate. #1057 removed the config-flag refusal from `scripts/opfs-targeted-recovery.mjs` because leader-only cleanup made it unnecessary. #1713 (2026-08-30/31) reintroduced it for hollow-row drops and stale secondary rows without reconciling with the leader-only design, and no review thread challenged it. The correct gate is "refuse unless this tab is the RxDB leader", which requires the leadership fact to reach the storage worker (#1982). Leadership narrows the window but does not close it: an adversarial pass on #1982 reproduced corruption across a leadership handoff, because rxdb-premium applies positional broadcast ops without identity checks and does not reload index rows when it takes the collection lock, so a late-arriving `D` can land on a healthy neighbour. That is #1049 one layer down. Until premium ships identity-checked ops or a resync on lock acquisition, refused repairs on web leave a row unrepaired; they do not corrupt anything, and that is the safer failure.
