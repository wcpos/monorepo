...
async function repairDocument(instance, documentId) {
	...
	return instance.taskQueue.runCleanup(async (runState) => {
		...
		const indexRows = state.indexStates.map((indexState) => {
			const position = indexState.rows.findIndex((row) => row[1] === oldStart && row[2] === oldEnd);
			return { indexState, position };
		});
		if (indexRows.some(({ position }) => position < 0)) return 'missing-index-row';

		let accessHandlePromise = runState.accessHandlers.get(state.documentFileHandle);
		...
		try {
			if (
				indexRows.some(
					({ indexState, position }) =>
						indexState.getIndexableString(document) !== indexState.rows[position][0]
				)
			)
...
async function reconcileSecondaryIndexes(instance) {
	...
	return instance.taskQueue.runCleanup(async (runState) => {
		...
		const accessHandle = await accessHandlePromise;
		const secondaries = state.indexStates.filter((indexState) => indexState !== state.firstIdx);
		...
		for (const [indexKey] of state.firstIdx.rows) {
			...
			if (primaryIds.has(documentId)) return `duplicate-primary-id:${documentId.trim()}`;
		...
		if (!classified.some(({ healthy }) => healthy)) return 'no-healthy-secondary';
		const documents = [];
		...
		for (const [indexKey, start, end] of state.firstIdx.rows) {
			...
			let document;
			try {
				document = JSON.parse(instance._decode(await accessHandle.read(start, end)));
			} catch {
			...
			if (
				...
				return `primary-row-mismatch:${documentId.trim()}`;
			if (seenRanges.has(start)) return `duplicate-primary-range:${documentId.trim()}`;
			...
			for (const { indexState, rowsById, healthy } of classified) {
				for (const row of rowsById.get(documentId) ?? []) {
					if (row[1] === start && row[2] === end) {
						if (!healthy) continue;
						try {
							if (indexState.getIndexableString(document) === row[0]) primaryVotes += 1;
						} catch {}
					} else {
			...
			for (const { range, voters } of competing.values()) {
				...
				let candidate;
				try {
					candidate = JSON.parse(instance._decode(await accessHandle.read(range[0], range[1])));
				} catch {
					continue;
				}
				...
				for (const [indexState, rowKey] of voters) {
					try {
						if (indexState.getIndexableString(candidate) === rowKey) validVotes += 1;
					} catch {}
				}
				if (validVotes >= primaryVotes) return `uncorroborated-primary-range:${documentId.trim()}`;
			}
		...
		const ordered = [...documents].sort((left, right) => left.start - right.start);
		...
		for (const indexState of state.indexStates) {
			...
			const rebuilt = documents
				.map(({ document, start, end }) => [indexState.getIndexableString(document), start, end])
	...
	return {
		...
		async createStorageInstance(params) {
			...
			instance.findDocumentsById = async (ids, withDeleted) => {
				try {
					...
					if (ids.length > 1) {
						...
						return typeof batches[0] === 'string' ? JSON.stringify(documents) : documents;
					}
					throw error;
				}
			};
			...
			instance.bulkWrite = async (documentWrites, context) => {
				const ids = documentWrites.map((row) => row.document[instance.primaryPath]);
			...
			const repairIndexedRead = async (error, generationAtStart) => {
				...
				const repairedDocuments = await repairMalformedIds(state.firstIdx.metaIdMap.keys());
			...
			instance.query = async (preparedQuery) => {
				...
				try {
					...
					if (!isMalformedJson(error) || !(await repairIndexedRead(error, generationAtStart))) {
						throw error;
					}
					return parseStorageResult(await query(preparedQuery));
				}
			...
			instance.getChangedDocumentsSince = async (limit, checkpoint) => {
				const generationAtStart = reconcileGeneration;
				try {
					return parseStorageResult(await getChangedDocumentsSince(limit, checkpoint));
				} catch (error) {
					if (!isMalformedJson(error) || !(await repairIndexedRead(error, generationAtStart))) {
						throw error;
					}
