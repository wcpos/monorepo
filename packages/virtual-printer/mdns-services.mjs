 ...
 *
 * @param {{ name?: string, port?: number, ipp?: boolean, txt?: boolean }} [options]
 * @returns {Array<{ name: string, type: string, port: number, txt: Record<string, string> }>}
 */
export function buildMdnsServices(options = {}) {
	const { name = 'Virtual WCPOS Printer', port = 9100, ipp = false, txt: withTxt = true } = options;
	// `txt: false` mimics a printer advertising an instance name only — no model to infer from.
	const txt = withTxt ? { product: name, ty: name } : {};
