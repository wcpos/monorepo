import { checkpointInstantMs, normalizeCheckpoint, type OrderDocument, type PullResponse, type ServerMetrics, type SyncCheckpoint } from '@woo-rxdb-lab/shared';
...
export async function syncCustomPullBatchIntoRepository(input: {
   ...
   */
  assembleDocument?: (document: PullResponse['documents'][number]) => PullResponse['documents'][number];
  afterUpsert?: (documents: PullResponse['documents'], result: Pick<PullResponse, 'hasMore' | 'checkpoint'>) => Promise<void>;
}): Promise<CustomPullBatchSyncResult> {
  const checkpoint: SyncCheckpoint = input.checkpoint === undefined
  ...
  const assembled = input.assembleDocument ? result.documents.map(input.assembleDocument) : result.documents;
...
export async function syncCustomPullIntoRepository(input: {
  ...
  let checkpoint: SyncCheckpoint = input.checkpoint === undefined
    ? normalizeCheckpoint(await input.checkpointStore?.readCustomPullCheckpoint())
    : normalizeCheckpoint(input.checkpoint);
  let hasMore = true;
  let batch = 1;
  ...
  while (hasMore) {
    ...
    const checkpointAdvanced =
      checkpointInstantMs(nextCheckpoint.updatedAtGmt) !== checkpointInstantMs(previousCheckpoint.updatedAtGmt) ||
