 ...
 * sinks are injected (this module stays dependency-free, in `@woo-rxdb-lab/shared`).
 */
 ...
 */
export function composeObservers(...observers: Array<SyncObserver | false | null | undefined>): SyncObserver {
...
export const DEFAULT_ADDITIVE_FIELDS: readonly string[] = [
  'requested', 'applied', 'count', 'docs', 'refetched', 'deleted', 'pulled', 'created', 'updated', 'bytes',
];
...
function bumpSum(map: Map<string, Map<string, number>>, key: string, field: string, value: number): void {
...
const nestedCountObj = (map: Map<string, Map<string, number>>): Record<string, Record<string, number>> =>
...
const nestedTimingObj = (map: Map<string, Map<string, TimingMetric>>): Record<string, Record<string, TimingMetric>> =>
    ...
    if (event.fields) {
      for (const [field, value] of Object.entries(event.fields)) {
        ...
        if (field === 'durationMs') {
          ...
          if (event.collection) bumpSum3(fieldSumsByCollection, event.type, event.collection, field, value);
        }
      }
    }
  };
 ...
 */
export function createLineLogger(sink: (line: string) => void, options: LineLoggerOptions = {}): SyncObserver {
