...
describe('resolveScan online outcomes', () => {
  it('resolves a parent product match', async () => {
    const match = { id: 11, type: 'product' as const, parent_id: 0, payload: { id: 11, sku: 'P-11' } };
    const fetcher = vi.fn<BarcodeResolveFetcher>(async () =>
      jsonResponse(resolveResponse({ found: true, match, meta: { duration_ms: 8, server_profile: 'good-local', candidates: 1 } })),
    );
    const events: ScanEvent[] = [];
    const result = await resolveScan(
    ...
    if (result.outcome === 'online') {
      expect(result.serverMeta).toEqual({ duration_ms: 8, server_profile: 'good-local', candidates: 1 });
    }
  ...
  it('surfaces ambiguous matches beyond the first', async () => {
    ...
    const ambiguous = [{ id: 6, type: 'product' as const }, { id: 7, type: 'variation' as const }];
    const fetcher = vi.fn<BarcodeResolveFetcher>(async () =>
    ...
    expect(events.map((event) => event.type)).toEqual(['searching-online', 'resolved-online', 'ambiguous']);
  });
...
describe('resolveScan timings (injected clock)', () => {
  it('measures feedback and resolution relative to scan start', async () => {
    ...
    const fetcher: BarcodeResolveFetcher = async () => {
      clock.advance(1_234);
      return jsonResponse(resolveResponse({ found: true, match: { id: 1, type: 'product', parent_id: 0, payload: {} } }));
    };
    const events: ScanEvent[] = [];
    const result = await resolveScan(
