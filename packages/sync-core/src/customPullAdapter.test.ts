...
import { normalizeCheckpoint, type PullResponse } from '@woo-rxdb-lab/shared';
import {
...
describe('pullCustomBatch', () => {
  it('posts checkpoint parameters and returns documents', async () => {
    ...
    server.seed({ uuid: fakeUuid(1), wooOrderId: 1, sequence: 1, modifiedGmt: '2026-05-19 00:00:00', revision: 'rev' });
    ...
    expect(fetcher).toHaveBeenCalledWith('http://wcpos.local/wp-json/wc-rxdb-sync/v1/orders/pull?limit=50&updated_at_gmt=1970-01-01T00%3A00%3A00.000Z&order_id=0&sequence=0');
    expect(result.documents).toHaveLength(1);
    expect(result.metrics?.duration_ms).toBe(7); // the fake's deterministic body metrics, surfaced verbatim
    expect(result.responseBytes).toBe(new TextEncoder().encode(server.responseBodies[0]).byteLength);
  });
  ...
  it('passes the abort signal to the custom pull fetcher', async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith(expect.stringContaining('/orders/pull?'), { signal: abortController.signal });
  });
...
describe('syncCustomPullIntoRepository', () => {
  it('pulls custom batches from wcpos and upserts each batch into the repository', async () => {
    ...
    const fetcher = vi.fn(server.fetch);
    const repository = { upsertMany: vi.fn(async (_documents: PullResponse['documents']) => undefined) };
    const onBatch = vi.fn();
    ...
    expect(repository.upsertMany.mock.calls.map(([documents]) => documents.map((document) => document.wooOrderId))).toEqual([[1], [2]]);
  ...
  it('forwards includeDeletes + the delete freshness hook to each batch (CodeRabbit Major)', async () => {
    ...
    const fetcher = vi.fn(server.fetch);
    const repository = { upsertMany: vi.fn(async (_documents: PullResponse['documents']) => undefined), removeDeletedOrders: vi.fn(async () => undefined) };
  ...
  it('deduplicates duplicate order IDs within one custom pull batch before bulk upsert', async () => {
    ...
    const fetcher = vi.fn().mockResolvedValue(response({
      ...
      checkpoint: { updatedAtGmt: '2026-05-20T10:05:00.000Z', orderId: 114, revision: 'new', sequence: 2 },
      hasMore: false,
    }));
    const repository = { upsertMany: vi.fn(async (_documents: PullResponse['documents']) => undefined) };
    const onBatch = vi.fn();
  ...
  it('resumes from the persisted checkpoint and stores the next checkpoint after a successful batch', async () => {
    const persistedCheckpoint = { updatedAtGmt: '2026-05-20T10:00:00.000Z', orderId: 10, revision: 'old', sequence: 10 };
    const server = createFakePullServer();
    server.seed({ uuid: fakeUuid(11), wooOrderId: 11, sequence: 11, modifiedGmt: '2026-05-20 10:05:00', revision: 'new' });
    const fetcher = vi.fn(server.fetch);
    const repository = { upsertMany: vi.fn(async (_documents: PullResponse['documents']) => undefined) };
    ...
    expect(fetcher).toHaveBeenCalledWith('http://wcpos.local/wp-json/wc-rxdb-sync/v1/orders/pull?limit=1&updated_at_gmt=2026-05-20T10%3A00%3A00.000Z&order_id=10&sequence=10');
    expect(repository.upsertMany.mock.calls.map(([documents]) => documents.map((document) => document.id))).toEqual([[fakeUuid(11)]]);
  ...
  it('does not store the returned checkpoint when the local upsert fails', async () => {
    ...
    const repository = { upsertMany: vi.fn(async () => { throw new Error('quota exceeded'); }) };
    const checkpointStore = {
      readCustomPullCheckpoint: vi.fn(async () => normalizeCheckpoint(null)),
      writeCustomPullCheckpoint: vi.fn(async () => undefined),
    };

    await expect(syncCustomPullIntoRepository({
      baseUrl: BASE_URL,
      limit: 1,
      repository,
      fetcher: server.fetch,
  ...
  it('treats one GMT instant in mixed timezone forms as a stall, not endless progress', async () => {
    ...
    server.script(() => ({
      kind: 'stall',
      checkpoint: { updatedAtGmt: sameInstantForms[page++], orderId: 114, revision: 'rev', sequence: 5 },
    }));
    const fetcher = vi.fn(server.fetch);
    const repository = { upsertMany: vi.fn(async (_documents: PullResponse['documents']) => undefined) };
  ...
  it('skips pulled documents with pending local mutations while applying the rest', async () => {
    ...
    expect(repository.upsertMany.mock.calls.map(([documents]) => documents.map((document) => document.id))).toEqual([[fakeUuid(8)]]);
    expect(result).toEqual({ batches: 1, documents: 1 });
  });
...
describe('syncCustomPullBatchIntoRepository journal epoch (F8)', () => {
  function epochStore(storedEpoch: string | undefined, storedCheckpoint = normalizeCheckpoint({ updatedAtGmt: '2026-05-19T00:00:00.000Z', orderId: 12, revision: 'r', sequence: 5000 })) {
    return {
  ...
  it('resyncs from zero when the stored epoch differs from the server epoch', async () => {
    ...
    const result = await syncCustomPullBatchIntoRepository({ baseUrl: BASE_URL, limit: 50, repository, fetcher: server.fetch, checkpointStore: store });
  ...
  it('reconciles the local collection on reset, using a fresh pending read (Codex P1)', async () => {
    const server = createFakePullServer({ epoch: 'epoch-NEW' });
    const repository = { upsertMany: vi.fn(async (_documents: PullResponse['documents']) => undefined), resetForResync: vi.fn(async () => undefined) };
    const store = epochStore('epoch-OLD', normalizeCheckpoint(null));
    const fresh = new Set<string | number>([777]);
    const refreshPendingMutationOrderIds = vi.fn(async () => fresh);

    await syncCustomPullBatchIntoRepository({ baseUrl: BASE_URL, limit: 50, repository, fetcher: server.fetch, checkpointStore: store, refreshPendingMutationOrderIds });
  ...
  it('advances normally when the epoch matches and the cursor is within the head', async () => {
    const server = createFakePullServer({ epoch: 'epoch-A' });
    server.seed({ uuid: fakeUuid(1), wooOrderId: 1, sequence: 5001, modifiedGmt: '2026-05-20 10:05:00', revision: 'r' });
    const repository = { upsertMany: vi.fn(async (_documents: PullResponse['documents']) => undefined) };
    ...
    expect(repository.upsertMany.mock.calls.map(([documents]) => documents.map((document) => document.id))).toEqual([[fakeUuid(1)]]); // applied — no reset
  ...
  it('adopts the epoch on a first pull (no stored epoch) without resyncing', async () => {
    ...
    expect(repository.upsertMany.mock.calls.map(([documents]) => documents.map((document) => document.id))).toEqual([[fakeUuid(1)]]);
...
describe('hostile-envelope poison guards (B7, ADR 0017 family)', () => {
  ...
  function trackingStore(initialSequence: number, epoch = 'epoch-A') {
    let stored = normalizeCheckpoint({ updatedAtGmt: '2026-05-19T00:00:00.000Z', orderId: 1, revision: 'r', sequence: initialSequence });
    return {
  ...
  it('rejects a response whose checkpoint sequence exceeds its own head, without applying or advancing', async () => {
    ...
    const fetcher = hostileFetcher({
      documents: [{ id: fakeUuid(1), wooOrderId: 1, payload: { id: 1 }, sync: { partial: false, checkpoint: { sequence: 5 } }, local: { dirty: false, pendingMutationIds: [] } }],
      checkpoint: { updatedAtGmt: '2026-05-20T00:00:00.000Z', orderId: 1, revision: 'r5', sequence: 5 },
      hasMore: true,
      epoch: 'epoch-A',
      head: 2,
    });
    ...
    })).rejects.toThrow(new CustomPullPoisonError('Custom pull poisoned: response checkpoint sequence 5 exceeds the reported journal head 2'));
  ...
  it('rejects a checkpoint that regresses below the request cursor within the same generation', async () => {
    ...
    const fetcher = hostileFetcher({
      documents: [],
      checkpoint: { updatedAtGmt: '2026-05-18T00:00:00.000Z', orderId: 3, revision: 'r3', sequence: 3 },
      hasMore: true,
      epoch: 'epoch-A',
      head: 12,
    });
    ...
    await expect(syncCustomPullBatchIntoRepository({
      ...
      fetcher,
      checkpointStore: store,
    })).rejects.toThrow(new CustomPullPoisonError('Custom pull poisoned: response checkpoint sequence 3 regressed below the request cursor sequence 10'));
  ...
  it('catches a regression even when the server reports no head or epoch', async () => {
    ...
    await expect(syncCustomPullBatchIntoRepository({
      ...
      repository,
      fetcher,
      checkpoint: { updatedAtGmt: '2026-05-19T00:00:00.000Z', orderId: 1, revision: 'r', sequence: 10 },
    })).rejects.toThrow(CustomPullPoisonError);
  ...
  it('allows a no-F8 fallback page that reports sequence zero below a nonzero cursor', async () => {
    ...
    const fetcher = hostileFetcher({
      documents: [],
      checkpoint: { updatedAtGmt: '1970-01-01T00:00:00.000Z', orderId: 0, revision: '', sequence: 0 },
      hasMore: true,
    });
    const repository = { upsertMany: vi.fn(async (_documents: PullResponse['documents']) => undefined) };
    const store = trackingStore(10);
...
describe('shouldApplyPulledDocument', () => {
  ...
  it('blocks pulled documents whose document id has a pending mutation', () => {
    expect(shouldApplyPulledDocument({ id: 'woo-order:204', wooOrderId: 204 }, pending)).toBe(false);
  });

  it('blocks pulled documents whose temp id has a pending mutation', () => {
    expect(shouldApplyPulledDocument({ id: 'local-order:sat-rush-r1-1', wooOrderId: null }, pending)).toBe(false);
  });

  it('blocks pulled documents whose woo order id has a pending mutation', () => {
    expect(shouldApplyPulledDocument({ id: 'woo-order:207', wooOrderId: 207 }, pending)).toBe(false);
  });
  ...
  it('applies pulled documents with no pending mutations', () => {
    ...
    expect(shouldApplyPulledDocument({ id: 'woo-order:999', wooOrderId: null }, pending)).toBe(true);
...
describe('syncCustomPullBatchIntoRepository pull guard wiring', () => {
  function seededServer() {
    const server = createFakePullServer();
    server.seed({ uuid: fakeUuid(11), wooOrderId: 11, payload: { status: 'completed' }, sequence: 11, modifiedGmt: '2026-06-10 10:00:00', revision: 'rev-11' });
    server.seed({ uuid: fakeUuid(12), wooOrderId: 12, sequence: 12, modifiedGmt: '2026-06-10 10:05:00', revision: 'rev-12' });
    return server;
  }
  const advancedCheckpoint = { updatedAtGmt: '2026-06-10 10:05:00', orderId: 12, revision: 'rev-12', sequence: 12 };
  ...
  it('filters pending documents out of the upsert but still advances the checkpoint', async () => {
    ...
    expect(repository.upsertMany.mock.calls.map(([documents]) => documents.map((document) => document.id))).toEqual([[fakeUuid(12)]]);
  ...
  it('applies every document when no pending mutation set is provided', async () => {
    ...
    expect(repository.upsertMany.mock.calls.map(([documents]) => documents.map((document) => document.id))).toEqual([[fakeUuid(11), fakeUuid(12)]]);
  ...
  it('maps each pulled record through the injected assembleDocument before dedup/upsert', async () => {
    ...
    expect(repository.upsertMany.mock.calls.map(([documents]) => documents.map((document) => document.id))).toEqual([['assembled:12']]);
  });
});
...
describe('syncCustomPullBatchIntoRepository delete channel (F6)', () => {
  ...
  it('no-ops gracefully when the repository cannot apply deletes', async () => {
    const server = serverWithDeleted([999]);
    const repository = { upsertMany: vi.fn(async (_documents: PullResponse['documents']) => undefined) }; // no removeDeletedOrders
