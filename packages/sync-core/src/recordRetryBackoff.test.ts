...
const NO_JITTER: RetryBackoffPolicy = {
	baseMs: 1_000,
	multiplier: 2,
	maxMs: 60_000,
	jitterRatio: 0,
...
describe('computeRetryBackoffMs', () => {
	...
	it('is deterministic — same inputs, same output', () => {
		expect(computeRetryBackoffMs(3, DEFAULT_RETRY_BACKOFF, 42)).toBe(
			computeRetryBackoffMs(3, DEFAULT_RETRY_BACKOFF, 42)
		);
	});

	it('never returns a negative delay', () => {
		expect(
			computeRetryBackoffMs(1, { baseMs: 1, multiplier: 2, maxMs: 10, jitterRatio: 1 }, 0)
	...
	it('treats a non-finite jitter ratio as no jitter (never NaN)', () => {
		...
		expect(
			computeRetryBackoffMs(2, { ...NO_JITTER, jitterRatio: Number.POSITIVE_INFINITY }, 7)
		).toBe(2_000);
	});
