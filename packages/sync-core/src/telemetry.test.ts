...
const ev = (type: string, over: Partial<SyncEvent> = {}): SyncEvent => ({ type, level: 'info', ...over });
...
describe('composeObservers', () => {
  ...
  it('isolates a throwing observer so others still receive the event (best-effort)', () => {
    const boom = vi.fn(() => { throw new Error('sink down'); });
    const ok = vi.fn();
  ...
  it('freezes the event before fan-out so a sink cannot corrupt it for later sinks', () => {
    ...
    composeObservers((e) => { received = e; })(ev('x', { fields: { a: 1 } }));
...
describe('createMetricsCollector', () => {
  ...
  it('aggregates durationMs into count/total/min/max timings, ignoring non-numeric', () => {
    ...
    expect(m.snapshot().timings['change-signal.poll']).toEqual({ count: 2, totalMs: 400, minMs: 100, maxMs: 300 });
  });
  ...
  it('reset zeros every tally', () => {
    ...
    m.reset();
    expect(m.snapshot()).toEqual({ events: {}, byCollection: {}, timings: {}, timingsByCollection: {}, fieldSums: {}, fieldSumsByCollection: {}, errors: 0 });
  });

  it('sums numeric fields (except durationMs) as throughput, by type and per collection', () => {
    const m = createMetricsCollector();
    m.observe(ev('apply.pull', { collection: 'products', fields: { requested: 50, applied: 48, durationMs: 100 } }));
    m.observe(ev('apply.pull', { collection: 'products', fields: { requested: 30, applied: 30, durationMs: 80 } }));
    m.observe(ev('apply.pull', { collection: 'variations', fields: { requested: 10, applied: 9 } }));
    const snap = m.snapshot();
  ...
  it('does NOT sum non-additive numeric fields like record ids — only recognized counters', () => {
    const m = createMetricsCollector();
    m.observe(ev('apply.escalation', { level: 'warn', collection: 'products', fields: { id: 80, count: 1 } }));
    m.observe(ev('apply.escalation', { level: 'warn', collection: 'products', fields: { id: 99, count: 1 } }));
    const snap = m.snapshot();
...
describe('formatSyncEventLine', () => {
  it('renders level + type, then optional collection + message + fields in order', () => {
    ...
    expect(formatSyncEventLine(ev('pull.batch', { collection: 'orders' }))).toBe('[info] pull.batch orders');
    expect(formatSyncEventLine(ev('push.outcome', { level: 'warn', collection: 'products', message: 'partial' })))
      .toBe('[warn] push.outcome products partial');
    expect(formatSyncEventLine(ev('x', { fields: { count: 50, ok: true, durationMs: 120 } })))
      .toBe('[info] x count=50 ok=true durationMs=120');
  });

  it('quotes field values that need it, and renders objects/null/undefined safely', () => {
    expect(formatSyncEventLine(ev('x', { fields: { id: 'woo-order:1' } }))).toBe('[info] x id=woo-order:1');
    expect(formatSyncEventLine(ev('x', { fields: { msg: 'two words' } }))).toBe('[info] x msg="two words"');
    // an object whose JSON has internal whitespace is quoted as one token
    expect(formatSyncEventLine(ev('x', { fields: { obj: { m: 'a b' }, n: null, u: undefined } })))
      .toBe('[info] x obj="{\\"m\\":\\"a b\\"}" n=null u=undefined');
  });

  it('quotes a multi-word collection/message so they stay single tokens', () => {
    expect(formatSyncEventLine(ev('push.outcome', { level: 'warn', collection: 'products', message: 'partial failure', fields: { status: 'error' } })))
...
describe('createLineLogger', () => {
  ...
  it('composes with a metrics collector — one emit feeds both', () => {
    ...
    const observe = composeObservers(metrics.observe, createLineLogger((l) => lines.push(l)));
  ...
  it('isolates a throwing host sink even when used as the sole observer', () => {
    const logger = createLineLogger(() => { throw new Error('host log down'); });
...
describe('NOOP_OBSERVER hot-path tripwire', () => {
  it('accepts 10k typical events within a generous CI bound', () => {
    const event = ev('pull.batch', { collection: 'orders', fields: { documents: 50, requests: 1, durationMs: 12 } });
