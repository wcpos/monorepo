...
describe('mirrorRecordUuid', () => {
  ...
  it('dedupes duplicate uuid entries down to one canonical entry', () => {
    expect(mirrorRecordUuid([uuidEntry(U_SERVER), uuidEntry(U_MINT), { key: 'x', value: 1 }], U_SERVER))
...
describe('resolveRecordIdentity', () => {
  ...
  it('LOCAL-BORN with no meta_data at all: mints + mirrors', () => {
    expect(resolveRecordIdentity({ mintUuid })).toMatchObject({ id: U_MINT, origin: 'minted', metaData: [uuidEntry(U_MINT)] });
  });

  it('EXISTING key takes precedence (never re-keyed); a legacy non-uuid key is accepted + mirrored', () => {
    const result = resolveRecordIdentity({ currentId: 'woo-order:123', metaData: [{ key: 'x', value: 1 }], mintUuid });
  ...
  it('is idempotent: re-resolving a resolved record yields the same id and stable meta', () => {
    ...
    const second = resolveRecordIdentity({ currentId: first.id, metaData: first.metaData, mintUuid });
  ...
  it('mintOnMissing:false throws when a record carries no key or valid uuid (server pull contract)', () => {
    expect(() => resolveRecordIdentity({ metaData: [uuidEntry('not-a-uuid')], mintUuid, mintOnMissing: false }))
      .toThrow(/minting is disabled/);
    expect(() => resolveRecordIdentity({ mintUuid, mintOnMissing: false })).toThrow(/minting is disabled/);
  });

  it('mintOnMissing:false still resolves a server-born or keyed record', () => {
    expect(resolveRecordIdentity({ metaData: [uuidEntry(U_SERVER)], mintUuid, mintOnMissing: false }).id).toBe(U_SERVER);
    expect(resolveRecordIdentity({ currentId: 'woo-order:9', mintUuid, mintOnMissing: false }).id).toBe('woo-order:9');
  });

  it('throws on an identity conflict — key disagrees with a valid server uuid (never re-keyed)', () => {
    expect(() => resolveRecordIdentity({ currentId: 'woo-order:1', metaData: [uuidEntry(U_SERVER)], mintUuid }))
      .toThrow(/identity conflict/);
  });

  it('does NOT conflict when key matches the server uuid', () => {
    const result = resolveRecordIdentity({ currentId: U_SERVER, metaData: [uuidEntry(U_SERVER)], mintUuid });
    expect(result.id).toBe(U_SERVER);
    expect(result.origin).toBe('existing');
  });
});
...
describe('identifyRecord', () => {
  ...
  it('SERVER-BORN payload: keys off the meta uuid, preserves other fields, mirrors meta', () => {
    const payload = { name: 'Widget', price: '9.99', meta_data: [{ key: 'x', value: 1 }, uuidEntry(U_SERVER)] };
    const result = identifyRecord(payload, { mintUuid });
    expect(result.id).toBe(U_SERVER);
    expect(result.origin).toBe('server-meta');
    expect(result.payload).toEqual({ name: 'Widget', price: '9.99', meta_data: [{ key: 'x', value: 1 }, uuidEntry(U_SERVER)] });
  });
  ...
  it('handles a payload with no meta_data field (local-born)', () => {
    ...
    expect(result).toMatchObject({ id: U_MINT, origin: 'minted', payload: { name: 'X', meta_data: [uuidEntry(U_MINT)] } });
  });

  it('mintOnMissing:false: a server pull missing its identity throws (no divergent mint)', () => {
    expect(() => identifyRecord({ name: 'X', meta_data: [] }, { mintUuid, mintOnMissing: false }))
      .toThrow(/minting is disabled/);
  });
...
describe('webCryptoUuid', () => {
  it('returns a v4-shaped uuid string', () => {
    expect(webCryptoUuid()).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
  });
});
