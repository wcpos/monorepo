...
import { pushEndpointResolver, pushRecordMutation, reconcileCreateAck, type PushResult } from './recordPushAdapter';
...
let n = 0;
const deps = { mintUuid: () => `00000000-0000-4000-8000-${String(++n).padStart(12, '0')}`, now: () => `2026-06-26T00:00:0${n}.000Z` };
...
const fakeServer = (assignedId: number, revision: string) =>
  vi.fn(async (_url: string, init?: RequestInit) => {
    ...
    const body = { document: { id: assignedId, meta_data: [{ key: RECORD_UUID_META_KEY, value: env.recordId }] }, currentRevision: revision };
...
describe('write path integration', () => {
  it('build → enqueue → drain → push → reconcile, with identity + revision round-trip', async () => {
    ...
    const reconciled: Array<{ recordId: string; remoteId: unknown; revision: string | null }> = [];
    ...
    expect(fetcher.mock.calls[0][0]).toBe('https://shop.example/wp-json/wc-rxdb-sync/v1/push/customers');
    ...
    expect(reconciled).toEqual([{ recordId: create.recordId, remoteId: 555, revision: 'sha256:rev1' }]);
  });
  ...
  it('an update uses the prior revision as baseRevision and conflicts are surfaced, not acknowledged', async () => {
    ...
    const update = buildUpdateMutation(
      { collectionName: 'products', recordId: '5b8e1a3c-2f4d-4a6b-9c8e-1d2f3a4b5c6d', payload: { regular_price: '9' }, baseRevision: 'sha256:rev1' },
      deps,
    );
    ...
    const conflictFetcher = vi.fn(async () =>
      ({ status: 409, ok: false, json: async () => ({ code: 'woo_rxdb_sync_conflict', current: { id: 9 }, currentRevision: 'sha256:rev2' }) }) as unknown as Response,
    );

    const result = await drainMutationQueue({ queue, push: (m) => pushRecordMutation({ mutation: m, resolveEndpoint: resolve, fetcher: conflictFetcher }) });
