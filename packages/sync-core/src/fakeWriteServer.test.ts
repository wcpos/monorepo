...
let seq = 0;
const deps = { mintUuid: () => `00000000-0000-4000-8000-${String(++seq).padStart(12, '0')}`, now: () => `2026-07-01T00:00:${String(seq).padStart(2, '0')}.000Z` };
const resolve = pushEndpointResolver('https://shop.example/wp-json');
const push = (server: ReturnType<typeof createFakeWriteServer>, mutation: Parameters<typeof pushRecordMutation>[0]['mutation']) =>
...
describe('createFakeWriteServer (faithful write-contract harness)', () => {
  it('echoes the client uuid back and assigns a stable numeric id per record', async () => {
    ...
    const create = buildCreateMutation({ collectionName: 'customers', payload: { email: 'a@b.c' } }, deps);

    const created = await push(server, create);

    expect(created.outcome).toBe('created');
    expect(created.document?.id).toBe(555);
    expect(created.document?.meta_data?.[0]).toEqual({ key: RECORD_UUID_META_KEY, value: create.recordId });
    expect(typeof created.currentRevision).toBe('string');
    expect(server.applied.get(create.recordId)).toEqual({ id: 555, revision: created.currentRevision });

    // A later update of the SAME record (anchored on the returned revision) keeps the id, advances the revision.
    const update = buildUpdateMutation({ collectionName: 'customers', recordId: create.recordId, payload: { email: 'x@y.z' }, baseRevision: created.currentRevision! }, deps);
  ...
  it('assigns a fresh id per distinct record', async () => {
    ...
    const a = await push(server, buildCreateMutation({ collectionName: 'orders', payload: {} }, deps));
    const b = await push(server, buildCreateMutation({ collectionName: 'orders', payload: {} }, deps));
  ...
  it('deliberately returns 201 for a memoized create replay (#526)', async () => {
    ...
    const init = { method: 'POST', body: JSON.stringify({ ...create, collection: create.collectionName }) };
  ...
  it('404s an update to a record the server does not know', async () => {
    seq = 0;
    const server = createFakeWriteServer();
    const update = buildUpdateMutation({ collectionName: 'products', recordId: '5b8e1a3c-2f4d-4a6b-9c8e-1d2f3a4b5c6d', payload: {}, baseRevision: 'sha256:whatever' }, deps);
  ...
  it('409s an update whose baseRevision is stale, and succeeds once seeded with the matching revision', async () => {
    ...
    const stale = buildUpdateMutation({ collectionName: 'orders', recordId: 'uuid-seeded', payload: {}, baseRevision: 'sha256:stale' }, deps);
    const conflict = await push(server, stale);
    expect(conflict.outcome).toBe('conflict');
    expect(conflict.conflict?.currentRevision).toBe('sha256:current');

    const fresh = buildUpdateMutation({ collectionName: 'orders', recordId: 'uuid-seeded', payload: {}, baseRevision: 'sha256:current' }, deps);
  ...
  it('400s a doubled-namespace URL — the endpoint guard is real (URL is validated, not just the body)', async () => {
    ...
    await expect(pushRecordMutation({ mutation: create, resolveEndpoint: doubledResolve, fetcher: server.fetch })).rejects.toMatchObject({ status: 400 });
  ...
  it('scripts a 409 conflict surfaced by the push adapter', async () => {
    ...
    server.script((env: PushEnvelope) => (env.operation === 'update' ? { kind: 'conflict', currentRevision: 'sha256:theirs' } : undefined));
    const update = buildUpdateMutation({ collectionName: 'products', recordId: '5b8e1a3c-2f4d-4a6b-9c8e-1d2f3a4b5c6d', payload: { regular_price: '9' }, baseRevision: 'sha256:mine' }, deps);
  ...
  it('scripts a 409 identity_ambiguous in the real WP_Error byte-shape (F4a fail-closed)', async () => {
    ...
    const raw = await server.fetch('https://shop.example/wp-json/wc-rxdb-sync/v1/push/products', {
      method: 'POST',
      body: JSON.stringify({ mutationId: 'm-amb', operation: 'update', collection: 'products', recordId: 'uuid-dup', baseRevision: 'sha256:current', payload: {} }),
    });
    expect(raw.status).toBe(409);
    expect(await raw.json()).toEqual({
      code: 'woo_rxdb_sync_identity_ambiguous',
      message: 'uuid uuid-dup resolves to more than one record; refusing to write to an arbitrary match.',
      data: { status: 409 },
    });
  });
  ...
  it('scripted identity_ambiguous is classified PERMANENT by the push adapter (dead-letter, not conflict/retry)', async () => {
    ...
    await expect(push(server, update)).rejects.toMatchObject({ status: 409, reason: 'identity-ambiguous', permanent: true });
  });
  ...
  it('scripts a 428 precondition_required on a delete as a thrown retryable 428 (routes to the drain refresh recovery — #516 item 4)', async () => {
    ...
    const del = { mutationId: 'm-del', operation: 'delete' as const, collectionName: 'orders', recordId: 'uuid-del', origin: 'existing' as const, payload: { id: 'uuid-del' }, baseRevision: 'sha256:base', queuedAt: '2026-07-01T00:00:00.000Z' };
    ...
    await expect(push(server, del)).rejects.toMatchObject({ status: 428, permanent: false, reason: 'woo_rxdb_sync_precondition_required' });
  });

  describe('ADR 0011 header mirror (faithful to Header_Mirror::assert)', () => {
    const envelope = (over: Partial<PushEnvelope> = {}): string =>
      JSON.stringify({ mutationId: 'm-1', operation: 'create', collection: 'orders', recordId: 'uuid-m1', baseRevision: null, payload: {}, ...over });
    ...
    it('422s when Idempotency-Key disagrees with the body mutationId', async () => {
      ...
      expect(((await response.json()) as { code: string }).code).toBe('woo_rxdb_sync_header_body_mismatch');
    });
    ...
    it('422s when If-Match disagrees with the body baseRevision', async () => {
      ...
      const response = await server.fetch(url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Idempotency-Key': 'm-1', 'If-Match': '"sha256:other"' },
    ...
    it('accepts a quoted If-Match entity-tag and a W/-weakened one when the bare revision matches', async () => {
      ...
      const quoted = await server.fetch(url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Idempotency-Key': 'm-1', 'If-Match': '"sha256:current"' },
        body: envelope({ operation: 'update', baseRevision: 'sha256:current' }),
      });
      ...
      const weakened = await server.fetch(url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Idempotency-Key': 'm-2', 'If-Match': 'W/"sha256:r2"' },
    ...
    it('the client push adapter sends the mirror headers the fake server cross-checks (regression tripwire)', async () => {
      ...
      const pushWithHeaderTripwire = (mutation: Parameters<typeof pushRecordMutation>[0]['mutation']) =>
      ...
      const update = buildUpdateMutation({ collectionName: 'orders', recordId: create.recordId, payload: {}, baseRevision: created.currentRevision! }, deps);
