...
function change(id: number, collection: HybridChange['collection'], type = 'product.updated'): HybridChange {
...
describe('planReplicationActions — routine TIER 1 changes', () => {
  it('routes non-delete changes into targetedPulls grouped by collection', () => {
    const actions = planReplicationActions(
      baseOutcome({ changes: [change(10, 'products'), change(11, 'products'), change(20, 'variations')] }),
    );
...
describe('planReplicationActions — delete tombstones', () => {
  it('routes delete-type changes into deletes, NOT targetedPulls', () => {
    const actions = planReplicationActions(
      baseOutcome({ changes: [change(50, 'products', 'product.deleted'), change(51, 'products', 'product.trashed')] }),
    );
  ...
  it('a delete excludes the same id from targetedPulls when both appear', () => {
    const actions = planReplicationActions(
      baseOutcome({
        changes: [change(70, 'products', 'product.updated'), change(70, 'products', 'product.deleted')],
      }),
    );
  ...
  it('a stale collection WITH configBarcodeFields routes to reDeriveBarcode', () => {
    const actions = planReplicationActions(
      baseOutcome({
        ...
        configBarcodeFields: { products: ['sku', 'global_unique_id'], variations: [], tax_rates: [] },
      }),
    );
    expect(actions.reDeriveBarcode).toEqual([{ collection: 'products', activeFields: ['sku', 'global_unique_id'] }]);
...
describe('planReplicationActions — multi-collection grouping', () => {
  it('groups and dedupes across collections from both changes and drill-down', () => {
    const actions = planReplicationActions(
      baseOutcome({
        changes: [change(1, 'products'), change(2, 'variations'), change(3, 'tax_rates', 'tax_rate.updated')],
        idsToPull: [repair(2, 'variations'), repair(4, 'products'), repair(5, 'tax_rates', 'changed', 'range-checksum')],
      }),
    );
    expect(actions.targetedPulls).toEqual([
