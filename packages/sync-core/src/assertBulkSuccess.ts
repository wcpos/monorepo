...
export function assertBulkSuccess<T>(result: T, context: string): T {
  ...
  const ids = failures.map((failure, index) => failure.documentId
    ?? String(failure.writeRow?.document?.id ?? failure.writeRow?.document?.mutationId ?? `row-${index + 1}`));
  const error = Object.assign(new Error(`${context} failed for ${failures.length} row(s): ${ids.join(', ')}`), {
    failedIds: ids,
