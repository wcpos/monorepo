...
async function runPullUnderBound(
	...
	const guardedCheckpointStore: CustomPullCheckpointStore = {
		...
		writeCustomPullCheckpoint: async (checkpoint) => {
			const result = await guardWrite(() => store.writeCustomPullCheckpoint(checkpoint));
			if (result === 'dropped') {
				dropped = true;
			}
		},
	...
	const pendingMutationOrderIds =
		typeof rawPending === 'function' ? await rawPending() : rawPending;
	try {
