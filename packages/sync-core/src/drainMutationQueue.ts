...
export async function drainMutationQueue(input: {
  ...
  const batch = scanned.filter((mutation) => mutation.status === undefined || mutation.status === 'pending' || mutation.status === 'claimed');
  ...
  const blockedRecords = new Set<string>(
    scanned.filter((mutation) => mutation.status === 'conflicted' || mutation.status === 'needs-revision').map((mutation) => mutation.recordId),
  );
  ...
  const applyBackoff = async (mutation: QueuedMutation): Promise<void> => {
    ...
    try {
      await input.queue.reschedule({ ...mutation, attempts, nextAttemptAt: new Date(now() + delayMs).toISOString() });
    } catch {
      ...
      emit({ type: 'queue.write.reschedule-failed', level: 'warn', collection: mutation.collectionName, fields: { recordId: mutation.recordId, mutationId: mutation.mutationId, attempts } });
    }
  };
  ...
  const parkNeedsRevision = async (mutation: QueuedMutation): Promise<void> => {
    conflicts.push({ outcome: 'conflict', mutation, document: null, currentRevision: null, conflict: { current: null, currentRevision: null } });
    await input.queue.replace({ ...mutation, status: 'needs-revision' });
    emit({ type: 'queue.write.needs-revision', level: 'warn', collection: mutation.collectionName, fields: { recordId: mutation.recordId, mutationId: mutation.mutationId } });
  ...
  const deadLetter = async (mutation: QueuedMutation, error: unknown): Promise<void> => {
    ...
    emit({ type: 'push.rejected', level: 'warn', collection: mutation.collectionName, fields: { recordId: mutation.recordId, mutationId: mutation.mutationId, status: (error as { status?: number }).status } });
    try {
  ...
  for (const mutation of batch) {
    ...
    const freshRevision = mutation.operation === 'create' ? undefined : await input.currentRevision?.(mutation);
    const draining = {
      ...mutation,
    ...
    if (result.outcome === 'conflict') {
      ...
      emit({ type: 'queue.write.conflict-transition', level: 'warn', collection: mutation.collectionName, fields: { recordId: mutation.recordId, mutationId: mutation.mutationId } });
  ...
  emit({ type: 'queue.write.drain', level: 'info', fields: { scanned: batch.length, attempted, pushed, deferred, conflicts: conflicts.length, failed, rejected: rejected.length } });
