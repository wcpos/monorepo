...
export function createConfigChangeSignal(input: {
  ...
  async function pollCommitting(): Promise<ConfigChangePollOutcome> {
    ...
    return { staleCollections, changed, barcodeFields: snapshot.barcodeFields, baseline: { ...baseline } };
  }
  ...
  async function pollDeferring(): Promise<DeferredConfigPoll> {
    ...
    return {
      ...
      outcome: { staleCollections, changed, barcodeFields: snapshot.barcodeFields, baseline: { ...nextBaseline } },
