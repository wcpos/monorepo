...
export function withOrderColumns<T extends { payload: WooOrderPayload }>(doc: T): T & PromotedOrderColumns {
...
export function withProductColumns<T extends { payload: WooProductPayload }>(doc: T): T & PromotedProductColumns {
...
export type ServerPhaseMetrics = Partial<Record<
  | 'candidate_query_ms'
...
export function normalizeCheckpoint(input: Partial<SyncCheckpoint> | null | undefined): SyncCheckpoint {
  return {
