...
export type ReplicationActionHandlers = {
   ...
   * a false success.
   */
  applyBarcodeIndex(collection: HybridCollection, index: RebuildBarcodeIndexResult): Promise<boolean> | boolean;
  /**
...
function idsFor(
  groups: ReadonlyArray<{ collection: HybridCollection; ids: number[] }>,
 ...
 */
function referenceRefreshNeeded(actions: ReplicationActions, collection: ReferenceCollection): boolean {
  return (
...
export async function applyReplicationActions(
  ...
  const emitCount = (type: string, collection: HybridCollection, requested: number, applied: number): void =>
    emit({ type, level: applied < requested ? 'warn' : 'info', collection, fields: { requested, applied } });
  ...
  for (const target of actions.reDeriveBarcode) {
    ...
    if (rebuilt.staleCollection) {
      ...
      emit({ type: 'apply.refetch', level: 'info', collection: target.collection, fields: { refetched, reason: 'barcode-field-absent' } });
    } else {
      ...
      if (applied) {
        ...
        emit({ type: 'apply.barcode-rederive', level: 'info', collection: target.collection, fields: { docs: docs.length, applied: true } });
      } else {
        ...
        reDerived.push({ collection: target.collection, rederived: false });
        emit({ type: 'apply.barcode-rederive', level: 'warn', collection: target.collection, fields: { docs: docs.length, applied: false } });
      }
    }
  }
  ...
  for (const collection of actions.reFetchCollections) {
    ...
    log(`change-signal: ${collection} config stale (no barcode field) — re-fetch of ${refetched} id(s)`);
    emit({ type: 'apply.refetch', level: 'info', collection, fields: { refetched, reason: 'config-stale' } });
  }
