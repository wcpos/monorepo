...
function createFakeLongTaskObserverCtor(options: { supportedEntryTypes?: readonly string[]; observeThrows?: boolean; pendingRecords?: number[] } = {}) {
  const state = {
    instances: 0,
...
describe('installHiddenPageIdlePolyfill', () => {
  ...
  it('hidden calls run unthrottled via microtasks; cancellation works', async () => {
    ...
    let ran = 0;
    target.requestIdleCallback(() => { ran += 1; });
    const handle = target.requestIdleCallback(() => { ran += 100; });
  ...
  it('delegates to the captured native callback while visible (codex review)', () => {
    ...
    const win = {
      ...
      clearTimeout,
      queueMicrotask,
      requestIdleCallback: () => { nativeCalls += 1; return 7; },
      cancelIdleCallback: () => {},
