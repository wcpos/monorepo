...
type RecordMutationPayload = { key: string; value: unknown }[];
...
describe('buildCreateMutation', () => {
	it('mints a born-local recordId, mirrors it into meta_data, and uses a SEPARATE mutationId', () => {
		const m = buildCreateMutation(
			{ collectionName: 'products', payload: { name: 'Widget' } },
			counterDeps()
		);
		...
		const m = buildCreateMutation(
			{
				...
				payload: { total: '9.99', meta_data: [{ key: RECORD_UUID_META_KEY, value: UUID_A }] },
			},
			counterDeps()
		);
	...
	it('honors an explicit currentId (existing origin) and still awaits the server ack', () => {
		const m = buildCreateMutation(
			{ collectionName: 'customers', payload: { email: 'a@b.c' }, currentId: UUID_A },
			counterDeps()
		);
			...
			buildUpdateMutation(
				{
					collectionName: 'orders',
					recordId: UUID_A,
					payload: { meta_data: [{ key: RECORD_UUID_META_KEY, value: uuidOf(99) }] },
					baseRevision: null,
				},
				counterDeps()
			)
...
describe('buildDeleteMutation', () => {
	it('targets an existing record by uuid and carries the required baseRevision precondition', () => {
		const m = buildDeleteMutation(
			{ collectionName: 'tax_rates', recordId: UUID_A, baseRevision: 'sha256:rev-A' },
			counterDeps()
		);
