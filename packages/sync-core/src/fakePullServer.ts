...
const FAKE_PHASES = {
	candidate_query_ms: 1,
	serialize_documents_ms: 2,
	assemble_response_ms: 3,
} as const;
...
export function createFakePullServer(options: FakePullServerOptions = {}): FakePullServer {
	...
	const headSequence = (): number => journal.reduce((head, row) => Math.max(head, row.sequence), 0); // class-sync-index.php:125-132
	...
	const nextSequence = (explicit?: number): number => {
		...
		if (explicit !== undefined) {
			if (explicit <= head) {
				throw new Error(
					`fakePullServer: sequence ${explicit} must exceed the journal head ${head} (append-only)`
				);
			}
			return explicit;
		}
	...
	const htmlResponse = (status: number): Response =>
		new Response(WP_HTML_ERROR_PAGE, {
			status,
	...
	const echoEnvelope = (
		...
		const echoedSequence =
			typeof checkpoint.sequence === 'number' ? checkpoint.sequence : request.sequence;
		return {
			documents: [],
			deletes: [],
			checkpoint,
	...
	const contractEnvelope = (request: FakePullRequest): Record<string, unknown> => {
		...
		for (const row of page) {
			...
			const baseMeta = Array.isArray(state.payload['meta_data'])
				? (state.payload['meta_data'] as MetaDataEntry[])
				: [];
			const fullPayload: Record<string, unknown> = {
				...
				meta_data:
					readValidUuidFromMeta(baseMeta) === state.uuid
						? baseMeta
						: [
								...baseMeta.filter((entry) => entry?.key !== RECORD_UUID_META_KEY),
								{ key: RECORD_UUID_META_KEY, value: state.uuid },
							],
			};
			const isPartial = request.orderFields !== null;
			const payload = isPartial
				? filterPayloadFields(fullPayload, request.orderFields as string[])
				: fullPayload;
			const revision = row.revision;
			// A payload date_modified_gmt wins the document checkpoint's updatedAtGmt (:178).
			const modified =
				typeof fullPayload['date_modified_gmt'] === 'string'
					? (fullPayload['date_modified_gmt'] as string)
					: row.modifiedGmt;
	...
	const parseRequest = (url: string): FakePullRequest | null => {
		...
		return {
			...
			benchmarkProfile:
				profileRaw === 'slow-php' ||
				profileRaw === 'slow-db' ||
				profileRaw === 'large-payload' ||
				profileRaw === 'good-local'
					? profileRaw
					: 'good-local',
		};
	};
	...
	return {
		...
		seed: (order) => {
			...
			if (!isUuid(order.uuid)) {
				throw new Error(
					`fakePullServer: seed uuid ${JSON.stringify(order.uuid)} is not a uuid — use fakeUuid(n)`
				);
			}
				...
				switch (fault.kind) {
					...
					case 'stall':
						return envelopeResponse(
							request,
							echoEnvelope(request, true, { checkpoint: fault.checkpoint, raiseHeadToCursor: true })
						);
					case 'empty':
