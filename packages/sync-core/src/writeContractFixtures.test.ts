...
describe('version 1 write-contract golden fixtures', () => {
	it('validates every golden envelope and response against the contract root', () => {
		...
		for (const fixture of validFixtures)
			expect(validate(fixture.envelope), fixture.name).toBe(true);
		for (const fixture of responseFixtures) expect(validate(fixture.body), fixture.name).toBe(true);
	});

	it.each(validFixtures)(
		'recordPushAdapter emits the exact $name body and header mirrors',
		async (fixture) => {
			...
			const fetcher = vi.fn(async (_url: string, _init?: RequestInit) =>
				response(
					...
					mutation.operation === 'delete'
						? {}
						: { document: { id: 1 }, currentRevision: 'sha256:ack' }
				)
			);
	...
	it.each(responseFixtures)(
		'parses the $name response envelope into durable push types',
		async (fixture) => {
			const operation: RecordMutation['operation'] =
				fixture.name === 'deleted' || fixture.name === 'delete-precondition'
					? 'delete'
					: fixture.name === 'created' || fixture.name === 'replayed-create'
						? 'create'
						: 'update';
			const mutation = mutationFrom({
				...validFixtures[0],
				envelope: { ...validFixtures[0].envelope, operation },
			} as WireFixture);
			const promise = pushRecordMutation({
				mutation,
				resolveEndpoint: () => ({ url: 'https://fixture.invalid', method: 'POST' }),
	...
	it('pins the exact delete-success and stale-conflict wire shapes', () => {
		expect(responseFixtures.find(({ name }) => name === 'replayed-create')).toMatchObject({
			status: 201,
			body: { document: { id: 41 } },
		});
