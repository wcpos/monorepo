...
import { createCategoryLoggerObserver, type CategoryLoggerLike } from './categoryLoggerObserver';

function fakeLogger() {
  const calls: Array<{ category: string; context: Record<string, unknown>; level: string; message: unknown; options?: unknown }> = [];
  const make = (category = 'root', context: Record<string, unknown> = {}): CategoryLoggerLike => ({
    ...
    error: (message, options) => calls.push({ category, context, level: 'error', message, options }),
  });
...
describe('createCategoryLoggerObserver', () => {
  it('routes collections, preserves the canonical type, and maps every level', () => {
    ...
    observe({ type: 'pull.batch', level: 'debug', collection: 'orders', message: 'pulled', fields: { type: 'spoofed', count: 2 } });
  ...
  it('passes debug text lazily and unevaluated', () => {
    const { root, calls } = fakeLogger();
    createCategoryLoggerObserver(root)({ type: 'pull.batch', level: 'debug', message: 'debug text' });
