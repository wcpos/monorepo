			...
			case 'conflict':
				return {
					...
					body: {
						...
						current: fault.current ?? null,
						currentRevision: fault.currentRevision ?? null,
					},
				};
			case 'in_progress':
	...
	return {
		...
		fetch: async (url: string, init?: RequestInit) => {
			...
			if (headerMutationId !== null && headerMutationId !== env.mutationId) {
				return asResponse(422, {
					code: 'woo_rxdb_sync_header_body_mismatch',
					message: 'Idempotency-Key header disagrees with body mutationId.',
				});
			}
			...
			if (headerBaseRevision !== null && headerBaseRevision !== bodyBaseRevision) {
				return asResponse(422, {
					code: 'woo_rxdb_sync_header_body_mismatch',
					message: 'If-Match header disagrees with body baseRevision.',
				});
			}
