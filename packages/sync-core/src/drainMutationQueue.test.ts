...
import type { SyncEvent } from '@woo-rxdb-lab/shared';
...
const ok = (mutation: RecordMutation): PushResult => ({ outcome: 'updated', mutation, document: { id: 1 }, currentRevision: 'sha256:r1' });
const conflict = (mutation: RecordMutation): PushResult => ({ outcome: 'conflict', mutation, document: null, currentRevision: null, conflict: { current: { id: 1 }, currentRevision: 'r1' } });
...
describe('drainMutationQueue', () => {
  it('pushes each pending mutation, applies the ack, and acknowledges on success', async () => {
    const q = await queueWith(mut({ mutationId: 'm1', queuedAt: '..1' }), mut({ mutationId: 'm2', recordId: 'rec-B', queuedAt: '..2' }));
  ...
  it('leaves a mutation queued when the push throws a retryable error (retry next drain)', async () => {
    const q = await queueWith(mut({ mutationId: 'm1' }));
    const result = await drainMutationQueue({ queue: q, push: async () => { throw new Error('network'); } });
  ...
  it('dead-letters a non-retryable 4xx (e.g. unsupported collection) instead of retrying forever', async () => {
    ...
    const result = await drainMutationQueue({ queue: q, push: async () => { throw err; }, observe: (e) => events.push(e) });
  ...
  it('dead-letters a 409 identity_ambiguous (F4a fail-closed) through the real adapter + write fake — not retried, not dropped silently', async () => {
    ...
    const q = await queueWith(mut({ mutationId: 'm-amb', recordId: 'rec-dup', baseRevision: 'sha256:mine' }));
    ...
    const result = await drainMutationQueue({
      queue: q,
      push: (mutation) => pushRecordMutation({
        mutation,
  ...
  it('gate2 item 4: a delete-428 with no refresh port parks as durable needs-revision — not dead-lettered, not a fake conflict', async () => {
    const q = await queueWith(mut({ mutationId: 'm-delete', operation: 'delete', baseRevision: null }));
    const events: SyncEvent[] = [];
    const fetcher = vi.fn(async () => jsonResponse(428, { code: 'woo_rxdb_sync_precondition_required' }));
    const result = await drainMutationQueue({
      queue: q,
      push: (mutation) => pushRecordMutation({
        mutation,
  ...
  it('gate2 item 4: a delete-428 routes through refreshRevision and the restamped retry lands (the real adapter + write fake)', async () => {
    const q = await queueWith(mut({ mutationId: 'm-del-428', operation: 'delete', recordId: 'rec-del', collectionName: 'products', baseRevision: null }));
    const server = createFakeWriteServer();
  ...
  it('re-pulls and re-stamps a null-base update after 428, then retries it once', async () => {
    const q = await queueWith(mut({ mutationId: 'm-428', operation: 'update', baseRevision: null }));
    const push = vi.fn(async (mutation: RecordMutation) => {
      if (push.mock.calls.length === 1) throw Object.assign(new Error('revision required'), { status: 428 });
      return ok(mutation);
    });
  ...
  it('gate2 item 4: parks a 428 as needs-revision when a targeted re-pull still finds no revision', async () => {
    const q = await queueWith(mut({ mutationId: 'm-428-none', operation: 'update', baseRevision: null }));
    const result = await drainMutationQueue({
      queue: q,
      push: async () => { throw Object.assign(new Error('revision required'), { status: 428 }); },
  ...
  it('gate2 item 4: parks as needs-revision when the post-refresh retry 428s again (no same-base retry loop)', async () => {
    const q = await queueWith(mut({ mutationId: 'm-428-again', operation: 'update', baseRevision: null }));
    const push = vi.fn(async () => { throw Object.assign(new Error('revision required'), { status: 428 }); });
    const result = await drainMutationQueue({ queue: q, push, refreshRevision: async () => 'sha256:repulled' });

    expect(push).toHaveBeenCalledTimes(2); // one refresh-restamped retry, then park — never a loop
    expect(result.conflicts).toHaveLength(1);
    expect((await q.all())[0]).toMatchObject({ status: 'needs-revision', baseRevision: 'sha256:repulled' });
  });
  ...
  it('backs off a mutation when its 428 refresh throws, continues draining, and defers it on the next tick', async () => {
    ...
    const first = await drainMutationQueue({
      queue: q,
      push,
      refreshRevision: async () => { throw new Error('refresh network failure'); },
      now: () => 1_000,
    ...
    push.mockClear();
    const second = await drainMutationQueue({ queue: q, push, now: () => 1_000, backoff: NO_JITTER_BACKOFF });
  ...
  it('dead-letters a non-retryable 422 returned by the post-refresh retry', async () => {
    ...
    expect(await q.pending()).toEqual([]);
    expect((await q.all())[0]).toMatchObject({ mutationId: 'm-refresh-422', status: 'rejected', baseRevision: 'sha256:repulled' });
  });

  it('does NOT dead-letter a retryable 4xx (409/429) — those stay queued', async () => {
    const q = await queueWith(mut({ mutationId: 'm1' }));
    const result = await drainMutationQueue({ queue: q, push: async () => { throw Object.assign(new Error('rate limited'), { status: 429 }); } });
  ...
  it('does NOT acknowledge when applyAck throws (the write is incomplete)', async () => {
    const q = await queueWith(mut({ mutationId: 'm1' }));
    const result = await drainMutationQueue({ queue: q, push: async (m) => ok(m), applyAck: async () => { throw new Error('write failed'); } });
  ...
  it('acknowledges ONLY the pushed mutation — an edit that lands mid-drain survives', async () => {
    const q = await queueWith(mut({ mutationId: 'm-create', operation: 'create', recordId: 'rec-A', queuedAt: '..1' }));
  ...
  it('discards an in-flight push that resolves after an abort (does not ack into a switched scope)', async () => {
    ...
    const controller = new AbortController();
    const applyAck = vi.fn(async () => {});
    const push = vi.fn(async (m: RecordMutation) => { controller.abort(); return ok(m); }); // aborts mid-flight
    const result = await drainMutationQueue({ queue: q, push, applyAck, signal: controller.signal });
  ...
  it('does not dead-letter a permanent push error when the signal aborts mid-push', async () => {
    ...
    const result = await drainMutationQueue({ queue: q, push, signal: controller.signal, observe: (e) => events.push(e) });
  ...
  it('does not acknowledge when the signal aborts DURING applyAck (no drop-after-cancel)', async () => {
    ...
    const result = await drainMutationQueue({ queue: q, push: async (m) => ok(m), applyAck, signal: controller.signal });
  ...
  it('emits a queue.write.drain event', async () => {
    const events: SyncEvent[] = [];
    const q = await queueWith(mut({ mutationId: 'm1' }));
    await drainMutationQueue({ queue: q, push: async (m) => ok(m), observe: (e) => events.push(e) });
    expect(events.at(-1)).toMatchObject({ type: 'queue.write.drain', fields: { scanned: 1, pushed: 1, conflicts: 0, failed: 0 } });
  });
...
describe('drainMutationQueue — retry backoff (ADR 0012)', () => {
  ...
  it('defers a mutation whose backoff window has not elapsed (does not push it)', async () => {
    ...
    const push = vi.fn(async (m: RecordMutation) => ok(m));
    const result = await drainMutationQueue({ queue: q, push, now: () => 1_000, backoff: NO_JITTER_BACKOFF });
    expect(push).not.toHaveBeenCalled();
  ...
  it('escalates the backoff on each successive failure', async () => {
    ...
    await drainMutationQueue({ queue: q, push: async () => { throw new Error('network'); }, now: () => 1_000, backoff: NO_JITTER_BACKOFF });
    const [queued] = await q.pending();
  ...
  it('emits a warn event when persisting the backoff fails (rare double-fault)', async () => {
    ...
    expect(events.some((e) => e.type === 'queue.write.reschedule-failed' && e.level === 'warn')).toBe(true);
  });

  it('a deferred head row does not consume the limit — a ready row behind it still pushes', async () => {
    const q = new RecordMutationQueue(new InMemoryRecordMutationStorage());
    await q.reschedule({ ...mut({ mutationId: 'm1', recordId: 'rec-A', queuedAt: '..1' }), attempts: 1, nextAttemptAt: at(9_000) }); // deferred
    await q.enqueue(mut({ mutationId: 'm2', recordId: 'rec-B', queuedAt: '..2' })); // ready, different record
    const push = vi.fn(async (m: RecordMutation) => ok(m));
    const result = await drainMutationQueue({ queue: q, push, now: () => 1_000, backoff: NO_JITTER_BACKOFF, limit: 1 });
  ...
  it('backs off after an ack failure too, not only a push failure', async () => {
    const q = await queueWith(mut({ mutationId: 'm1' }));
    vi.spyOn(q, 'acknowledge').mockRejectedValueOnce(new Error('storage down'));
    const result = await drainMutationQueue({ queue: q, push: async (m) => ok(m), now: () => 1_000, backoff: NO_JITTER_BACKOFF });
    expect(result).toMatchObject({ pushed: 0, failed: 1 });
    const [queued] = await q.pending();
    expect(queued.attempts).toBe(1);
  ...
  it('does NOT back off a push aborted by a scope switch (abort is not a failure) — and the row STAYS claimed (gate2 item 1)', async () => {
    const q = await queueWith(mut({ mutationId: 'm1' }));
    const controller = new AbortController();
    const push = vi.fn(async () => { controller.abort(); throw new Error('aborted'); });
    await drainMutationQueue({ queue: q, push, signal: controller.signal, now: () => 1_000, backoff: NO_JITTER_BACKOFF });
    const [queued] = await q.pending();
  ...
  it('a deferred mutation holds later edits to the same record (FIFO)', async () => {
    const q = new RecordMutationQueue(new InMemoryRecordMutationStorage());
    await q.reschedule({ ...mut({ mutationId: 'm1', recordId: 'rec-A', queuedAt: '..1' }), attempts: 1, nextAttemptAt: at(9_000) });
...
describe('drainMutationQueue — #507 claim + conflict lifecycle', () => {
  it("re-stamps an update's baseRevision from the record's CURRENT revision at drain time (creates keep null)", async () => {
    const q = await queueWith(
      mut({ mutationId: 'm-create', operation: 'create', recordId: 'rec-A', queuedAt: '..1' }),
      mut({ mutationId: 'm-edit', operation: 'update', recordId: 'rec-B', baseRevision: 'sha256:enqueue-time', queuedAt: '..2' }),
    );
    const bases: Array<string | null> = [];
    const currentRevision = vi.fn(async () => 'sha256:re-anchored');
    const result = await drainMutationQueue({
      queue: q,
      push: async (m) => { bases.push(m.baseRevision); return ok(m); },
      currentRevision,
    });
  ...
  it('transitions a 409 conflict to a durable conflicted row storing server truth, and it LEAVES the drain', async () => {
    ...
    expect(row).toMatchObject({ status: 'conflicted', conflictRevision: 'r1', conflictDocument: { id: 1 } });
  ...
  it('holds later pending mutations for a record while one of its mutations is conflicted', async () => {
    ...
    const push = vi.fn(async (m: RecordMutation) => (m.mutationId === 'm-first' ? conflict(m) : ok(m)));
  ...
  it('persists a dead-lettered row as durable status rejected — out of pending(), kept for the conflicts surface', async () => {
    const q = await queueWith(mut({ mutationId: 'm1' }));
    await drainMutationQueue({ queue: q, push: async () => { throw Object.assign(new Error('validation'), { status: 400 }); } });
    expect(await q.pending()).toEqual([]); // the record is syncable again
    expect((await q.all()).map((m) => ({ id: m.mutationId, status: m.status }))).toEqual([{ id: 'm1', status: 'rejected' }]);
  });
