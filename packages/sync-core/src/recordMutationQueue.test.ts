...
describe('RecordMutationQueue', () => {
  ...
  it('v1 → v2 migration preserves cross-row queuedAt order (per-doc, no other rows consulted)', () => {
    ...
    const early = recordMutationQueueMigrationStrategies[2](mut({ mutationId: 'zzz-early', queuedAt: '2026-06-26T00:00:00.000Z' }));
    const late = recordMutationQueueMigrationStrategies[2](mut({ mutationId: 'aaa-late', queuedAt: '2026-06-26T00:00:01.000Z' }));
  ...
  it('appends each mutation as a distinct immutable entry (no in-place coalescing)', async () => {
    ...
    await q.enqueue(mut({ mutationId: 'm-create', operation: 'create', recordId: 'rec-A', queuedAt: '2026-06-26T00:00:00.000Z' }));
    await q.enqueue(mut({ mutationId: 'm-edit', operation: 'update', recordId: 'rec-A', queuedAt: '2026-06-26T00:00:01.000Z' }));
  ...
  it('acknowledge removes ONLY the exact pushed mutationId — an edit that lands mid-push survives', async () => {
    ...
    await q.enqueue(mut({ mutationId: 'm-edit', operation: 'update', recordId: 'rec-A', queuedAt: '2026-06-26T00:00:02.000Z' }));
  ...
  it('keeps both a create and a later delete (no premature cancel of a maybe-in-flight create)', async () => {
    const q = new RecordMutationQueue(new InMemoryRecordMutationStorage());
    await q.enqueue(mut({ mutationId: 'm-create', operation: 'create', recordId: 'rec-A', queuedAt: '2026-06-26T00:00:00.000Z' }));
    await q.enqueue(mut({ mutationId: 'm-delete', operation: 'delete', recordId: 'rec-A', queuedAt: '2026-06-26T00:00:01.000Z' }));
  ...
  it('mints strictly increasing unique seqs across CONCURRENT enqueues (#507 E)', async () => {
    ...
    const queued = await Promise.all(
      ['m-1', 'm-2', 'm-3', 'm-4', 'm-5'].map((mutationId, index) => q.enqueue(mut({ mutationId, recordId: `rec-${index}` }))),
    );
    expect(queued.map((m) => m.seq)).toEqual([1, 2, 3, 4, 5]); // call order, no duplicates
    expect((await q.pending()).map((m) => m.mutationId)).toEqual(['m-1', 'm-2', 'm-3', 'm-4', 'm-5']);
  });
    ...
    const storage = {
      ...
      append: async (m: Parameters<InMemoryRecordMutationStorage['append']>[0]) => {
        if (!raced && m.mutationId === 'm-race') {
          ...
          await inner.append({ ...mut({ mutationId: 'm-rival', recordId: 'rec-R' }), seq: 1, status: 'pending' });
  ...
  it('a stranded same-seq pair drains superseded-first: the coalesce generation breaks the tie, not the mutationId', async () => {
    ...
    await q.replace({ ...mut({ mutationId: 'aa-replacement' }), seq: 1, coalesced: 1, status: 'pending' });
    expect((await q.pending()).map((m) => m.mutationId)).toEqual(['zz-superseded', 'aa-replacement']);
  });

  it('restores the original queue when inserting a removed chain fails after suffix rows were shifted', async () => {
    const inner = new InMemoryRecordMutationStorage();
    const original = [
      { ...mut({ mutationId: 'm-earlier', recordId: 'rec-B' }), seq: 1, status: 'pending' as const },
    ...
    const removed = [
      { ...mut({ mutationId: 'm-create', operation: 'create' }), seq: 1, status: 'pending' as const },
  ...
  it('pending() excludes rejected dead letters but keeps claimed, conflicted, and needs-revision rows', async () => {
    ...
    expect((await q.pending()).map((m) => m.mutationId)).toEqual(['m-claimed', 'm-conflicted', 'm-needs-revision']);
    expect((await q.all()).map((m) => m.mutationId).sort()).toEqual(['m-claimed', 'm-conflicted', 'm-needs-revision', 'm-rejected']); // terminal rows persist for conflicts()
  });
...
describe('RecordMutationQueue — conditional (CAS) transitions (#507 P1-2)', () => {
  ...
  it('coalesceInto swaps only a still-pending prior; a claimed or consumed prior refuses untouched', async () => {
    ...
    const replacement = { ...mut({ mutationId: 'm-replacement', payload: { id: 'rec-A', v: 2 } }), seq: prior.seq, coalesced: 1, status: 'pending' as const };
  ...
  it('removePending refuses a claimed row — an in-flight create cannot be annihilated', async () => {
    ...
    expect((await q.all()).map((m) => ({ id: m.mutationId, status: m.status }))).toEqual([{ id: 'm-create', status: 'claimed' }]);
  ...
  it('#516 review P1: enqueueWhen appends only while the precondition holds against the in-turn re-read', async () => {
    ...
    const appended = await q.enqueueWhen(mut({ mutationId: 'm-b', recordId: 'rec-B' }), (rows) => rows.length === 1);
    ...
    expect(await q.enqueueWhen(mut({ mutationId: 'm-c', recordId: 'rec-C' }), (rows) => rows.length === 1)).toBeNull();
...
describe('pendingRecordIds', () => {
  it('collects the record ids of all pending mutations (for the pull-apply guard)', () => {
    const ids = pendingRecordIds([mut({ recordId: 'rec-A' }), mut({ recordId: 'rec-B' }), mut({ recordId: 'rec-A' })]);
...
describe('RxRecordMutationStorage', () => {
  ...
  function fakeCollection() {
    ...
    const col = {
      store,
      bulkUpsert: async (items: RecordMutation[]) => { for (const m of items) store.set(m.mutationId, m); },
      find: () => ({ exec: async () => [...store.values()].map((m) => ({ toJSON: () => m })) }),
      bulkRemove: async (ids: string[]) => { for (const id of ids) store.delete(id); },
    };
    return col;
  }
  ...
  it('remove of an empty list is a no-op (skips bulkRemove)', async () => {
    ...
    let called = false;
    col.bulkRemove = async () => { called = true; };
  ...
  it('remove rejects when RxDB resolves with a per-row failure', async () => {
    ...
    await expect(new RxRecordMutationStorage(col).remove(['m-create'])).rejects.toThrow('mutation queue remove');
  });
