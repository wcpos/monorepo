...
import { normalizeCheckpoint, type PullResponse, type SyncCheckpoint } from '@woo-rxdb-lab/shared';
import { createFakePullServer, fakeUuid, type FakePullServer } from './fakePullServer';
import { runScopeGuardedPull } from './scopeGuardedPull';
import { StoreScopeManager, type Fetcher, type ScopeDatabase } from './storeScopeManager';
...
function seededServer(): FakePullServer {
  ...
  server.seed({ uuid: fakeUuid(1), wooOrderId: 1, modifiedGmt: '2026-01-01 00:00:00', revision: 'r1' });
  server.seed({ uuid: fakeUuid(2), wooOrderId: 2, modifiedGmt: '2026-01-01 00:00:00', revision: 'r2' });
  return server;
}

/** The checkpoint the seeded fake advances to (its second row, sequence 2). */
const seededServerCheckpoint: SyncCheckpoint = { updatedAtGmt: '2026-01-01 00:00:00', orderId: 2, revision: 'r2', sequence: 2 };
...
describe('runScopeGuardedPull', () => {
  ...
  it('forwards the F8 journal-epoch hooks + resetForResync through the guarded wrappers (Codex P2)', async () => {
    const manager = await makeManager();
    const resetCalls: Array<ReadonlySet<string | number> | undefined> = [];
    const repository = {
      upsertMany: async (): Promise<void> => undefined,
      resetForResync: async (pending?: ReadonlySet<string | number>): Promise<void> => { resetCalls.push(pending); },
    };
    const epochWrites: string[] = [];
    let storedEpoch: string | undefined = 'epoch-OLD';
    let checkpoint = normalizeCheckpoint({ updatedAtGmt: 'x', orderId: 1, revision: 'r', sequence: 1 });
    const store = {
      readCustomPullCheckpoint: async (): Promise<SyncCheckpoint> => checkpoint,
      writeCustomPullCheckpoint: async (next: SyncCheckpoint): Promise<void> => { checkpoint = next; },
      readJournalEpoch: async (): Promise<string | undefined> => storedEpoch,
      writeJournalEpoch: async (epoch: string): Promise<void> => { storedEpoch = epoch; epochWrites.push(epoch); },
    };
  ...
  it('forwards includeDeletes to the pull request so the delete hook is reachable (Codex P2)', async () => {
    ...
    const server = seededServer();
    const urls: string[] = [];
    const fetcher: Fetcher = async (url, init) => { urls.push(String(url)); return server.fetch(String(url), init ?? undefined); };
  ...
  it('re-resolves the pending provider for the resync purge on a scoped reset (Codex P1)', async () => {
    const manager = await makeManager();
    let providerCalls = 0;
    const provider = async (): Promise<ReadonlySet<string | number>> => { providerCalls++; return new Set<string | number>(); };
    const resetCalls: Array<ReadonlySet<string | number> | undefined> = [];
    const repository = {
      upsertMany: async (): Promise<void> => undefined,
      resetForResync: async (pending?: ReadonlySet<string | number>): Promise<void> => { resetCalls.push(pending); },
    ...
    const store = {
      ...
      writeCustomPullCheckpoint: async (next: SyncCheckpoint): Promise<void> => { checkpoint = next; },
      readJournalEpoch: async (): Promise<string | undefined> => storedEpoch,
      writeJournalEpoch: async (epoch: string): Promise<void> => { storedEpoch = epoch; },
    };
    const server = createFakePullServer({ epoch: 'epoch-NEW' });
...
describe('ticket-bound fetching (stale pulls refuse network work)', () => {
  ...
  it('classifies a response landing after a switch as stale, write never invoked', async () => {
    ...
    const fetcher: Fetcher = (url) => new Promise<Response>((resolve) => { requestedUrl = String(url); resolveFetch = resolve; });
