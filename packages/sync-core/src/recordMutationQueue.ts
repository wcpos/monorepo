...
export class RecordMutationQueue {
  ...
  async enqueue(mutation: RecordMutation): Promise<QueuedMutation> {
    return this.transact(async () => {
      for (;;) {
        ...
        try { await this.storage.append(queued); return queued; } catch (error) {
   ...
   */
  async enqueueWhen(mutation: RecordMutation, unchanged: (rows: QueuedMutation[]) => boolean): Promise<QueuedMutation | null> {
    return this.transact(async () => {
      for (;;) {
  ...
  async claim(next: QueuedMutation): Promise<boolean> {
    return this.transact(async () => {
      ...
      if (!row || (row.status !== undefined && row.status !== 'pending' && row.status !== 'claimed')) return false;
  ...
  async restoreAheadOfRecordNewcomers(rows: readonly QueuedMutation[]): Promise<QueuedMutation[]> {
    return this.transact(async () => {
      ...
      const head = rows[0];
      const newcomers = existing.filter((item) => item.collectionName === head.collectionName && item.recordId === head.recordId);
      const insertionSeq = newcomers.length > 0
   ...
   */
  async replace(mutation: QueuedMutation): Promise<void> { await this.storage.append(mutation); }

  /** Remove exact entries by `mutationId` — coalesce supersession, annihilation, and conflict discard. */
  async remove(mutationIds: readonly string[]): Promise<void> { await this.storage.remove(mutationIds); }

  /** EVERY stored entry, terminal rows included — the engine's `conflicts()` reads this. */
  async all(): Promise<QueuedMutation[]> { return this.storage.list(); }
...
export type RxRecordMutationCollection = {
  ...
  find(query?: unknown): { exec(): Promise<Array<RxRecordMutationDocument | QueuedMutation>> };
...
export class RxRecordMutationStorage implements RecordMutationStorage {
  public constructor(private readonly collectionOrResolver: RxRecordMutationCollection | (() => RxRecordMutationCollection)) {}

  private collection(): RxRecordMutationCollection {
    return typeof this.collectionOrResolver === 'function' ? this.collectionOrResolver() : this.collectionOrResolver;
  }
  ...
  async remove(mutationIds: readonly string[]): Promise<void> {
    if (mutationIds.length === 0) return;
    assertBulkSuccess(await this.collection().bulkRemove([...mutationIds]), 'mutation queue remove');
  }
}
...
export const recordMutationQueueSchema = {
  ...
  properties: {
    ...
    status: { type: 'string', enum: ['pending', 'claimed', 'conflicted', 'needs-revision', 'rejected'] },
...
export const recordMutationQueueMigrationStrategies = {
  ...
  2: (doc: QueuedMutation): QueuedMutation => ({
    ...doc,
    seq: doc.seq ?? (Math.max(0, Date.parse(doc.queuedAt)) * 1_000 + [...doc.mutationId].reduce((n, c) => (n * 33 + c.charCodeAt(0)) % 1_000, 0) + 1),
