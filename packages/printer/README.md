...
- **2026-09-05 · Netum NT-1809 (58 mm generic) over USB on Electron · receipt at 32 columns.** Found
  ...
  honours it (double-centring). Fixed by emitting each line through the aligned path (Spec I1).
  The dotted rule and the shifted payment amounts in the same photo were **not** in the emitted
  bytes — the raw job is now logged as hex at debug level (Spec I2) so the next test compares bytes,
  not theories. The diagnostic test print at 32 columns was perfect, which is what narrowed it.
