...
function isCanonicalShape(data: Record<string, any>): boolean {
	...
	if (meta && typeof meta === 'object') {
		if ('schema_version' in meta || 'order_id' in meta) {
			return true;
		}
	}
	const totals = data.totals;
	if (totals && typeof totals === 'object') {
		if ('subtotal_incl' in totals) {
			return true;
		}
	}
	return false;
...
function mapLine(src: Record<string, any>): ReceiptLineItem {
	...
	return {
		key: toStr(src.key) || toStr(src.sku) || `line-${Math.random().toString(36).slice(2, 8)}`,
		sku: toStr(src.sku),
		name: toStr(src.name),
		qty,
...
export function mapReceiptData(data: Record<string, any>): ReceiptData {
	...
	return {
		...
		lines: toArr(data.lines).map((item) =>
			mapLine(item && typeof item === 'object' ? item as Record<string, any> : {})
		),
		fees: [] as ReceiptFee[],
