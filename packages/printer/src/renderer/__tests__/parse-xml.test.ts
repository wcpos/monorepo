...
describe('parseXml', () => {
		...
		expect(row).toEqual({
			...
			children: [
				{ type: 'col', width: 24, align: 'left', children: [{ type: 'raw-text', value: 'left' }] },
				{
					...
					width: 24,
					align: 'right',
					children: [{ type: 'raw-text', value: 'right' }],
				},
			],
		});
	});
	...
	it('defaults col align to left and width to 12', () => {
		...
		const row = ast.children[0] as {
			type: 'row';
			children: { type: string; width: number; align: string }[];
		};
	...
	it('parses barcode with type, height, and text content', () => {
		const ast = parseXml(
			'<receipt><barcode type="ean13" height="60">5901234123457</barcode></receipt>'
		);
	...
	it('throws when root is not <receipt>', () => {
		expect(() => parseXml('<document></document>')).toThrow(
			'Expected <receipt> root element, got <document>'
		);
	});
	...
	it('parses col with width="*"', () => {
		const ast = parseXml(
			'<receipt><row><col width="*">name</col><col width="10" align="right">price</col></row></receipt>'
		);
		const row = ast.children[0] as { type: 'row'; children: { width: number | '*' }[] };
		expect(row.children[0].width).toBe('*');
		expect(row.children[1].width).toBe(10);
	});

	it('parses multiple star columns', () => {
		const ast = parseXml(
			'<receipt><row><col width="*">a</col><col width="6">b</col><col width="*">c</col></row></receipt>'
		);
		const row = ast.children[0] as { type: 'row'; children: { width: number | '*' }[] };
		expect(row.children[0].width).toBe('*');
		expect(row.children[1].width).toBe(6);
		expect(row.children[2].width).toBe('*');
	});
});
