...
The **primary axis** of the Add Printer dialog: _how the printer is reached_. One of
...
depend on it. It is _not_ a top-level peer of "Scan Network." On web, only `epson` and
...
- **Vendor HTTP(S) protocol** — the only network path a browser has:
  - **Epson ePOS-Print** — HTTPS port `443` (probed first; the only lane that prints with
    Secure Printing on) or `8043`, HTTP `80` / `8008` (Secure Printing off only), endpoint
    `/cgi-bin/epos/service.cgi`. Structured ePOS XML, never ESC/POS bytes in `<command>`, on
    RED-era printers. See `README.md` (doctrine + lessons log).
...
device does not edit them. Only _routing_ (template → cloud printer) is device-local.
...
(wcpos/woocommerce-pos#1351). Only a legacy profile with no or an unknown provider
still uploads raw pre-encoded bytes — as does any one-off payload with no order behind
it, such as a standalone cash-drawer kick.

### Printer target id (routing)

The stable id a template routing override points at (persisted in
`template_printer_overrides.printer_profile_id`, which is a _generic target id_, not
...
The properties that describe the _physical printer_, not how it is reached: Name,
