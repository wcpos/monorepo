...
// Both are exact-pinned devDependencies of this package. The pin on `playwright` must track
// whatever version the rest of the workspace declares — `scripts/check-dep-duplicates.mjs`
// fails the build if two packages resolve different versions of the same dependency.
//
// The output does not appear to depend on the browser build: the committed bitmap has
// regenerated byte for byte across playwright 1.62.1 and 1.63.0 and two Chromium revisions,
// which is what thresholding to pure black and white before encoding is for.
