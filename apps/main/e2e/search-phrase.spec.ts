...
for (const qualifier of ['MY', 'A']) {
	test(`preserves ${qualifier} in cold and warm Georgian phrase searches`, async ({
		posPage: page,
	}, testInfo) => {
		const probe = mintSearchProbeToken(testInfo.workerIndex);
		const phrase = `${qualifier} საბარგული ${probe}`;
		const offset = 900_000_000 + Math.floor(Math.random() * 90_000_000);
		const names = [
			`xxxx ${phrase} xxxx`,
			`M3 საბარგული ${probe}`,
			`საბარგული ${qualifier} ${probe}`,
			`${qualifier} xxxx საბარგული ${probe}`,
		];
		...
		await page.route(
			...
			async (route) => {
				...
				// Broad response intentionally exercises local phrase filtering, not PHP SQL.
		...
		for (const visit of ['cold', 'warm']) {
			...
			await expect(row(offset).first(), visit).toBeVisible();
			await expect(page.getByTestId('data-table-loaded-count').first()).toHaveText('1');
			for (let i = 1; i < rows.length; i++) await expect(row(offset + i)).toHaveCount(0);
			await input.fill('');
