...
| App log `engine.lane.tick`, `lane: change-signal`, `status: error`, RxDB `COL22`, collection `coupons` | Duplicate UUIDs in a greedy refresh batch; overlapping numbered pages reproduce it. In run 34994405295 attempt 1 the error was at 16:46:57, a successful signal cycle followed at 16:47:58, and flow 10 started at 16:58:05 in a new app process — not a wedged lane causing that burst failure. The captured payload is truncated, so the exact server-side overlap is unverified. | `refreshPrunable` coalesces projected rows by UUID (last fetched occurrence wins), before local-work protection and set-difference pruning; a real-RxDB regression covers same-page and cross-page duplicates | swallow COL22; weaken the quantity assertion; infer causation from one red run |
